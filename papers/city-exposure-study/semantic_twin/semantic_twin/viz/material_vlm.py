...
def plot_material_vlm(out: pathlib.Path, crop: str, mode: str) -> None:
    ...
    apply_monograph_style(mode=mode, constrained_layout=True)
    figure, axes = plt.subplots(2, 3, figsize=fig_size_ieee(columns=2, aspect=0.58))

    # Panel a, the two sources side by side.
    panel = axes[0, 0]
    left = np.asarray(Image.open(out / "crops" / f"{crop}_panorama.png"))
    right = np.asarray(Image.open(out / "crops" / f"{crop}_texture.png"))
    ...
    panel.legend(fontsize=4.5, ncol=2, loc=LEGEND_LOWER_RIGHT)
    ...
    panel.legend(fontsize=4.5, loc=LEGEND_LOWER_RIGHT)
    ...
    suffix = "pdf" if mode == "pdf" else "png"
    path = out / f"material_vlm.{suffix}"
