...
$10.2\,\%$ of body-slots; ZF with primal projection holds violations
at $0.0\,\%$ at $74.0\,$Gbps, the MCS28 sum-cap, by
feasibility-by-construction. The multi-body MMSE-with-exposure
dual-ascent QCQP of Ying et al.~\cite{Ying2017} extended to $K + B$
bodies, when its noise-power regulariser is matched to the receiver
floor, attains the same MCS sum-cap on the same instance but at
$\sim 2\,\%$ residual violations from imperfect dual convergence on
a rank-truncated solver. Projected ZF is the preferred deployable
consumer of the body twin: it is a single scalar scaling at
$\sim 0.5\,$ms per slot vs $\sim 350\,$ms for the dual ascent, and
its compliance is by construction. The dual-ascent solver is the
right tool for the academic uncapped-Shannon objective and the
min-TX-power formulation; we provide it in \cref{app:dualascent}. The naive projected-area Cauchy bound
...
leaves enough SINR headroom above the $2^{S_{\max}} - 1 \approx
22.3\,$dB MCS28 clip threshold to absorb the worst-body overshoot
factor $\rho/\eta_S$ without falling below the cap. In the
plaza-scale binding regime of \cref{sec:eval} we measure $\rho \in
[4, 17]$ at $E_{\mathrm{RL}} = 3\,$V/m, so the hypothesis is
equivalent to $\mathrm{SINR}^{\mathrm{ZF}}_{\min} \geq
(\rho/\eta_S)\,(2^{S_{\max}} - 1) \approx 17.5 \cdot 168 \approx
34.7\,$dB. Empirically, ZF saturates the MCS sum-cap of
...
\begin{figure*}[!t]
  ...
  the dual ascent ($0.85\,\%$ violation, $20.75\,$Gbps under
...
regime tested, both algorithms saturate the MCS sum-cap; projected
ZF wins on compliance at $0\,\%$ violations by construction while
the rank-truncated dual ascent leaves $0.5$-to-$2.4\,\%$ residual
violations.
...
$\Qmat^{(u)}$ on the $600$-slot trace at $E_{\mathrm{RL}} =
3\,$V/m, $43\,$dBm. \emph{ZF + proj} is the closed-form scaling of
\cref{eq:primal-projection}. \emph{Dual ascent (tuned)} is the
multi-body MMSE-with-exposure of \cref{app:dualascent} with its
noise-power regulariser matched to the receiver floor and the
projection step at exit. The MCS28 cap of $7.4\,$bps/Hz at 400 MHz
puts $K\,S_{\max}\,$BW at $74.0\,$Gbps for $K = 25$, $29.6\,$Gbps
for $K = 10$, $14.8\,$Gbps for $K = 5$. Both algorithms saturate
the cap; projected ZF achieves zero violations in every case
while dual ascent leaves a small residual.}
\label{tab:baselines}
\centering
\small
\begin{tabular}{@{}lrrrr@{}}
...
The compliance gap is what separates the two methods. Projected
ZF achieves $0\,\%$ violations by construction: the projection
scale $\alpha$ is computed against the full-rank $\Qmat^{(u)}$,
so post-scaling absorption $\leq \eta_S\,L^{(u)} \leq L^{(u)}$ for
every body. The dual ascent operates at rank-$r$ truncation of
$\Qmat^{(u)}$ ($r = 4$ at the $99\,\%$ trace fraction) so the dual
variables converge to a rank-$r$-faithful active set; the residual
$1$-to-$5\,\%$ absorption tail at full rank exits the solver as a
$0.5$-to-$2.4\,\%$ violation rate. The exit-projection step trims
the worst-margin body but does not re-enter the dual ascent against
...
The mechanism of the residual violation is the rank-truncated
operator $\Qmat^{(u)}$ used inside the dual-ascent solver: the
solver uses $r = 4$ at the $99\,\%$ trace fraction but the cap is
checked against the full-rank operator at evaluation time, leaving
a $1$-to-$5\,\%$ absorption tail that the dual ascent does not
see. The exit projection trims the worst-margin body to the safety
fraction $\eta_S$ but does not re-enter the dual ascent against
the corrected residuals. T-pose specifically adds a second source
of mismatch: the canonical pose has the arms extended laterally
while the walking population has the arms inward, so the T-pose
$\Qmat^{(u)}$ misrepresents the absorption-mode geometry in some
BS-precoder directions. Both sources are absent from projected ZF,
which projects against the full-rank pose-aware $\Qmat^{(u)}$
directly and is feasibility-by-construction at every slot
regardless of pose source.
...
\begin{figure}[!t]
  ...
  below the T-pose-fallback rate. Green dashed: mean sum-rate. The
  T-pose-fallback asymptote (dotted) is dominated on both metrics
  across the deployment-realistic
...
highest unconstrained sum-rate but at $10.2\,\%$ violation rate.
ZF with primal projection holds violations at $0.0\,\%$ at the MCS
sum-cap of $74.0\,$Gbps; the dual-ascent multi-body
MMSE-with-exposure, with its noise-power regulariser matched to
the receiver noise floor (a deployable choice that the original
Ying et al. formulation~\cite{Ying2017} silently makes), tracks the
same MCS sum-cap but exits at $\sim 2\,\%$ residual violations
because the rank-truncated dual ascent leaves a $1$-to-$5\,\%$
absorption tail that the exit projection only partially closes.
...
under the deployable rate model is structural, not numerical. The
per-stream MCS28 cap of $7.4\,$bps/Hz clips the rate model at the
$\sim 30\,$dB SINR threshold. Both algorithms saturate this cap in
the binding regime; what differs is feasibility. Projected ZF is
feasibility-by-construction: the projection scale $\alpha$ in
\cref{eq:primal-projection} is computed against the full-rank
$\Qmat^{(u)}$ at every slot, so $\Pabs^{(u)} \leq \eta_S\,L^{(u)}$
holds for every body in the constraint set. The dual ascent
operates at rank-$r$ truncation of $\Qmat^{(u)}$ ($r = 4$ at the
$99\,\%$ trace fraction), so the dual variables converge to the
rank-$r$-faithful active set; the residual $1$-to-$5\,\%$
absorption tail that the dual ascent does not see exits the solver
as a $\sim 2\,\%$ violation rate against the full-rank operator.
The exit-projection step closes part of that gap but cannot do so
without re-running the full QCQP. Projected ZF absorbs the same
gap upstream because its projection step \emph{is} the optimum, not
a safety net. The dual-ascent solver retains its place in the
academic uncapped-Shannon objective, the min-TX-power objective,
and the SINR-margin-tight regime (low-element panels, dense
served-user counts) where uniform scaling can drop the served-user
SINR below the clip.
...
sum-cap of $74.0\,$Gbps. The dual-ascent QCQP solver, when its
noise-power regulariser is matched to the receiver floor, attains
the same MCS sum-cap on the same instance but with $\sim 2\,\%$
residual violations from imperfect dual-variable convergence at the
rank-truncated solver exit. The same conclusion holds at $K = 10$
and $K = 5$ served-user counts and on a bystander-binding geometry
...
with zero violations in every case; the dual-ascent solver tracks
the rate when properly tuned but never matches the projection's
hard-feasibility property at exit. The naive projected-area bound on
