...
- Run `python -m ruff check src/ tests/` and `python -m ruff format --check src/ tests/` before committing. Fix issues first.
- Optionally run `python -m pytest tests/ -m "not slow" -x` locally. CI runs `pytest tests/` on every push and PR.
