...
from semantic_twin.vision.mesh_depth import first_hit_range
...
class BodyPlacementConfig:
    reconstructions: pathlib.Path
    pose: pathlib.Path
    mesh: pathlib.Path
    out: pathlib.Path
    depth_compare: pathlib.Path | None = None
    mesh_depth: pathlib.Path | None = None
...
def place_dynamic_bodies(config: BodyPlacementConfig) -> None:
    args = config
