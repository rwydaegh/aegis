...
def main() -> None:
    ...
    ap.add_argument("--site", default="korenmarkt")
    ap.add_argument(
        "--locations",
        default=None,
    ...
    ap.add_argument("--standpoints", type=int, default=20, help="how many, taken lowest sky fraction first")
    ap.add_argument("--azimuth", type=int, default=720)
    ap.add_argument("--elevation", type=int, default=600)
    ap.add_argument("--elevation-floor-deg", type=float, default=0.05)
    ap.add_argument("--out", default=None)
    args = ap.parse_args()

    loc = pathlib.Path(
        args.locations
        or ROOT / "outputs" / "exposure_korenmarkt" / f"city250_corrected_{args.site}_15ghz_locations.jsonl"
    )
    records = []
    ...
    if args.standpoints > 0:
        records = records[: args.standpoints]

    manifest = json.loads(loc.with_name(loc.name.replace("_locations.jsonl", "_manifest.json")).read_text())
    mesh = pathlib.Path(args.mesh or manifest["mesh"])
    ...
    unit, omega, _ = direction_grid(args.azimuth, args.elevation, args.elevation_floor_deg)
    ...
    out = pathlib.Path(args.out or ROOT / "outputs" / "diffraction_bound" / f"{args.site}_diffraction_bound.json")
        ...
        json.dumps(
            {
                ...
                "grid": {
                    ...
                    "elevation_floor_deg": args.elevation_floor_deg,
                },
                "standpoints": rows,
            },
            indent=2,
