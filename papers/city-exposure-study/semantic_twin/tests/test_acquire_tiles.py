...
from semantic_twin.acquire.tiles import (
    ...
    GoogleTilesDownloader,
    RegionOfInterest,
    ball_ground_clearance_m,
    ball_horizontal_reach_m,
    ball_radius_for_reach,
    check_reaches_terrain,
    sanitized_uri,
)
from semantic_twin.scene.enu import enu_rotation, llh_to_ecef

#: Piazza del Duomo's pavement, measured off the tiles and recorded in
#: ``data/geometry/milan_duomo/ACQUISITION.md``. Every number in the ball table
#: below is quoted from that file.
MILAN_PAVEMENT_M = 162.93862133337055
...
def test_the_ball_reach_formula_reproduces_the_milan_acquisition_table() -> None:
    """The three radii that acquisition considered, and what each one reached.

    Quoted from ``ACQUISITION.md``. 200 m was the first pull and it truncated
    the cathedral. 258 m is the smallest ball that would have reached 200 m
    horizontally. 320 m is what was bought, so the band would still clear air
    above the far ring.
    """
    assert ball_horizontal_reach_m(200.0, MILAN_PAVEMENT_M) == pytest.approx(116.0, abs=0.05)
    assert ball_horizontal_reach_m(258.0, MILAN_PAVEMENT_M) == pytest.approx(200.0, abs=0.05)
    assert ball_horizontal_reach_m(320.0, MILAN_PAVEMENT_M) == pytest.approx(275.4, abs=0.05)
    assert ball_radius_for_reach(200.0, MILAN_PAVEMENT_M) == pytest.approx(258.0, abs=0.5)

    # At Ghent, 50 m up, a nominal 200 m ball still reaches 194 m and the
    # defect is invisible. That is why it survived a whole site.
    assert ball_horizontal_reach_m(200.0, 50.87) == pytest.approx(193.4, abs=0.1)


def test_the_first_milan_pull_did_not_reach_the_ground_past_a_hundred_and_sixteen_metres() -> None:
    """Horizontal reach alone hides the case that actually truncated the mesh."""
    assert ball_ground_clearance_m(200.0, MILAN_PAVEMENT_M, 0.0) == pytest.approx(37.1, abs=0.05)
    assert ball_ground_clearance_m(200.0, MILAN_PAVEMENT_M, 130.0) < 0.0
    assert ball_ground_clearance_m(200.0, MILAN_PAVEMENT_M, 160.0) < 0.0
    assert ball_ground_clearance_m(320.0, MILAN_PAVEMENT_M, 130.0) == pytest.approx(129.5, abs=0.05)
    assert ball_ground_clearance_m(320.0, MILAN_PAVEMENT_M, 160.0) == pytest.approx(114.2, abs=0.05)


def test_a_two_hundred_metre_request_at_milan_now_reaches_two_hundred_metres() -> None:
    """The regression the cylinder exists to prevent, at the site that met it.

    Under the ball this box sat 195 m out at pavement level and was pruned,
    because its 3D distance from an ellipsoid-centred origin is 253 m. It has to
    be kept now, and a box genuinely outside the radius still has to go.
    """
    lat, lon, radius = 45.4642, 9.19, 200.0
    roi = RegionOfInterest(lat, lon, radius, 3000.0)
    east, _, up = enu_rotation(lat, lon)
    inside, transform = unit_box(roi.center + MILAN_PAVEMENT_M * up + 195.0 * east, 1.0)
    outside, outside_transform = unit_box(roi.center + MILAN_PAVEMENT_M * up + 260.0 * east, 1.0)
    assert roi.intersects_box(inside, transform)
    assert not roi.intersects_box(outside, outside_transform)
    assert ball_horizontal_reach_m(radius, MILAN_PAVEMENT_M) < 195.0


def test_a_site_whose_terrain_sits_outside_the_band_is_refused_with_its_own_height() -> None:
    """Otherwise the traversal returns nothing, which reads as a bad radius."""
    with pytest.raises(ValueError, match="3600"):
        check_reaches_terrain(3000.0, 3300.0)
    with pytest.raises(ValueError, match="terrain is outside"):
        RegionOfInterest(-16.5, -68.15, 250.0, 3000.0, site_ellipsoid_height_m=3300.0)
    # Milan, the highest site in the study, clears the default with room over.
    RegionOfInterest(45.4642, 9.19, 320.0, 3000.0, site_ellipsoid_height_m=MILAN_PAVEMENT_M)
...
def tile_downloader(tmp_path: pathlib.Path, http: FakeHttp) -> GoogleTilesDownloader:
    return GoogleTilesDownloader(
        "real-api-key",
        lat=51.055,
        lon=3.722,
        radius_m=100.0,
