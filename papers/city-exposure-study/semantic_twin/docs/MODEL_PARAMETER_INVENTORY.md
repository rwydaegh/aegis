...
| Comparable-city v2 transport | 250 m, 15 GHz, 200000 IID rays, 4096 passive output cells, `first_material_interaction_v1`, exact direct, exact order-1 all-specular, and stochastic first-diffuse next-event estimation at the first blocking material vertex, finish-only roughness, physical 3D edge-length baseline, level-2 body with explicit CUDA Float64 reduction or NumPy reference | `parameters.study_transport.comparable_city_v2_production` | comparable production transport and coupling | canonical | yes | horizontal projected edge length is a sensitivity. 4096 cells are not launch bins. Candidate caps, broad-phase thresholds, and chunks are computational guards, not model parameters. Body backend is recorded in provenance. | No mixed suffix or higher specular order is in the current contract. |
| Historical hybrid interaction budget | 3 interactions | `RunConfig.max_bounces`, `DEFAULT_MAX_BOUNCES` | sensitivity evidence | diagnostic | conditional | current production contract is `first_material_interaction_v1`. Studies use 4 and order 8 | `max_bounces=3` does not define a complete three-bounce production model. |
...
| Historical roofline sampled-specular pilots and paired campaign | Pilot code `0844eadde0308ff8dfeda7142ec3903165f5a053`. Korenmarkt and Prague, 250 m crop, atlas, full panorama-link route, route-tangent yaw, 200000 rays, 400000 batch, 4096 cells, 15 GHz, seed 7, curve 1e-4 m, top-edge tolerance 0.25 m, specular order exactly 1, candidate budget 120000000, chunk 262144, relative tolerance 0.02, sampled suffix one sample per eligible diffuse vertex with offset 2000. Paired config commit `b985ab58` uses IID and Fibonacci modes, seeds 7-22, and looks 4, 8, 12, and 16 | `config/roofline_campaign_*_pilot_cuda_sampled_specular.json`, `config/roofline_campaign_*_convergence_cuda_{iid,rotated_fibonacci}.json`, `semantic_twin/exposure/roofline_setup.py`, `semantic_twin/walk/orientation.py` | historical hybrid sensitivity evidence | diagnostic | conditional | `max_bounces=3` is an interaction cap, not three specular reflections. The sampled suffix is absent from current production. Candidate budgets and chunks are computational guards. | Korenmarkt and Prague pilots and paired campaigns passed their historical audits. Fibonacci is not adopted. See [roofline campaign results](ROOFLINE_CAMPAIGN_RESULTS.md). |
...
| Comparable runtime anchors | The historical hybrid Korenmarkt provider-corridor campaign completed 16 replicas over 10 standpoints in 83.37 s wall, with 61.584 s accumulated estimator, 42.119 s stochastic, 13.317 s specular, and 2.041 s body work. Landed seed-independent device kernels separately measured 3.14-3.24 s for the 14-point stochastic stage versus roughly 62 s before the fix. CUDA level-2 body coupling is exact to 6.64e-16 relative error, 46x faster on Duke and 81x on the real-point benchmark. Persistent Korenmarkt cache is 74.09 to 38.90 s wall and 39.22 to 4.35 s estimator with byte-identical arrays. | `parameters.runtime_and_visualisation.comparable_runtime_anchors` | measured implementation timing | diagnostic | no | measured historical and stage anchors, not free runtime parameters | Values are hardware-specific observations. The 83.37 s run is historical hybrid evidence, not the current baseline. The roughly 62 s value is a historical kernel-compilation symptom. Route contracts remain different estimands. |
| CUDA Float64 specular broad phase | about 96 MiB resident memory, host exact final kernel unchanged, byte-identical path arrays, zero lost candidates, Mexico 16.596 to 0.694 s, Tokyo 35.862 to 1.084 s, independent Mexico subset 6.8x exact parity | certified CUDA transport kernel | performance implementation | canonical | no | CPU broad-phase percentages remain historical audit measurements | Real full-solve speedups are 23.9x Mexico and 33.1x Tokyo. |
| Resident minimal CUDA reductions | reduced fields plus about 216 bytes of scalar metadata instead of about 11.5 MB per-ray host transfer, rich and audit path retained | certified CUDA transport kernel | performance implementation | canonical | no | plane microbenchmark 4.2x is not a city speedup claim | Real Korenmarkt point 0 fell from 0.0925 to 0.0756 s, or 1.22x. Independent rich parity is exact or within measured roundoff. |
| Current first-material-interaction campaigns | Korenmarkt 10 points and 16 replicas in 29.79 s, Prague 22 and 16 in 69.02 s, Madrid 14 and 16 in 40.70 s, Mexico City 11 and 16 in 30.92 s, Tokyo Hachiko 16 and 16 in 50.11 s. Seeds 7-22, looks 4, 8, 12, and 16, 200000 IID rays, 4096 output cells. | current first-material-interaction campaign manifests | multicity result snapshot | diagnostic | no | not a parameter change or a ten-city completion claim | 12-to-16 total-transfer changes are 0.0000819, 0.0001559, 0.0004083, 0.043625, and 0.019732 dB. Zero-direct Mexico points 0, 1, 3 and Tokyo points 13, 14, 15 remain included. |
| Historical provider-corridor hybrid campaigns | Korenmarkt 10 points and 16 replicas in 83.37 s, Prague 22 and 16 in 185.68 s, Madrid 14 and 16 in 115.75 s, Mexico City 11 and 64 in 219.52 s, Tokyo Hachiko 16 and 64 in 325.76 s. | historical provider-corridor campaign manifests | sensitivity result snapshot | diagnostic | no | not a current baseline or a ten-city completion claim | Mexico and Tokyo timings are not direct comparisons with current first-material-interaction campaigns. |
| Source-conditioned mixed-suffix screen | rejected experimental result, no production value | `SOURCE_CONDITIONED_SUFFIX_PILOT.md` | historical estimator experiment | diagnostic | conditional | not landed and not pending production | It removed zero scores but failed the variance-time gate. |
...
| Comparable-city v2 contract | Ten intended sites, nine panorama acquisitions, Krakow quota-pending, all primary materials atlas, registered-span street v1 route, 90 m radius and 6 m stride, Times Square excluded. Opt-in provider-corridor v1 uses a continuous 20 m evidence gap and leaves legacy route bytes unchanged | `parameters.study_transport.comparable_city_v2_contract` | cohort membership and route contract | canonical | yes | does not duplicate per-site scenario JSON or promote the opt-in route | Readiness remains an independent gate before execution. The provider-corridor snapshot covers five sites, not a ten-city run. |
