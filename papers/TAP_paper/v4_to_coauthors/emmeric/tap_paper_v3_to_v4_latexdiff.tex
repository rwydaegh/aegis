...
%DIF DEL v3_to_coauthors/paper.tex   Sat May 30 11:30:41 2026
%DIF ADD v4_to_coauthors/paper.tex   Mon Aug 10 22:58:34 2026
% Paper A+B (merged)
...
%DIF 16c16
%DIF < \usepackage{amsmath,amssymb,amsthm,mathtools}
%DIF -------
\usepackage{amsmath,amssymb,mathtools} %DIF >
%DIF -------
\usepackage{bm}
...
surface yields a generalized Cauchy whole-body identity \DIFdelbegin \DIFdel{with one
geometry }\DIFdelend \DIFaddbegin \DIFadd{governed by a
single geometry-dependent }\DIFaddend scalar. A layered transmission term captures the sub-6~GHz
...
\IEEEPARstart{W}{ireless} exposure \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend the human body is regulated
...
\gls{APD} law for the human body, validated \DIFaddbegin \DIFadd{in }\DIFaddend four independent ways. The
...
\begin{enumerate}
  ...
  absorbed power reduces to a flux-weighted transmission scalar and \DIFdelbegin \DIFdel{an
  }\DIFdelend \DIFaddbegin \DIFadd{a
  single }\DIFaddend ambient-occlusion \DIFdelbegin \DIFdel{geometry }\DIFdelend scalar.
  ...
  under many incident paths, the absorbed-power map is \DIFdelbegin \DIFdel{one }\DIFdelend \DIFaddbegin \DIFadd{a single
  matrix-vector multiplication, evaluated in under }\DIFaddend $10$~ms\DIFdelbegin \DIFdel{matrix-vector multiply}\DIFdelend .
...
$\xi = \ntilde$, giving the \DIFdelbegin \DIFdel{polarization-degenerate }\DIFdelend \DIFaddbegin \DIFadd{polarization-independent normal-incidence }\DIFaddend value
...
vanishes pointwise for \DIFdelbegin \DIFdel{circular }\DIFdelend \DIFaddbegin \DIFadd{circularly polarized }\DIFaddend illumination, in expectation
for \DIFdelbegin \DIFdel{random-orientation linear }\DIFdelend \DIFaddbegin \DIFadd{randomly oriented linearly polarized }\DIFaddend illumination, and to within
$2.5\%$ \DIFdelbegin \DIFdel{for
multipath averaging above~}\DIFdelend \DIFaddbegin \DIFadd{under multipath averaging with at least }\DIFaddend 20 paths. First, for circular polarization
...
approximation error. \DIFdelbegin \DIFdel{Table~I quantifies the deviation
of $\Tavg$ from $T_0$ across
$[0^\circ, 75^\circ]$ }\DIFdelend \DIFaddbegin \DIFadd{$\Tavg/T_0$ is at most $1.056$ across
$[0^\circ,90^\circ]$ }\DIFaddend on skin at 28~GHz. \DIFdelbegin \DIFdel{The maximum deviation is $5.6\%$ at $70$--$75^\circ$. Below $30^\circ$
}\DIFdelend \DIFaddbegin \DIFadd{Below $20^\circ$, }\DIFaddend the
...
\begin{figure}[!t]
  ...
  \DIFdelbeginFL \DIFdelFL{$\Tavg$ stays within $5.6\%$ of $T_0$ up to $75^\circ$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\Tavg/T_0$ is at most $1.056$ over $[0^\circ,90^\circ]$}\DIFaddendFL .
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Fresnel transmission for skin at 28~GHz. Here $T_0 =
\Tavg(0)$ is the normal-incidence value, and $\Tavg/T_0$ stays
within $5.6\%$ of unity over $[0^\circ, 75^\circ]$.}}
%DIFAUXCMD
%DIFDELCMD < \label{tab:fresnel-skin}
%DIFDELCMD < \begin{tabular}{ccccc}
%DIFDELCMD < \toprule
%DIFDELCMD < %%%
\DIFdelFL{$\theta$ }%DIFDELCMD < & %%%
\DIFdelFL{$T_s$ (TE) }%DIFDELCMD < & %%%
\DIFdelFL{$T_p$ (TM) }%DIFDELCMD < & %%%
\DIFdelFL{$\Tavg$ }%DIFDELCMD < & %%%
\DIFdelFL{$\Tavg/T_0$ }%DIFDELCMD < \\
%DIFDELCMD < \midrule
%DIFDELCMD < %%%
\DIFdelFL{$0^\circ$  }%DIFDELCMD < & %%%
\DIFdelFL{0.539 }%DIFDELCMD < & %%%
\DIFdelFL{0.539 }%DIFDELCMD < & %%%
\DIFdelFL{0.539 }%DIFDELCMD < & %%%
\DIFdelFL{1.000 }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{$30^\circ$ }%DIFDELCMD < & %%%
\DIFdelFL{0.489 }%DIFDELCMD < & %%%
\DIFdelFL{0.591 }%DIFDELCMD < & %%%
\DIFdelFL{0.540 }%DIFDELCMD < & %%%
\DIFdelFL{1.002 }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{$45^\circ$ }%DIFDELCMD < & %%%
\DIFdelFL{0.422 }%DIFDELCMD < & %%%
\DIFdelFL{0.666 }%DIFDELCMD < & %%%
\DIFdelFL{0.544 }%DIFDELCMD < & %%%
\DIFdelFL{1.010 }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{$60^\circ$ }%DIFDELCMD < & %%%
\DIFdelFL{0.321 }%DIFDELCMD < & %%%
\DIFdelFL{0.791 }%DIFDELCMD < & %%%
\DIFdelFL{0.556 }%DIFDELCMD < & %%%
\DIFdelFL{1.032 }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{$70^\circ$ }%DIFDELCMD < & %%%
\DIFdelFL{0.233 }%DIFDELCMD < & %%%
\DIFdelFL{0.902 }%DIFDELCMD < & %%%
\DIFdelFL{0.568 }%DIFDELCMD < & %%%
\DIFdelFL{1.054 }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{$75^\circ$ }%DIFDELCMD < & %%%
\DIFdelFL{0.182 }%DIFDELCMD < & %%%
\DIFdelFL{0.952 }%DIFDELCMD < & %%%
\DIFdelFL{0.567 }%DIFDELCMD < & %%%
\DIFdelFL{1.053 }%DIFDELCMD < \\
...
%DIFDELCMD < %%%
\DIFdelend \subsection{Tissue universality}\label{subsec:pB-tissues}
...
\DIFdelbegin %DIFDELCMD < \begin{theorem}\label{thm:cauchy}
%DIFDELCMD < %%%
\DIFdelend Let a body $\Sigma$ have surface area $A$, exposure fraction
...
%DIFDELCMD < %%%
\DIFdel{Apply }\DIFdelend \DIFaddbegin \DIFadd{To see why, apply }\DIFaddend Fubini's theorem to exchange the surface and direction
...
The following bound links the cube quantity to APD. \DIFdelbegin %DIFDELCMD < \begin{theorem}\label{thm:apd-bound}
%DIFDELCMD < %%%
\DIFdelend For an axis-aligned
...
R.~Wydaeghe, B.~Stroobandt, S.~Gallucci, M.~Parazzini, G.~Tognola,
  ...
  \emph{Phys. Med. Biol.}, \DIFaddbegin \DIFadd{accepted for publication, }\DIFaddend 2026,
  \DIFdelbegin \DIFdel{under review}\DIFdelend \DIFaddbegin \DIFadd{doi: }\doi{10.1088/1361-6560/ae97ac}\DIFaddend .
