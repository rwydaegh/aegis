...
RODIN_FREE_TRIAL_KEY_ENV = "BLENDERMCP_HYPER3D_FREE_TRIAL_KEY"
...
def get_rodin_free_trial_key():
    """Return the optional Hyper3D free-trial key from the environment."""
    return os.environ.get(RODIN_FREE_TRIAL_KEY_ENV, "").strip()


def is_rodin_free_trial_key(api_key):
    """Check whether the configured key matches the env-provided free-trial key."""
    free_trial_key = get_rodin_free_trial_key()
    return bool(api_key and free_trial_key and api_key == free_trial_key)
    ...
    def get_hyper3d_status(self):
        ...
        if enabled:
            ...
            message = f"Hyper3D Rodin integration is enabled and ready to use. Mode: {mode}. " + \
                f"Key type: {'free_trial' if is_rodin_free_trial_key(bpy.context.scene.blendermcp_hyper3d_api_key) else 'private'}"
            return {
                "enabled": True,
                "message": message
            }
...
class BLENDERMCP_OT_SetFreeTrialHyper3DAPIKey(bpy.types.Operator):
    ...
    def execute(self, context):
        free_trial_key = get_rodin_free_trial_key()
        if not free_trial_key:
            self.report(
                {'ERROR'},
                f"Set {RODIN_FREE_TRIAL_KEY_ENV} in your environment or repo .env first.",
            )
            return {'CANCELLED'}
        context.scene.blendermcp_hyper3d_api_key = free_trial_key
