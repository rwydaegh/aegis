"""Build a scene config for a screened site, with the ground datum measured.
...
def build(
    ...
    *,
    ...
    source_mesh = paths.geometry_dir(name) / f"{paths.GOOGLE_MESH_STEM}_{crop_m}m.ply"
    source_manifest = paths.mesh_manifest(source_mesh)
    if not source_mesh.exists():
        raise SystemExit(f"no {crop_m} m mesh for {name}")
    try:
        mesh = paths.site_mesh(name, crop_m)
    except FileNotFoundError as error:
        raise SystemExit(f"no {crop_m} m mesh for {name}") from error
    ...
    return {
        "name": name,
        "location": {"lat": row["lat"], "lon": row["lon"], "radius_m": 50.0},
        "enu_origin": {"lat": row["lat"], "lon": row["lon"], "ellipsoid_height_m": 0.0},
        ...
        "source_mesh": str(source_mesh.relative_to(paths.root())),
        "source_mesh_manifest": str(source_manifest.relative_to(paths.root())),
        "geometry_selection": {
