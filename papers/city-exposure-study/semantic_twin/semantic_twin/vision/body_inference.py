...
from semantic_twin.vision.bodies import person_instances, pinhole_intrinsics
...
class BodyInferenceConfig:
    views: pathlib.Path
    semantics_json: pathlib.Path
    out: pathlib.Path
    checkpoint: pathlib.Path
    view_names: tuple[str, ...] | None = None
    mhr_path: pathlib.Path | None = None
    fov_deg: float = 90.0
    min_pixels: int = 600
    min_height_px: int = 48
    max_people_per_view: int | None = None


def _instances_for_view(config: BodyInferenceConfig, name: str, ids: set[int]) -> list[Any]:
    labels = np.load(config.views / f"{name}_labels.npy")
    confidence_path = config.views / f"{name}_confidence.npy"
    ...
    if config.max_people_per_view is not None:
        return instances[: config.max_people_per_view]
    return instances


def infer_bodies(config: BodyInferenceConfig) -> None:
    args = config
    import torch
    ...
    for name in names:
        instances = _instances_for_view(args, name, ids)
        if not instances:
