...
which corresponds to $\alpha^2 \geq \eta_S/17 \approx 0.057$, and
unconstrained ZF holds enough SINR margin above the
$\sim 30\,$dB MCS clip threshold to absorb the projection without
falling below the clip. The dual-ascent solver achieves the same
...
\begin{figure}[!t]
  ...
  ground-truth $\Qmat^{(u)}$, rising monotonically with pose-source
  mismatch: oracle $0.29\,\%$, virtual-IMU $0.53\,\%$, T-pose
  fallback $0.75\,\%$, all well below the $10.2\,\%$ unconstrained-ZF
  baseline. (b) Mean sum-rate is the MCS sum-cap of $74.00\,$Gbps
  in all three pose sources, since the dual-ascent solver redirects
  power per the noise-floor regulariser without dropping below the
  MCS clip. (c) Solver health: the always-on primal projection at
  exit drives min-absorption fallback to $0\,\%$ in all three
  regimes.}
  \label{fig:pose-info}
\end{figure}
