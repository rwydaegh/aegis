% NEXT: % claim: replica_convergence_48_to_64
% NEXT: \begin{table}[!t]
% NEXT:   \caption{Current-contract convergence from 48 to 64 replicas. Bootstrap width is the 95\% interval width for route $q_{10}$ whole-body SAR. Shadow change is the largest stepwise change among the six declared shadowed standpoints.}
% NEXT:   \label{tab:si-convergence}
% NEXT:   \centering
% NEXT:   \begin{tabular}{lrrr}
% NEXT:     \toprule
% NEXT:     Site & \shortstack{$q_{10}$ change\\(dB)} & \shortstack{Bootstrap width\\(dB)} & \shortstack{Shadow change\\(dB)} \\
% NEXT:     \midrule
% NEXT:     Korenmarkt & 0.0000723 & 0.000222 & -- \\
% NEXT:     Prague & 0.0000203 & 0.000218 & -- \\
% NEXT:     Madrid & 0.00000615 & 0.000378 & -- \\
% NEXT:     Mexico City & 0.00344 & 0.364 & 0.0125 \\
% NEXT:     Tokyo Hachiko & 0.00491 & 0.0538 & 0.0104 \\
% NEXT:     \bottomrule
% NEXT:   \end{tabular}
% NEXT: \end{table}
\section{Replica convergence and lower tails}
\label{sec:si-convergence}

## Reviews

_(empty)_
