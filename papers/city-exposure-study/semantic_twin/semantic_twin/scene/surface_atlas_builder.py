...
def build(
    ...
    *,
    ...
    site_mesh_fn = site_mesh if site_mesh_fn is None else site_mesh_fn
    stations_fn = stations if stations_fn is None else stations_fn
    sha256_file_fn = sha256_file if sha256_file_fn is None else sha256_file_fn

    mesh_path = site_mesh_fn(site, options.crop_m)
