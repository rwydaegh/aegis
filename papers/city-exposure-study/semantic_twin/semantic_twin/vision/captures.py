"""The set of panoramas a site was photographed from, and what makes two of them independent.

This module used to be called ``walk.py``, and the package now has a
:mod:`semantic_twin.walk` that means something else. Both names were fair. This
one is the *capture* walk, the drive or ride a survey camera made past the
square, and it is about evidence. :mod:`semantic_twin.walk` is the *pedestrian*
walk, where a body stands while it is exposed. Nothing here places a head and
nothing there fetches an image.
...
only officially linked structure the provider exposes. Assembling a capture set
means picking any image near the site, asking which sequence it belongs to, and
enumerating that sequence in full.
...
a capture set from the box alone would have thrown away eleven of them without
saying so. The transport that does the enumerating lives in
:mod:`semantic_twin.acquire.mapillary`, which is the one client for that API.
...
class WalkStation:
    """One panorama of a capture set, placed in the scene's local ENU frame."""
...
def select_walk(
    ...
    *,
    ...
    """Choose the capture set: nearest first, thinned by separation, capped per capture.
    ...
    fill the whole set and leave the fusion with no independent capture to
...
def spread(stations: Sequence[WalkStation]) -> dict[str, Any]:
    """What the capture set actually covers: extent, baselines, dates, sequences."""
    if not stations:
        return {"n_stations": 0}
    points = np.array([station.position_xy for station in stations], dtype=float)
...
def parallax_independence(
    ...
    *,
    ...
    Measured at Korenmarkt, where twelve panoramas raise directly observed facade
    area from 6.9 to 24.1 percent of the crop. The union supplies 3.43 raw looks
    per face and only 1.92 independent ones, so 44 percent of the apparent
    multi-view evidence is redundant. From the independence block of
    ``outputs/walk_korenmarkt/walk_coverage.json``, at a 10 degree decorrelation
    angle over 25,054 sampled union faces.
    """
