...
A production pose is admitted when its diagnostics are complete, its residual
is at most $4.0^\circ$, and the vertical optimum lies inside the altitude search
interval. It is rejected as inside the geometry only when the sky conflict is
greater than 0.5 \emph{and} the median conflict range is less than 2.0\,m.
Thus a conflict greater than 0.5 at a range of at least 2.0\,m is recorded as a
large sky--mesh mismatch, not as a camera inside the geometry.
...
with a median dive of 2.97\,m against a 3.0\,m bound. In that original cohort,
the 41 poses admitted by the residual and paired sky screen have a median dive
of 0.58\,m and only one in eight goes past 1.5\,m.
...
and 33.9\,m. Zero usable stations become three under the residual and paired
sky screen, while version 2 retains two. The two changes do different work:
the wider crop moves the residual, from a median of $6.19^\circ$ to
...
re-registrations take the count of poses inside the geometry from 26 to 14. The
residual and paired sky rule admit 51 poses. Requiring an interior altitude
optimum, the version-2 production rule, admits 39. Six of the 14 inside poses
still pass the residual gate, so the disagreement between the tests survives
the fix.

Korenmarkt was then audited without changing its sealed version-1 atlas or
route. The five fits at the old lower altitude bound were repeated over
$-2.0$ to $+3.0$\,m. Three still reached the new lower bound. The two interior
fits placed the optical centre only 0.618 and 0.765\,m above measured pavement,
so both remain on hold as low-height warnings.
\Cref{tab:si-korenmarkt-widened} records the widened results.

\begin{table*}[!t]
  \caption{Korenmarkt widened-bound audit. Height is optical-centre height above
  measured pavement. An interior result lies at least 0.1\,m from either bound}
  \label{tab:si-korenmarkt-widened}
  \centering
  \begin{tabular}{lrrrrrl}
    \toprule
    Station & $d_z$ (m) & Residual (deg) & Height (m) & Sky hit & Range (m) & Result \\
    \midrule
    Korenmarkt & $-1.6026$ & 1.1924 & 0.6184 & 0.0601 & 19.3857 & interior, held \\
    706535575184668  & $-1.7354$ & 2.2307 & 0.7646 & 0.0328 & 20.5671 & interior, held \\
    1419513849204492 & $-1.9931$ & 2.8098 & 0.5069 & 0.0497 & 16.8922 & lower boundary \\
    1019442960256615 & $-1.9187$ & 2.0419 & 0.5813 & 0.0452 & 18.1720 & lower boundary \\
    3367014310197013 & $-1.9040$ & 2.2541 & 0.5960 & 0.0794 & 18.5355 & lower boundary \\
    \bottomrule
  \end{tabular}
\end{table*}

Applying only the widened-bound result would reduce the frozen five-camera
route to three cameras and its 13 standpoints to eight, with five interpolated
points. Holding the two low cameras leaves no unique continuation of that
route. The nine-camera atlas covers 3.2497\% of mesh area. The six-camera
boundary-gated counterfactual covers 2.7097\%, and the four-camera promotion
hold covers 2.5577\%. The corresponding route unions cover 2.5791\% for the
sealed five-camera route, 2.1801\% for the three-camera counterfactual, and
2.2596\% for the current two-camera fragment after the hold. This is a no-go
for promotion. The sealed version-1 atlas, route and results remain unchanged.
...
stations admitted by that sealed version-1 binding, ordered by their skyline
residual.
...
\begin{table}[!t]
  \caption{First-interaction coverage at the eight Korenmarkt stations admitted
  by the sealed version-1 binding, 130\,m crop, ordered by skyline residual}
...
acquired across the eleven squares and all 83 registered. The current
version-2 production gate admits 39 poses. The 51-pose count above is the
historical residual-and-paired-sky cohort before the interior-altitude rule.
...
\begin{table*}[!t]
  ...
  ensemble covariance. Version-2 admission requires complete diagnostics, a
  residual of at most $4.0^\circ$, an interior altitude optimum, and rejects the
  paired condition sky conflict greater than 0.5 \emph{and} conflict range less
  than 2.0\,m}
  ...
  \begin{tabular}{lcccccccc}
    ...
    Ghent, Korenmarkt            & 13 & 13 & 3.06 & 8.10  & 0.18 & 7 & 0 & 4 \\
    Krakow, Rynek Glowny         & 0  & 0  & --   & --    & --   & 0 & 0 & 0 \\
    London, Trafalgar Square     & 0  & 0  & --   & --    & --   & 0 & 0 & 0 \\
    Madrid, Plaza Mayor          & 10 & 10 & 0.40 & 2.92  & 0.26 & 5 & 4 & 4 \\
    Mexico City, Zocalo          & 14 & 14 & 2.76 & 4.60  & 1.86 & 4 & 0 & 9 \\
    Milan, Piazza del Duomo      & 1  & 1  & 0.75 & 0.75  & 0.26 & 1 & 0 & 0 \\
    New York, Times Square       & 14 & 14 & 9.13 & 11.25 & 1.44 & 5 & 3 & 0 \\
    Prague, Old Town Square      & 14 & 14 & 0.82 & 9.33  & 0.25 & 2 & 1 & 12 \\
    Tokyo, Hachiko crossing      & 3  & 3  & 3.08 & 3.70  & 0.48 & 1 & 0 & 2 \\
    Toulouse, Place du Capitole  & 0  & 0  & --   & --    & --   & 0 & 0 & 0 \\
    \midrule
    Total                        & 83 & 83 &      &       &      & 31 & 14 & 39 \\
    \bottomrule
  \end{tabular}
\end{table*}
...
The stored station bindings in this section predate version 2 and remain sealed
version-1 artifacts. In particular, the Korenmarkt probes use their recorded
eight- or nine-camera variants, and the Zocalo binding uses 12 stations. A
version-2 rebuild would admit four Korenmarkt poses and nine Zocalo poses. These
coverage values describe the stored bindings, not a version-2 counterfactual.

From each station admitted by its binding an equirectangular ray grid of 1536 rows is cast
...
the same six version-1 stations bind 18.04\% of the triangle area at 384 rows and
...
on the sealed version-1 nine-station variant, where a uniform 1024 rebuild agrees with the stored
...
quality signal. Bound area rises with the number of stations admitted by the
stored binding and falls
...
reaches 8.1\% from three stations for the same reason in reverse. Those cameras
at every square with a station set sit within 29 to 62\,m of their own
...
% The illumination law is being replaced. Sites move onto facade tips read off
% the geometry, so the height bands and the range bands go, and with them the
% rooftop and street small cell models as defined.
% In this section: every Monte Carlo floor quoted per illumination model is a
% floor on an integral against the old weights, so the rooftop and street
% columns need recomputing. The isotropic column stands. The pairing argument,
% the seed assignment scheme and the standpoint sampling result are about the
% experimental design rather than about Q, and they stand.
% Numbers unchanged on purpose. The replacement's own numbers are provisional.
% ======================================================================
...
Korenmarkt standpoints of the geometric rung were retraced over 32 disjoint seed
...
\begin{table}[t]
  \centering
  \caption{Monte Carlo standard deviation from 32 disjoint seed streams over 120
  Korenmarkt standpoints at 200\,000 rays. All entries in dB. An earlier
  eight-stream measurement gave walk medians of 0.0042, 0.0136, and 0.0343\,dB.
  The rooftop figure survives the larger sample and the other two do not.}
  \label{tab:si-noise}
  \begin{tabular}{lrrrr}
    \toprule
    ...
    \midrule
    Isotropic          & 0.0042 & 0.0059 & 0.0125 & 0.0028 \\
    Rooftop macro      & 0.0263 & 0.0381 & 0.0583 & 0.0162 \\
    Street small cell  & 0.1302 & 0.2170 & 0.5791 & 0.0594 \\
    \bottomrule
  \end{tabular}
\end{table}

The walk median column should be read as indicative rather than as a tolerance.
A median over 120 standpoints does not average the per-standpoint noise down as
the square root of 120, because with the standpoints well separated the median
sits close to whichever standpoint is in the middle and moves in steps as ranks
swap. Its sampling distribution is therefore not normal, and the eight-stream
isotropic interval of 0.0010 to 0.0029\,dB contains the 32-stream answer only at
its edge. The column is also specific to the 130\,m crop: measured at the 250\,m
crop over 32 streams the street figure falls to 0.0392\,dB while the rooftop
figure rises to 0.0258\,dB, so neither moves as a change of standpoint count
alone would predict.
...
street-cell shift across the evidence ladder is $0.1505 \pm 0.0092$\,dB over 32
disjoint streams, the uncertainty being the standard error on a mean of 32. That
is 16.4 standard errors from zero on the mean and 2.9 on a single run, and the
0.079\,dB value once quoted as a result is the second lowest of the 32. The
isotropic and rooftop shifts over the same 32 streams are $0.3473 \pm 0.0007$
and $0.3855 \pm 0.0036$\,dB, which sit far enough from zero that the choice of
seed does not reach them.
...
% LAW CHANGE NOTE, see ../docs/LAW_CHANGE.md. The rooftop macro and street
% small cell column pairs of tab:si-sites are stale, as are the spreads read
% off them and the two paragraphs that explain the ordering reversal between
% isotropic and street. The isotropic pair, the sky fraction column, the ground
% datum column and the walk column stand. The datum shifts attributed to Krakow
% and Toulouse stand as a story and need their directional decibels redone.
...
% LAW CHANGE NOTE, see ../docs/LAW_CHANGE.md. The crop radius sweep is per
% illumination model, and the rooftop and street radii set the operating point,
% so the choice of 250 m has to be measured again under the new law. The 60 m
% isotropic radius and the sky fraction column stand. The surface interaction
% sweep stands, because truncation is measured as a fraction of escaping power
% before any weighting, though its per-model decibel columns move.
