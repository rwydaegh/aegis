...
\title{Body Digital Twins for Compliance-Aware mmWave Downlink:
Per-Slot Primal Projection under European Reference Levels}
...
{Wydaeghe: Body digital twins for compliance-aware mmWave downlink}
...
error. We then show that under the 3GPP NR FR2 MCS27 spectral-efficiency
...
$E_{\mathrm{RL}} = 3\,$V/m. Channel-inversion ZF without exposure
shaping violates the cap on $10.2\,\%$ of body-slots; ZF passed
through the per-slot primal projection of the body twin's posed
$\Qmat^{(u)}$ holds violations at $0.0\,\%$ across all detected
bodies at $74.0\,$Gbps, the MCS27 sum-cap, by
feasibility-by-construction. The multi-body MMSE-with-exposure
dual-ascent QCQP of Ying et al.~\cite{Ying2017} extended to
$K + B$ bodies, when its noise-power regulariser is matched to the
receiver floor, attains the same $74.0\,$Gbps MCS sum-cap and the
same $0\,\%$ violation rate on detected bodies on the same
instance. Aggregate violations against the full ground-truth
population sit at $\sim 0.6$-to-$2.4\,\%$ for both algorithms
across two seeds at $K = 25$, with statistical-tie-on-mean and
larger seed-to-seed variance for the dual ascent; both residuals
are concentrated entirely on undetected tier-C bystanders that
absent themselves from the QCQP, a sensing-pipeline floor rather
than a precoder-side property. Projected ZF is the preferred
deployable consumer of the body twin: it is a single scalar
scaling at $\sim 0.5\,$ms per slot vs $\sim 350\,$ms for the dual
ascent, and its feasibility on the detected set is by construction
at every slot rather than only at solver convergence. The dual-ascent solver
is the right tool for the academic uncapped-Shannon objective and
the min-TX-power formulation; we provide it in
...
of 5G NR FR2 is the per-stream MCS cap of $7.4\,$bps/Hz at MCS27,
...
model and the SINR-margin condition that channel-inversion ZF
achieves the cap unconstrained, any $K$-stream precoder satisfying
the SINR-margin condition and the per-body caps attains the
$K$-stream upper bound $K\,S_{\max}$, and the per-slot primal
projection $\ww_k \leftarrow \alpha\,\ww_k$ with
$\alpha = \min(1, \sqrt{\eta_S \min_u L^{(u)}/\Pabs^{(u)}})$ is one
such precoder reachable in $O(B M r)$ per slot. Feasibility is by construction; the
...
$99\,\%$ trace fraction. Channel-inversion ZF without exposure shaping
violates the cap on $10.2\,\%$ of body-slots over the 50-second walk;
projected ZF holds violations at $0.0\,\%$ on detected bodies at the
MCS27 cap of $74.0\,$Gbps, within $0.0\,\%$ of unconstrained-and-non-compliant
ZF. The dual-ascent QCQP, with its noise-power regulariser matched
to the receiver floor and the same projection step at exit, attains
the same MCS sum-cap of $74.0\,$Gbps with $0\,\%$ violations on its
detected-body input set; its residual $0.5\,\%$ to $2.4\,\%$
aggregate violation rate is concentrated on undetected
tier-C bystanders that are excluded from both algorithms' QCQP and
manifest only at evaluation against the ground-truth $\Qmat^{(u)}$
for all bodies. The undetected-tier-C floor is a sensing-pipeline
property that ZF + primal projection inherits identically when its
projection scale is computed from the same detected-body input set
(rather than from oracle full-population $\Qmat$); we report both
variants in \cref{sec:eval-baselines}. We replicate the result on
...
projection meets the MCS sum-cap and matches the dual-ascent solver's
rate-and-violation tuple at $\sim 700\times$ lower wall-clock. The
conditional projected-area bound
...
twin's exposure operator then varies between the IMU-pose
$\Qmat^{(u)}$ (the production reading), the T-pose-fallback
$\Qmat^{(u)}$ for a body without IMU (the canonical SMPL-X T-pose
substituted for the actual pose), and the ground-truth oracle
(an upper bound that no real telemetry pipeline reaches). The
body twin's worst-case operating point is between IMU and T-pose,
not at the oracle. The conditional projected-area Cauchy bound of
\cref{app:cauchy} is a separate object and applies to bodies
without ANY pose information (tier C and tier D).
...
$M = 64$, NumPy + thread-limited LAPACK on a 16-core CPU; JAX vmap
on a single GPU where indicated. Per-slot updates with ``$1\,$ms''
label run at the PHY frame; the precoder solve cadence in the
present implementation is the slot window, $\sim 30\,$ms, not the
PHY frame. The closed-form scaling fits the slot budget on CPU
NumPy alone; the dual ascent needs GPU vmap to fit.}
\label{tab:cadence}
\centering
\small
...
Precoder solve (proj.\ ZF) & $\sim 30\,$ms & median $\sim 0.5\,$ms; one Cholesky + scalar $\alpha$ \\
Precoder solve (dual ascent) & $\sim 30\,$ms & median $\sim 350\,$ms CPU, $\sim 25\,$ms GPU vmap, p95 $\sim 800\,$ms \\
Translation phasor (50 bodies, batched) & $1\,$ms & $\sim 35\,$ms NumPy serial; sub-ms with JAX vmap \\
...
S_{\max}\bigr)$ with $S_{\max} = 7.4\,$bps/Hz, the MCS27 cap of NR
...
beyond the $\sim 30\,$dB threshold at which MCS27 clips reliably.
...
binding regime, it attains the $K$-stream upper bound on the QCQP
\cref{eq:P-MB} sum-rate, alongside any other $K$-stream precoder
that satisfies the SINR-margin condition.

\begin{proposition}[ZF + projection optimality under MCS cap]
\label{prop:projection-optimality}
Fix the served-user count $K$ and the channel matrix
$\HH \in \Complex^{M \times K}$. Consider the QCQP
\cref{eq:P-MB} restricted to $K$-stream precoders
$\WW \in \Complex^{M\times K}$ under the MCS-cap rate model
...
$R_k = S_{\max}$ on every served stream and maximises
$R_{\mathrm{sum}}$ subject to all $K + B + 1$ constraints of
\cref{eq:P-MB} over the $K$-stream feasible set.
\end{proposition}
...
every $k$. Hence $R_{\mathrm{sum}} = K\,S_{\max}$. With $K$ streams
and a per-stream MCS clip at $S_{\max}$, the rate is upper-bounded
by $K\,S_{\max}$ regardless of the precoder choice. Therefore
$\WW = \alpha\,\WW_{\mathrm{ZF}}$ attains this upper bound and is a global
optimum.
\end{proof}

The hypothesis $\alpha^2\,\mathrm{SINR}^{\mathrm{ZF}}_{\min} \geq
2^{S_{\max}} - 1$ has a single physical meaning: channel-inversion
ZF leaves enough SINR headroom above the linear MCS27 clip
threshold $2^{S_{\max}} - 1 = 167.9$ (i.e.\ $22.25\,$dB) to absorb
the worst-body overshoot factor $\rho/\eta_S$ without falling below
the cap. In the plaza-scale binding regime of \cref{sec:eval} we
measure $\rho \in [1, 20]$ at $E_{\mathrm{RL}} = 3\,$V/m
across the four regimes of \cref{tab:baselines}, so the hypothesis
is equivalent to a worst-case
$\mathrm{SINR}^{\mathrm{ZF}}_{\min} \gtrsim
(\rho/\eta_S)\,(2^{S_{\max}} - 1) \approx 20.6 \cdot 167.9
\approx 3460$ in linear units, i.e.\ $34.7\,$dB. Empirically, ZF
saturates the MCS sum-cap of $74.0\,$Gbps on every slot of the
trace and projected ZF retains the same cap at zero violation on
the detected-body input set, which witnesses
$\mathrm{SINR}^{\mathrm{ZF}}_{\min}$ at or above this margin
threshold across the entire $1500$-slot window.
\Cref{prop:projection-optimality} establishes that any $K$-stream
precoder satisfying the SINR-margin condition and the per-body
caps attains $R_{\mathrm{sum}} = K\,S_{\max}$, and that the
closed-form scaled ZF of \cref{eq:primal-projection} is one such
precoder reachable in $O(B M r)$ per slot. The proposition does
not single out projected ZF as the unique global optimum: any
$K$-stream method that saturates the cap and meets the per-body
constraints is co-optimal in this regime, including the
dual-ascent QCQP of \cref{app:dualascent} when its noise-power
regulariser is matched to the receiver floor. The case for
projected ZF over the dual ascent is not algorithmic optimality
but feasibility-by-construction at every slot together with the
$\sim 700\times$ wall-clock advantage at the same rate
(\cref{sec:eval-baselines}). The body-twin pipeline of
\cref{sec:twin} is what makes the $\PabsZF^{(u)}$ in
...
rate model it attains the same MCS sum-cap as projected ZF on the
same instance but at $\sim 700\times$ the wall-clock and without
the per-slot feasibility-by-construction guarantee
(\cref{sec:eval-baselines}).
...
projection is rate-cost-free at MCS27 because the SINR margin
...
the mean illumination view-factor for a single-panel-illuminated
body. This $\bar\eta$ is twice the Cauchy projected-area
factor $1/4$ to leave headroom on the $D_{\max} \leq 2$
peak-to-average directivity bound; the unconditional Cauchy bound
is breached by a $+2.86$-to-$+6.34\,$dB median ratio on shaped
precoders (\cref{sec:eval-cauchy}), but the conditional bound of
\cref{app:cauchy} that holds when $\xx$ lies in the BS-side
beamforming codebook keeps the operational budget defensible.
This gives $L^{(u)}_{\mathrm{RL}} \approx 6.9\,$mW per body. The trace window is
...
surrogate is per-stream $\min(\log_2(1+\mathrm{SINR}_k),\,
S_{\max})$ with $S_{\max} = 7.4\,$bps/Hz from 3GPP TS 38.214
Table 5.1.3.1-2 (256-QAM, $\mathrm{MCS}=27$, coderate
$948/1024$)~\cite{TS38214MCS}. Receiver noise power is
$\sigma_n^2 = 10^{-12}\,$W ($-90\,$dBm at $400\,$MHz, optimistic
$0\,$dB NF) for the headline numbers; we additionally re-ran the
$K = 25$ instance at $\sigma_n^2 = 6.4 \cdot 10^{-12}\,$W
($-82\,$dBm, $\mathrm{NF} = 6\,$dB realistic receiver noise) and
both algorithms still saturate the MCS sum-cap with violation
rates within a factor 2 of the optimistic-floor numbers (ZF + proj
deployable $0.46\,\%$, dual ascent $0.52\,\%$ vs $1.21\,\%$ and
$2.39\,\%$ at the optimistic floor). The dual-ascent solver's
noise-power regulariser is matched to this floor; in the original
Ying et al.~\cite{Ying2017} formulation the regulariser is a free
parameter set to $10^{-2}$ or $10^{-3}$, and we observed
in early v3-era runs that this $7$-$9$ orders of magnitude
mismatch with the SE evaluator drove the dual ascent into
MRT-like degenerate behaviour and produced a spurious
``factor-4 rate gap'' that disappeared once the regulariser was
...
\begin{figure*}[!t]
  ...
  step at exit, also holds the tail at the cap. Right: per-precoder
  Pareto on cap-violation rate vs sum-rate. Both projected ZF and
  the dual ascent saturate the MCS27 sum-cap at $74.0\,$Gbps; both
  drive aggregate violations to $0.0\,\%$ on their detected-body
  input set (\cref{sec:eval-baselines} for per-tier breakdown).}
  \label{fig:hero}
\end{figure*}
...
$600$-slot steady-state trace at three served-user counts
($K = 25, 10, 5$ out of $50$ bodies) and on a bystander-binding
geometry where served users are at $25$-$50\,$m and the $35$
non-served bodies cluster at $5$-$12\,$m from the panel. The
bystander-binding geometry was constructed to test whether the
dual-ascent solver wins when binding bodies are non-served and ZF
cannot null them by construction. To separate algorithmic from
sensing-pipeline effects we report two ZF + projection variants:
\emph{ZF + proj (oracle)} sets the projection scale $\alpha$
against the ground-truth $\Qmat^{(u)}$ for every body in the
population, including undetected tier-C bystanders, and is
feasibility-by-construction at every slot; \emph{ZF + proj
(deployable)} sets $\alpha$ against only the bodies in
$Q_{\mathrm{proposed}}$ that the body twin sees at the slot, the
same input set the dual ascent receives. The deployable variant
re-introduces the sensing-pipeline floor that absent undetected
tier-C bodies impose at evaluation against the full population. In
every regime tested, both ZF + projection and the dual ascent
saturate the MCS sum-cap. On the seed-42 trace reported in
\cref{tab:baselines}, ZF + proj (deployable) edges out the dual
ascent on aggregate compliance with margins of $1.05\times$ to
$19\times$ on violation rate, traceable to the dual ascent's
rank-$r$ truncation of $\Qmat^{(u)}$ and FD-Jacobian noise that
the closed-form scaling does not incur. A multi-seed cross-check
at seed $1729$ on the $K = 25$ regime flips this ordering
(ZF + proj $0.58\,\%$, dual ascent $0.32\,\%$); the two methods
are therefore statistically tied on aggregate compliance with the
dual ascent showing larger seed-to-seed variance ($\sigma_{
\mathrm{viol}} \approx 1.5\,$pp on two-seed estimates) reflecting
the solver's path-dependent behaviour in the rank-truncated QCQP.
Both deployable variants inherit the same undetected-tier-C
floor.

\begin{table}[!t]
\caption{Projected ZF vs.\ dual-ascent QCQP across served-user
counts and the bystander-binding geometry, mean over the $600$-slot
trace at $E_{\mathrm{RL}} = 3\,$V/m, $43\,$dBm. \emph{ZF + proj
(oracle)} sets the projection scale against the full-population
ground-truth $\Qmat^{(u)}$; \emph{ZF + proj (deployable)} sets it
against only the detected-body input set the dual ascent receives.
\emph{Dual ascent (tuned)} is the multi-body MMSE-with-exposure of
\cref{app:dualascent} with its noise-power regulariser matched to
the receiver floor and the projection step at exit. The MCS27 cap
of $7.4\,$bps/Hz at 400 MHz puts $K\,S_{\max}\,$BW at $74.0\,$Gbps
for $K = 25$, $29.6\,$Gbps for $K = 10$, $14.8\,$Gbps for $K = 5$.
The undetected-tier-C floor is identical in both deployable
variants.}
\label{tab:baselines}
\centering
\small
...
\midrule
\multirow{4}{*}{$K = 25$ served, plaza}
 & ZF (illegal) & 74.0 & 14.6\% & 119.7 \\
 & ZF + proj (oracle) & \textbf{74.0} & \textbf{0.0\%} & 6.7 \\
 & ZF + proj (deployable) & 74.0 & 1.2\% & 27.7 \\
 & Dual ascent (tuned) & 74.0 & 2.4\% & 39.2 \\
\midrule
\multirow{4}{*}{$K = 10$ served, plaza}
 & ZF (illegal) & 29.6 & 50.0\% & 135.6 \\
 & ZF + proj (oracle) & \textbf{29.6} & \textbf{0.0\%} & 6.7 \\
 & ZF + proj (deployable) & 29.6 & 0.7\% & 14.3 \\
 & Dual ascent (tuned) & 29.6 & 0.7\% & 58.0 \\
\midrule
\multirow{4}{*}{$K = 5$ served, plaza}
 & ZF (illegal) & 14.8 & 37.2\% & 55.5 \\
 & ZF + proj (oracle) & \textbf{14.8} & \textbf{0.0\%} & 6.7 \\
 & ZF + proj (deployable) & 14.8 & 0.05\% & 8.8 \\
 & Dual ascent (tuned) & 14.8 & 1.0\% & 21.5 \\
\midrule
\multirow{4}{*}{Bystander-binding}
 & ZF (illegal) & 29.6 & 21.7\% & 104.8 \\
 & ZF + proj (oracle) & \textbf{29.6} & \textbf{0.0\%} & 6.7 \\
 & ZF + proj (deployable) & 29.6 & 0.3\% & 10.3 \\
 & Dual ascent (tuned) & 29.6 & 0.5\% & 19.0 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{per_tier_breakdown.pdf}
  \caption{Per-tier cap-violation breakdown for the $K = 25$ regime
  ($600$-slot, plaza-specular paths, $E_{\mathrm{RL}} = 3\,$V/m).
  Tier A and B carry zero violations under both compliance-aware
  algorithms (the closed-form scaling and the dual ascent both
  drive the detected-body cap to zero). Tier C carries the
  sensing-pipeline floor, with the deployable variants of ZF + proj
  and the dual ascent at $6.0\,\%$ and $11.9\,\%$ respectively.
  The gap reflects the difference between the closed-form scaling's
  uniform amplitude cut, which reduces absorbed power on undetected
  bodies as a side-effect, and the dual ascent's per-stream
  shaping, which has no analogous guarantee on bodies absent from
  the QCQP.}
  \label{fig:per-tier}
\end{figure}

Both algorithms saturate the MCS sum-cap in every regime tested.
The mechanism is the SINR-margin hypothesis of
\cref{prop:projection-optimality}: the worst-body absorbed-power
overshoot factor is bounded at $\rho \leq 20$ in the plaza traces,
which corresponds to $\alpha^2 \geq \eta_S/20 \approx 0.05$, and
unconstrained ZF holds enough SINR margin above the
$\sim 22.3\,$dB MCS clip threshold (linear $2^{S_{\max}}-1
\approx 168$) to absorb the projection without falling below the
clip. The dual-ascent solver achieves the same
clip-saturation when its noise-power regulariser is matched to the
receiver floor; with the over-regularised noise of
$\nu = 10^{-3}$ used in the original Ying et al.~formulation it
falls into the unsaturated logarithmic regime, which we
diagnosed and corrected in the v7 production reported in this
table.

The compliance gap requires careful framing. \emph{ZF + proj
(oracle)} in \cref{tab:baselines} projects against the ground-truth
$\Qmat^{(u)}$ for every body in the population, including
undetected tier-C bystanders, and achieves $0\,\%$ violation by
construction. The dual-ascent solver and \emph{ZF + proj
(deployable)} both operate on the same detected-body twin (tier A
and B with their pose estimates, tier-C bodies whose ISAC return
crosses the detector at the slot, plus their T-pose envelope), so
their $0.05\,\%$ to $2.4\,\%$ aggregate violation rates are
dominated by undetected tier-C bystanders that absent themselves
from the QCQP and only re-enter at evaluation against the full
ground-truth $\Qmat^{(u)}$. A per-tier breakdown of the $K = 25$
regime row makes the mechanism explicit: tier A and tier B carry
$0.00\,\%$ violation under both deployable algorithms, while
tier-C carries $6.03\,\%$ under ZF + proj (deployable) and
$11.93\,\%$ under the dual ascent. The mechanism behind the
deployable variant's smaller residual is that the uniform-scaling
projection cuts the precoder amplitude by the same $\alpha < 1$
on every direction, reducing absorbed power on undetected bystanders
as a free side-effect of constraint enforcement on detected bodies;
the dual ascent's per-stream MMSE-with-exposure shaping is more
aggressive on the detected constraint set but offers no analogous
guarantee on undetected bodies. The structural separation between
the two algorithms is therefore a wall-clock gap (\cref{tab:cadence})
plus a small algorithmic edge for the closed-form scaling on
undetected-body residuals, on top of an identical
feasibility-by-construction guarantee on the detected input set.
The dual-ascent solver's rank-$r$ truncation at $r = 6$
(conservative above the empirical rank-4 envelope of
\cref{sec:rank-eval}) and its FD-Jacobian noise on the order of
$10^{-5}$ account for the residual sub-percent gap on top of this;
we document the solver in \cref{app:dualascent}.
...
\Cref{tab:pose-triage} reports the binding-regime metrics across
the three pose sources at $\sigma_{\mathrm{joint}} = 4^\circ$. All
three saturate the MCS sum-cap of $74.00\,$Gbps; what differs is
the residual violation rate against the ground-truth full-body
$\Qmat^{(u)}$. \Cref{fig:pose-info} shows both compliance-aware
methods across the pose sources: the dual ascent's residual
ranges $0.29\,\%$ (oracle) $\to 0.53\,\%$ (IMU $4^\circ$)
$\to 0.75\,\%$ (T-pose), while ZF + proj (deployable) ranges
$0.45\,\%$ $\to 0.55\,\%$ $\to 0.20\,\%$. The T-pose row reverses
the ordering: uniform amplitude scaling's conservative behaviour
on the canonical-pose Q absorbs the T-pose mismatch better than
the dual ascent's per-stream shaping. All values are well below
the $10.2\,\%$ violation rate of channel-inversion ZF.

The dominant mechanism of the residual violation is the
sensing-pipeline floor: tier-C bystanders that ISAC fails to
detect on a slot are absent from $Q_{\mathrm{proposed}}$ and
re-enter at evaluation against the full ground-truth set. A
per-tier breakdown of the dual-ascent run at $K = 25$ in
\cref{fig:per-tier} shows
the cumulative tier-A and tier-B violation at $0.00\,\%$, with
all of the $\sim 0.5\,\%$ aggregate violation rate localised to
the undetected-tier-C subset. The pose-source progression
oracle/IMU/T-pose modulates this floor at a sub-percent
incremental scale: T-pose adds a second source of mismatch where
the canonical pose has the arms extended laterally while the
walking population has the arms inward, so the T-pose
$\Qmat^{(u)}$ misrepresents the absorption-mode geometry in some
BS-precoder directions. Both sources are absent from
\emph{ZF + proj (oracle)} of \cref{tab:baselines}, which projects
against the full-rank pose-aware $\Qmat^{(u)}$ for every body in
the population (including undetected tier-C) and is
feasibility-by-construction at every slot regardless of pose
source; the deployable variant inherits the same sensing-pipeline
floor as the dual ascent.
...
\midrule
Oracle (true pose) & $74.00\,$Gbps & $0.29\,\%$ & $0.0\,\%$ \\
Virtual IMU ($4^\circ$) & $74.00\,$Gbps & $0.53\,\%$ & $0.0\,\%$ \\
T-pose fallback & $74.00\,$Gbps & $0.75\,\%$ & $0.0\,\%$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[!t]
  ...
  plaza-specular paths, ZF + proj (deployable, green) and
  dual-ascent ECBF (red) compared across three pose sources.
  (a) Cap-violation rate against the ground-truth $\Qmat^{(u)}$:
  at oracle pose ECBF edges out projected ZF
  ($0.29\,\%$ vs $0.45\,\%$); at virtual IMU $4^\circ$ the two
  methods are essentially tied ($0.53\,\%$ vs $0.55\,\%$); at
  T-pose fallback projected ZF wins by $\sim 4 \times$
  ($0.20\,\%$ vs $0.75\,\%$) because the uniform amplitude
  scaling is more conservative against T-pose mismatch than the
  dual ascent's per-stream shaping. All values are well below the
  $10.2\,\%$ unconstrained-ZF baseline. (b) Mean sum-rate at the
  MCS27 sum-cap of $74.0\,$Gbps in all three pose sources because
  the SINR-margin condition holds throughout.}
  \label{fig:pose-info}
\end{figure}

\paragraph*{Sensitivity to the IMU calibration}
\Cref{fig:imu-sweep} measures the per-joint attitude RMS error
$\sigma_{\mathrm{joint}}$ at five points from $0^\circ$ (oracle) to
$16^\circ$ (poorly calibrated phone IMU) at fixed plaza load. The
dual-ascent ECBF mean cap-violation rate trends monotonically
upward with $\sigma_{\mathrm{joint}}$ from $0\,\%$ to $0.93\,\%$
with sub-percent slot-to-slot fluctuation between adjacent grid
points (single-seed). The closed-form scaling overlaid on the same
sweep tracks within $0.1\,$pp of the dual ascent at every point.
Sum-rate sits at the MCS27 sum-cap of $74.0\,$Gbps across the full
sweep, since the SINR-margin condition holds throughout. The
operationally-honest target sits at $\sigma_{\mathrm{joint}} \in
[1.5^\circ, 8^\circ]$, the consumer-IMU band: in this band the
violation rate stays below the T-pose-fallback asymptote without
requiring instrumented-mocap-grade hardware.

\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{imu_sweep.pdf}
  \caption{Sensitivity to the virtual-IMU per-joint attitude RMS
  error $\sigma_{\mathrm{joint}}$ at five measured points (v7
  noise-matched runs only). Red solid: dual-ascent ECBF
  cap-violation rate against the ground-truth $\Qmat^{(u)}$,
  trending monotonically upward with $\sigma_{\mathrm{joint}}$.
  Green dashed: ZF + proj (deployable) cap-violation rate on the
  subset of points where the closed-form scaling was logged
  (within $0.1\,$pp of the dual ascent at every measured point).
  Green dotted: mean sum-rate, flat at the MCS27 sum-cap of
  $74.0\,$Gbps across the sweep. The T-pose-fallback asymptote
  sits at $0.75\,\%$ for reference.}
  \label{fig:imu-sweep}
\end{figure}
...
past the chronic budget (median fraction $2.17$); channel-inversion
ZF violates the budget on the slot-binding bodies but keeps the
median at $0.40$ across the population; WC back-off keeps everyone
under at $0.31$; projected ZF lands at median $0.19$ and the
dual-ascent multi-body MMSE-with-exposure at median $0.12$. The
honest reading is that chronic dose is dominated by the
slot-distribution of binding mass, not by per-slot algorithm
choice: every per-slot rate-aware precoder that is not MRT keeps
the median chronic fraction comfortably under unity, with all
compliance-aware variants (WC, projected ZF, dual ascent) clearing
the budget for every body in the trace. The body-twin advantage on
this metric is real but quantitative ($\sim\!37\%$ tighter median
than projected ZF, $\sim\!2.6\times$ tighter than WC), not a
solve-vs-fail distinction. The ECDF scales linearly with window
length under stationary load, so the medians transfer to the
$6\,$min and $24\,$h regulatory windows unchanged.
...
\begin{figure}[!t]
  ...
  most bodies past the chronic budget. WC back-off, projected ZF,
  and the dual-ascent multi-body MMSE-with-exposure all clear the
  chronic budget for every body in the trace; raw ZF clears the
  budget at the median but leaves the slot-binding tail above
  unity. Among compliance-aware precoders the gap is quantitative
  (projected ZF $0.19$, ECBF $0.12$), reflecting the slot-binding
  fraction of dose, not a solve-vs-fail distinction.}
  \label{fig:chronic}
\end{figure}
...
machinery. Combined with \cref{sec:chronic}, the deployment value
is the marginal chronic-dose headroom at occupied points; chronic
compliance itself is delivered by any rate-aware compliance-aware
precoder at this regime, and the body twin contributes the
spectral-efficiency gain documented in \cref{tab:baselines}.
...
precoder choice is constrained on it. Channel-inversion ZF
without exposure shaping gives the highest unconstrained sum-rate
but at $10.2\,\%$ violation rate. ZF with primal projection
attains the MCS sum-cap of $74.0\,$Gbps and zero violation on its
input set; the dual-ascent multi-body MMSE-with-exposure, with
its noise-power regulariser matched to the receiver noise floor
(a deployable choice that the original Ying et
al.~formulation~\cite{Ying2017} silently makes), tracks the same
MCS sum-cap and the same zero compliance gap on the same
detected-body input set. The $\sim 0.5$-to-$2.4\,\%$ aggregate
violation rate that the dual ascent shows in \cref{tab:baselines}
is dominated by undetected tier-C bystanders that absent
themselves from the QCQP at the slot and re-enter at evaluation
against the full ground-truth $\Qmat^{(u)}$; this floor is a
sensing-pipeline property that ZF + primal projection with the
same input set inherits identically.
The post-2014 Brussels $14.57\,$V/m cumulative cap, the Italian
$15\,$V/m attention level for high-density urban centres, and the
$30\,$dBm conducted-power band sit in slack regimes where any
reasonable precoder satisfies the cap by orders of magnitude.

The preference for projected ZF over the dual-ascent QCQP solver
under the deployable rate model is structural and operational, not
algorithmic. The per-stream MCS27 cap of $7.4\,$bps/Hz clips the
rate model at the $22.25\,$dB linear SINR threshold. Both
algorithms saturate this cap in the binding regime and both drive
their detected-body violations to $0\,\%$. What differs is
deployability. Projected ZF is feasibility-by-construction at
every slot: the projection scale $\alpha$ in
\cref{eq:primal-projection} solves the constraint
$\Pabs^{(u)} \leq \eta_S\,L^{(u)}$ for every body in its input set
in closed form. The dual ascent only meets the constraints at
solver convergence, with a typical $\sim 350\,$ms wall-clock per
slot vs the $\sim 0.5\,$ms of the closed-form scaling; outside
convergence (e.g.\ when the FD-Jacobian goes near-singular) the
exit projection rescues feasibility on the worst-margin body but
not at the rate-saturation point. The dual-ascent solver retains
its place in the academic uncapped-Shannon objective, the
min-TX-power objective, and the SINR-margin-tight regime
(low-element panels, dense served-user counts at $K \approx M$)
where uniform scaling can drop the served-user SINR below the
clip; we identify this regime in \cref{sec:eval-baselines} as the
boundary where projected ZF and the dual ascent exchange roles.

The pose-information triage modulates compliance at sub-percent
scale across all three pose sources: oracle, virtual IMU at
$4^\circ$ per-joint RMS, and T-pose fallback. The aggregate
violation-rate spread (oracle $0.29\,\%$ to T-pose
$0.75\,\%$ on the dual ascent) is consistent with the
sensing-pipeline floor analysis above: pose only modulates which
detected-body $\Qmat^{(u)}$ the algorithm sees, while the
dominant residual driver is whether tier-C is detected at all.
The IMU-vs-T-pose gap is the one degree of freedom the body twin
buys for the dual ascent; for the closed-form scaling the
ground-truth-projected oracle variant gives the tighter upper
bound on what perfect detection would buy.
...
The present evaluation has six limitations. First, the PHY
...
single seed at this revision; multi-seed averaging would smooth
the chronic-dose ECDF tails and shrink the small ($\sim 1\,$pp)
slot-to-slot fluctuations on the violation curves. Third, the
bystander-binding geometry was constructed by hand; a
stadium-scale auditorium-and-stage geometry would test the
dual-ascent solver under a less favourable SINR-margin regime.
Fourth, the IMU model is a calibrated Madgwick/xsens proxy; a
deep-learning-augmented inertial-mocap pipeline would lower
$\sigma_{\mathrm{joint}}$ to the $1.5^\circ$ floor of
\cref{fig:imu-sweep}. Fifth, the head-to-head between projected
ZF and the dual ascent uses oracle channels modulo the ridge LS
calibration of \cref{eq:csi-LS}; a deployment with tight
$\sigma_{\mathrm{CSI}}$ on the served-user channel would degrade
ZF's interference structure first and could narrow the wall-clock
gap because the projection's SINR margin shrinks. Sixth, the
served-user count $K = 25$ on $M = 64$ leaves ample DoF for ZF;
the SINR-margin condition of \cref{prop:projection-optimality}
loses headroom as $K \to M$ and projected ZF can drop below the
MCS clip, the regime where the dual ascent earns its keep
algorithmically rather than only operationally. We ran a
$K = 50$ stress test on the same $M = 64$ panel (no
non-served bodies; pure served-user binding), which still
saturates the MCS sum-cap at $K \cdot S_{\max} \cdot \mathrm{BW}
= 148\,$Gbps with zero violations under both projected ZF and
the dual ascent; the SINR-margin condition therefore holds at
$K/M = 0.78$ in this geometry. A full $K \approx M$ regime that
includes near-panel bystanders is left as future work. None of these
limitations affect the qualitative conclusion that the body-twin
pipeline makes the multi-body QCQP closed-form-solvable in the
binding regime.
...
saturates the $K + B$ per-body caps and the per-stream MCS27 cap
...
substantial SINR at the served users, well above the MCS27 clip
...
Ying et al.~\cite{Ying2017} to $K + B$ bodies, attains the same
MCS sum-cap on the same detected-body input set, but at $\sim
700\times$ the wall-clock and only at solver convergence rather
than by closed form; it earns its keep in the academic
uncapped-Shannon objective, the min-TX-power formulation, and the
SINR-margin-tight regime $K \approx M$ where uniform scaling can
drop the served-user SINR below the clip.
...
$10.2\,\%$ of body-slots over a 50-second steady-state walk; ZF
with primal projection holds violations at $0.0\,\%$ on its
detected-body input set at the MCS27 sum-cap of $74.0\,$Gbps. The
dual-ascent QCQP solver, when its noise-power regulariser is
matched to the receiver floor, attains the same MCS sum-cap with
the same $0\,\%$ violations on the detected set; the
$\sim 0.5$-to-$2.4\,\%$ aggregate violation rate that the
solver shows at evaluation against the full-population ground
truth is concentrated on undetected tier-C bystanders, which is a
sensing-pipeline floor that ZF + projection on the same input set
inherits identically. The same conclusion holds at $K = 10$ and
...
$5$-$12\,$m of the panel: projected ZF saturates the MCS sum-cap
with by-construction feasibility on its input set in every case;
the dual-ascent solver matches the rate when properly tuned at
$\sim 700\times$ the wall-clock. The naive projected-area bound on
...
At the binding regime ($M = 64$, $K = 25$, $B = 50$), the solver
runs at low-rank truncation $r = 6$, conservatively above the
empirical rank-4 envelope of \cref{sec:rank-eval} at the $99\,\%$
trace fraction so the solver does not lose secondary modes. Each
Newton step costs $(B' + 1) M^2 r$ and is
single-precision-complex-bound. On a 16-core CPU with NumPy LAPACK
the median solve sits at $\sim 350\,$ms with
$\textsf{max\_outer} = 24$. JAX vmap over the FD fanout reduces
this $\sim 14\times$ on a single GPU. Under the MCS-cap rate model
the dual ascent matches projected ZF on rate and on detected-body
violation rate but at $\sim 700\times$ the wall-clock and without
the per-slot feasibility-by-construction guarantee
(\cref{sec:eval-baselines}); we include the dual ascent for the
uncapped-Shannon and min-power regimes of
\cref{sec:dualascent-ref}.
