...
\nu\,\mathbf{I})^{-1} \gvec_k$, in which the rank-one degenerate limit
recovers regularised zero-forcing. The loop runs on the Brussels Grand Place geometry with
...
$\ww_k^\star = (\Qmat_{\mathrm{tot}}(\bm\lambda) + \Hintf +
\nu \mathbf{I})^{-1} \gvec_k$ with KKT bisection on the dual variables,
a generalisation of
...
\end{equation}
also Hermitian positive-semidefinite. The WMMSE inner problem
contributes a $k$-independent inter-user interference Gram
\begin{equation}\label{eq:Hgram}
  \Hintf =
  \sum_{k'} u_{k'}\,|v_{k'}|^2\,\hh_{k'}\hh_{k'}^H,
\end{equation}
together with the per-stream drive $\gvec_k = u_k v_k\,\hh_k$, where
$u_k$ and $v_k$ are the WMMSE auxiliary weight and receive filter of
Shi-2011~\cite{Shi2011}, reproduced in \cref{app:ecbf}. The inner
stationarity gives
...
special case. With $\bm\lambda = 0$ the exposure piece vanishes and
$\ww_k^\star = (\Hintf + \nu\,\mathbf{I})^{-1} \gvec_k$ is the
unconstrained WMMSE precoder, which converges under the outer
fixed point on $(u_k, v_k)$ to MRT for $K = 1$ and to regularised
...
vector, aligned with the served-user channel $\hh_u \propto
\mathbf{a}_u$ for $u \in \Served$. Stack the steering vectors as the
rows of $\mathbf{A} \in \Complex^{(K+B)\times M}$, the combined
diagonal as $\mathbf{D}_{uu} = \lambda_u \sigma_u + u_u |v_u|^2\,
\mathbb{1}[u \in \Served]$, and the regularised inverse as
$(\mathbf{A}^H \mathbf{D} \mathbf{A} + \nu \mathbf{I})^{-1}$. Then
...
The proof is direct substitution in \cref{eq:Qtot,eq:Hgram}. The
rank-one
...
different: the plaza-wide median sits at $0.06 L_{\mathrm{RL}}$, the
$95^{\mathrm{th}}$ percentile at $0.20 L_{\mathrm{RL}}$, and the
brightest cells reach $0.40 L_{\mathrm{RL}}$. The cell averaging hides
the per-body-slot worst case: while only $0.10\%$ of body-slots
violate the budget, the worst tail reaches $3.4 L_{\mathrm{RL}}$
where a body walks through a high-gain ridge between two served-user
beams. ZF's interference-suppressing nulls are geometric, not
exposure-aware, and the spatial pattern depends on user geometry
rather than body absorption. The operational argument for
budget-aware precoding holds even when the cell-averaged compliance
metric looks acceptable: the rare per-body overshoots are exactly
where the closed-loop QCQP buys margin, and the geometric ZF
solution does not see them.

\paragraph*{Panel-size sensitivity}\label{par:panel-size-spatial}
Repeating the binding-regime trace at $M = 256$ (16$\times$16 UPA, half-wavelength spacing, same $43\,$dBm conducted transmit power and $6\,$V/m reference level) sharpens this point. The MRT violation rate climbs from $14.9\%$ to $39.5\%$ and the ZF violation rate from $0.10\%$ to $18.7\%$, even with $231$ nulling DOFs over the $K = 25$ served users. A larger array focuses energy more tightly on each served user, and the side-lobe geometry that kept the cell-averaged $0.06 L_{\mathrm{RL}}$ at $M = 64$ no longer keeps the per-body tail bounded: the ZF $99^{\mathrm{th}}$ percentile $\Pabs/L_{\mathrm{RL}}$ rises from $0.53$ at $M = 64$ to $3.73$ at $M = 256$, and the maximum from $3.4 L_{\mathrm{RL}}$ to higher values. The proposed multi-body ECBF holds the per-body $99^{\mathrm{th}}$ percentile at $1.00\,L_{\mathrm{RL}}$ at both panel sizes by construction. The argument for budget-aware precoding strengthens with array size: the geometric ZF solution is increasingly non-compliant as $M$ grows, while the closed-loop QCQP holds the cap regardless. The $M = 256$ runs use the GPU-vmapped JAX inner solver of \cref{sec:multibody-ecbf}; the per-slot wall-clock of the Newton sweep at this size is $\sim 1.1\,$s on a single RTX 4090 with the body-batch finite-difference Jacobian fanned out under \texttt{jax.vmap}, against $\sim 4\,$s on a 16-vCPU host that has the contraction memory-bandwidth-bound on the $\Mmat^{(u)}$ tensor.
...
\begin{figure*}[!t]
  ...
  heterogeneity. Right: binding regime ($43\,$dBm conducted transmit
  power, $E_{\mathrm{RL}} = 6\,$V/m post-2014 Brussels arr\^et\'e /
  Italy outdoor attention level), ZF. Plaza-wide cell-mean median
  $0.06 L_{\mathrm{RL}}$, hot cells reach $0.40 L_{\mathrm{RL}}$;
  per-body-slot tail extends to $3.4 L_{\mathrm{RL}}$. ZF nulling is
  geometric, not exposure-aware.}
  \label{fig:spatial}
\end{figure*}
...
\Cref{fig:pose-info} ablates this telemetry in the binding regime
($43\,$dBm conducted transmit power, $E_{\mathrm{RL}} = 6\,$V/m
post-2014 Brussels arr\^et\'e / Italy outdoor attention level): the
same $5$-min trace is re-executed with the tier-A and tier-B exposure
operators
swapped for the Cauchy worst-case envelope of \cref{sec:tiers}, which
removes the IMU information advantage by treating every served and
cooperating body as if it were a tier-C bystander. The slack regime
of \cref{tab:slack} produced a null result on this ablation: with the
budget margin at four orders of magnitude, the dual variables
$\bm\lambda$ in \cref{eq:closedform} sit at zero and the operator
value drops out of the precoder. The binding regime activates the
constraint and pose information becomes a real lever.

The result is more nuanced than a one-sided gain. The closed-form
precoders MRT and worst-case back-off do not consume $\Mmat^{(u)}$
and therefore exhibit identically zero $\Delta$\textsf{SR}, and the
per-body oracle uses the posed operator unconditionally so its
$\Delta$\textsf{SR} is also identically zero (a useful sanity check
for the implementation). The proposed multi-body ECBF, which
substitutes a T-pose Cauchy envelope on tier-A and tier-B bodies in
the ablate run, exhibits a bimodal slot-level distribution: $35.7\%$
of slots sit at $\Delta\textsf{SR} < -1\,$Gbps where the posed-Q
solver fell into min-absorption fallback while the Cauchy variant
found a feasible non-degenerate solution, $12.6\%$ sit at
$\Delta\textsf{SR} > +1\,$Gbps in the opposite configuration, and
$40.6\%$ sit within $|\Delta\textsf{SR}| < 100\,$Mbps where the two
methods land near the same operating point. Median
$\Delta\textsf{SR}$ is $-78\,$Mbps and the mean is $-3.1\,$Gbps,
pulled by the longer left tail. The mechanism is solver-side: the
posed operator is more spectrally concentrated than its T-pose
envelope, defining a tighter feasible cone in the $\bm\lambda$
direction and triggering the min-absorption fallback more often
($83.6\%$ for the proposed solver under aware versus $55.1\%$ under
ablate, in the $\textsf{max\_outer} = 8$ cap of
\cref{sec:multibody-ecbf}, with warm-started
$\bm\lambda^{(t)}$ from $\bm\lambda^{(t-1)}$). The exposure side
tells the opposite story.

The right panel of \cref{fig:pose-info} plots the per-body absorbed
power CDF for the multi-body ECBF, pose-aware versus pose-ablated.
The pose-aware curve dominates on compliance: only $0.96\%$ of
body-slots violate the budget under aware versus $4.58\%$ under
ablate, a $4.7\times$ reduction. The Cauchy solver's looser
feasible set lets the maximizer push the actual absorbed power
through the budget when the T-pose envelope under-estimates the
posed body's true absorption; the pose-aware solver, working against
the tighter posed operator, holds margin in hand. Pose telemetry
shapes the QCQP feasible set tighter and lowers realised exposure,
at the cost of more frequent solver fallback (and lower mean
sum-rate, $2.55\,$Gbps versus ablate's $5.63\,$Gbps) under the
present $\textsf{max\_outer} = 8$ Newton cap. Uncapping the outer
sweep, switching to a low-rank Q decomposition that already lands
in the implementation, and replacing the Cauchy-on-tier-C handling
with a Lanczos-projected envelope are the three direct follow-ups
this trade-off points to: pose information buys a tighter exposure
profile, and the solver path needs to be tightened in lockstep to
realise the sum-rate gain that the binding constraint promises.

\paragraph*{Panel-size sensitivity}\label{par:panel-size-pose}
At $M = 256$ the bimodal trade-off in $\Delta\textsf{SR}$ widens. The proposed solver puts $20.4\%$ of slots into the left tail ($\Delta\textsf{SR} < -1\,$Gbps) and $11.5\%$ into the right tail, with median $\Delta\textsf{SR} = -438\,$Mbps and mean $-3.4\,$Gbps; the per-slot compliance gain compresses from $4.7\times$ at $M = 64$ to $2.5\times$ at $M = 256$ ($3.41\%$ aware versus $8.44\%$ ablate violations), and the proposed-solver fallback rate climbs from $83.6\%$ to $88.5\%$ under aware. The mechanism is unchanged --- the posed operator is more spectrally concentrated than its T-pose envelope --- but the larger array spreads $\bm\lambda$ across more orders of magnitude per slot, and the $\textsf{max\_outer} = 8$ Newton cap of \cref{sec:multibody-ecbf} handles the wider dynamic range less gracefully. The same three follow-ups apply, with the panel-size scaling making them more, not less, urgent: an uncapped Newton with restart, the lifted Cauchy-on-tier-C envelope, and the warm-start dual that this implementation already carries.

\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{pose_info_gain.pdf}
  \caption{Pose-information ablation, plaza-specular path model,
  binding regime ($43\,$dBm conducted transmit power, $E_{\mathrm{RL}}
  = 6\,$V/m). Left:
  slot-level sum-rate delta (pose-aware minus pose-ablated) ECDF,
  four precoders. MRT and worst-case back-off coincide with
  $\Delta = 0$ by construction. The constrained QCQP variants are
  bimodal: a left tail at $\Delta\textsf{SR} \approx -10\,$Gbps
  (pose-aware in solver fallback) and a right tail at
  $+20\,$Gbps (Cauchy in solver fallback). Right: per-body
  absorbed-power ECDF for the proposed multi-body ECBF, pose-aware
  versus pose-ablated. Pose-aware dominates at every quantile, with
  realised $P_{\mathrm{abs}}$ held further below
  $L_{\mathrm{RL}}$. The trade-off is solver-side, not physical:
  the tighter pose-aware operator shapes a smaller feasible set that
  the present 8-iteration Newton sweep handles less robustly.}
  \label{fig:pose-info}
\end{figure}
...
The pose-information ablation tells a binding-regime story rather
than a single-direction gain. Pose telemetry shapes
$\Qmat^{(u)}(\bm\theta)$ by aligning the leading eigenvector with
the body's torso plane, but in the slack regime $\bm\lambda = 0$
drops the operator out of the precoder and any gain disappears. In
the binding regime ($43\,$dBm conducted transmit power against the
post-2014 Brussels / Italy attention level $E_{\mathrm{RL}} =
6\,$V/m), the operator binds and pose information becomes a real
lever with two distinct effects. The pose-aware solver lowers the
body-slot violation rate from $4.58\%$ to $0.96\%$, a $4.7\times$
compliance improvement, but the tighter posed feasible cone also
makes the dual-Newton sweep harder to converge inside the
$\textsf{max\_outer} = 8$ cap (min-absorption fallback rises from
$55\%$ under T-pose Cauchy to $84\%$ under posed). The slot-level
sum-rate gap is bimodal in consequence rather than monotone in
either direction; the proposed solver's mean sum-rate sits at
$2.55\,$Gbps under aware versus $5.63\,$Gbps under ablate, even
though the per-body oracle (which uses the posed operator
unconditionally) achieves $11.79\,$Gbps with binding compliance.
The honest reading is that pose telemetry buys a tighter exposure
profile in exchange for a more delicate solver under the present
inner-loop cap; uncapping the Newton sweep and the
low-rank-Q-driven Cholesky reuse already in the codebase close most
of this gap (the oracle's $26\%$ fallback rate is what the proposed
solver should reach once the Cauchy-on-tier-C envelope is replaced
with a tighter Lanczos-projected operator). The same logic applies
to the multi-body solver structure: it is a generalisation that
contains MRT, ZF, and worst-case back-off as limits and degrades to
one of them in the slack regime without a code path change.
...
dose tier separation, the pose ablation's solver/exposure trade-off,
and the bound geometry are all stable across these choices.
...
\gvec_k$, generalising regularised zero-forcing. The loop runs at the cadences
...
find a four-orders-of-magnitude budget margin at $K = 25$ at
the current Brussels reference level ($E_{\mathrm{RL}} = 14.57\,$V/m),
an empirical operator rank that we measure at exactly 3 to numerical
precision (relative Frobenius reconstruction error $5\times 10^{-13}$
at $r = 3$, a result that the closed-form low-rank Cholesky path of
\cref{sec:multibody-ecbf} exploits), a chronic-dose ECDF that
separates the served, cooperating, and bystander tiers by a factor
of 1.5 between served and cooperating after $5$-min steady-state
walks, and a binding-regime pose-information ablation under
$E_{\mathrm{RL}} = 6\,$V/m (post-2014 Brussels / Italy outdoor
attention level) in which pose-aware operators reduce the per-body
violation rate $4.7\times$ ($0.96\%$ versus $4.58\%$) at the cost of
a higher solver fallback rate, with the per-body oracle showing what
the proposed solver could achieve once the Cauchy-on-tier-C
envelope is tightened. The standard
...
\begin{equation}\label{eq:KKT}
  ...
  \ww_k = u_k v_k \hh_k,
\end{equation}
with $\Hintf$ as in \cref{eq:Hgram}. This is \cref{eq:closedform}
with $\gvec_k = u_k v_k \hh_k$. The dual variables $(\bm\lambda, \nu)$
