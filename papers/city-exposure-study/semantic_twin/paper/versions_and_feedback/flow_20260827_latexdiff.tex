...
%DIF DEL versions_and_feedback/flow_20260827_before/main.tex   Thu Aug 27 03:56:33 2026
%DIF ADD build/main.tex                                        Thu Aug 27 04:36:53 2026
...
\DIFdelbegin \DIFdel{We do not estimate whole-city or deployed-network exposure.
...
\DIFdelbegin \DIFdel{also depends on the arrival direction and }\DIFdelend \DIFaddbegin \DIFadd{depends on each arrival direction relative to }\DIFaddend body orientation~\cite{icnirp}.
\DIFdelbegin \DIFdel{Therefore, exposure assessment along a walking route must retain }\DIFdelend \DIFaddbegin \DIFadd{Thus, the city changes the power and directions that reach a pedestrian, while
...
retain each arrival }\DIFaddend direction until the field is applied to the body.

Prior exposure studies provide several parts of this \DIFaddbegin \DIFadd{route-to-body }\DIFaddend link. Ray tracing gives
...
absorption~\cite{varsier}. \DIFdelbegin \DIFdel{These studies use different sourceassumptions. A common normalizationis needed before their exposure values can be compared}\DIFdelend \DIFaddbegin \DIFadd{Taken together, these studies provide the main
propagation, source, and body components. However, they do not combine those
components across cities under one source normalization}\DIFaddend .

\DIFaddbegin \DIFadd{The combined calculation also needs surface materials. }\DIFaddend Detailed
...
support radio propagation. \DIFdelbegin \DIFdel{They do not connect }\DIFdelend \DIFaddbegin \DIFadd{These scene methods have not been joined with }\DIFaddend a
common source model \DIFdelbegin \DIFdel{, retained
arrival directions, and human whole-body SAR }\DIFdelend \DIFaddbegin \DIFadd{and direction-aware body calculation }\DIFaddend across routes in
several cities.
...
separate, with their arrival directions, until the body calculation. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{Together,
these stages connect the city scene to body absorption. The propagation }\DIFaddend model
includes one reflection\DIFdelbegin \DIFdel{only}\DIFdelend .
...
differ by a factor of 14.31. \DIFdelbegin \DIFdel{We do not estimate population exposure,
whole-city exposure, or exposure from a deployed network}\DIFdelend \DIFaddbegin \DIFadd{These values describe the 163 fixed observations
under the stated source model}\DIFaddend .
...
Fig.~\ref{fig:configuration} shows \DIFdelbegin \DIFdel{the configuration}\DIFdelend \DIFaddbegin \DIFadd{how the inputs come together at Prague Old
Town Square}\DIFaddend . A 360-degree street image is aligned with a photogrammetric city
...
\DIFaddbegin \DIFadd{With this configuration fixed, }\DIFaddend Table~\ref{tab:routes} \DIFdelbegin \DIFdel{lists }\DIFdelend \DIFaddbegin \DIFadd{summarizes }\DIFaddend the study
...
observations \DIFdelbegin \DIFdel{. Population and place sampling lie outside the study}\DIFdelend \DIFaddbegin \DIFadd{that describe the selected routes}\DIFaddend . A different route would change
...
Two image models are applied in sequence. \DIFaddbegin \DIFadd{First, }\DIFaddend Mask2Former assigns a Vistas object
class such as building, road, or vegetation to every pixel~\cite{mask2former,vistas}.
\DIFaddbegin \DIFadd{Next, }\DIFaddend SAM~3 Agent performs the agentic AI step by assigning material and vegetation
labels inside compatible object regions~\cite{sam3}. \DIFaddbegin \DIFadd{Its prompt-guided concepts
extend the fixed object classes with material evidence. }\DIFaddend The camera position and
...
Fig.~\ref{fig:flowchart} \DIFdelbegin \DIFdel{shows the flowchart of the method}\DIFdelend \DIFaddbegin \DIFadd{connects the scene-building and exposure stages. The
...
arrivals and apply them to the body}\DIFaddend .
...
\DIFdelbegin \DIFdel{Every }\DIFdelend \DIFaddbegin \DIFadd{Following the central path in Fig.~\ref{fig:flowchart}, every }\DIFaddend route point uses
...
\DIFdelbegin \DIFdel{We consider }\DIFdelend \DIFaddbegin \DIFadd{The possible roofline transmitters in Fig.~\ref{fig:flowchart} represent }\DIFaddend a
...
calculation \DIFdelbegin \DIFdel{does not model coordinated
transmission or beamforming. It }\DIFdelend \DIFaddbegin \DIFadd{treats the possible sites as independent power sources and
}\DIFaddend distributes expected transmitters uniformly per
...
uses the angular cells. \DIFaddbegin \DIFadd{Fig.~\ref{fig:adjoint} contrasts the two sampling paths
for this diffuse term. }\DIFaddend \emph{Adjoint ray tracing} launches rays from the
observation point and tests visibility from each first surface hit to every
roofline source~\cite{behlouli2014,cocheril2007}. \DIFdelbegin \DIFdel{Fig.~\ref{fig:adjoint} shows
the difference from forward sampling. }\DIFdelend This source connection is a next-event
...
omitted. Woody vegetation identified by the images \DIFdelbegin \DIFdel{does not block }\DIFdelend \DIFaddbegin \DIFadd{is transparent to }\DIFaddend rays
...
The \DIFaddbegin \DIFadd{last two blocks in Fig.~\ref{fig:flowchart} make the propagation-to-body
handoff explicit. Accordingly, the }\DIFaddend calculation keeps each arrival direction
...
Only the diffuse estimate varies between runs. \DIFdelbegin \DIFdel{Scalar values and body }\DIFdelend \DIFaddbegin \DIFadd{Whole-body SAR values and body-surface }\DIFaddend fields
...
image acquisition, alignment, and surface-label assignment because their full
...
\DIFdelbegin \DIFdel{Validation }\DIFdelend \DIFaddbegin \DIFadd{Before applying the model to the ten city routes, validation }\DIFaddend against
...
power gives a maximum adjoint-to-Sionna difference of 0.80\%. \DIFdelbegin \DIFdel{The figure }\DIFdelend \DIFaddbegin \DIFadd{Fig.~\ref{fig:controlled-validation}
also }\DIFaddend shows the reflected component and its signed percentage difference from quadrature. This test
covers diffuse normalization, visibility, inverse-square loss, and cosine terms
in a one-reflection scene.
\DIFdelbegin \DIFdel{It does not validate the image-derived surface
...
\DIFaddbegin \DIFadd{After this controlled check, }\DIFaddend Fig.~\ref{fig:route-distributions} \DIFdelbegin \DIFdel{shows the
fixed-route empirical distributions }\DIFdelend \DIFaddbegin \DIFadd{compares the
route distributions and component shares across all 163 observation points}\DIFaddend .
Table~\ref{tab:route-results}
\DIFdelbegin \DIFdel{gives the }\DIFdelend \DIFaddbegin \DIFadd{complements the figure with exact }\DIFaddend route quantiles and excess above direct-path
...
\caption{Normalized whole-body SAR on the ten fixed routes. (a)~Empirical CDFs include all 163 observation points. Hollow triangles mark the six points with zero direct and zero single-reflection specular power. (b)~Route-mean component shares. The \DIFaddbeginFL \DIFaddFL{distributions describe the selected }\DIFaddendFL routes \DIFdelbeginFL \DIFdelFL{are case studies and support no city or population inference}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{under the stated source model}\DIFaddendFL .}
...
Mexico City & \DIFdelbeginFL \DIFdelFL{$9.92\times10^{-7}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$1.01\times10^{-6}$ }\DIFaddendFL & 0.1291 & 0.2958 & 0.69 & 3 \\
Milan & 0.008529 & 0.009016 & 0.009861 & 1.14 & 0 \\
Prague & 0.01216 & 0.01307 & 0.01460 & 1.09 & 0 \\
Tokyo Hachiko & \DIFdelbeginFL \DIFdelFL{$3.66\times10^{-5}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$3.63\times10^{-5}$ }\DIFaddendFL & 0.009674 & 0.02520 & 0.84 & 3 \\
...
concentration explains the larger lower-tail variation. \DIFdelbegin \DIFdel{It does not measure
city-sampling or route-selection uncertainty}\DIFdelend \DIFaddbegin \DIFadd{This comparison
quantifies ray-sampling convergence for the fixed routes}\DIFaddend . The supplementary material gives
...
treatment of woody vegetation as nonblocking. \DIFdelbegin \DIFdel{It does not measure material
accuracy or isolate reflectance alone}\DIFdelend \DIFaddbegin \DIFadd{Independent surface measurements
are required for a material-accuracy assessment}\DIFaddend . The supplementary material gives
...
ten selected routes. \DIFdelbegin \DIFdel{Street }\DIFdelend \DIFaddbegin \DIFadd{Fig.~\ref{fig:flowchart} links this spread to street }\DIFaddend width,
building height, roofline visibility, surface materials, \DIFdelbegin \DIFdel{and canopyall differ between these routes. The study does
not separate their individual effects}\DIFdelend \DIFaddbegin \DIFadd{canopy, and
directional body coupling}\DIFaddend . Each
distribution \DIFdelbegin \DIFdel{applies only to its }\DIFdelend \DIFaddbegin \DIFadd{describes its fixed }\DIFaddend route and observation points\DIFdelbegin \DIFdel{, so the
result is not a city ranking}\DIFdelend \DIFaddbegin \DIFadd{. Therefore, the
14.31-fold contrast describes the selected routes under this model}\DIFaddend .
...
both the surface parameters and vegetation rule. \DIFdelbegin \DIFdel{It does not establish material
accuracy}\DIFdelend \DIFaddbegin \DIFadd{Independent surface
measurements are required for a material-accuracy assessment}\DIFaddend .

\DIFdelbegin \DIFdel{The study has several limitations. It uses one frequency , }\DIFdelend \DIFaddbegin \DIFadd{This study covers ten selected routes at one frequency with }\DIFaddend one body model, \DIFdelbegin \DIFdel{ten
selected routes, }\DIFdelend one
roofline source model, and one set of image-derived surface labels. The
normalized values can be scaled to a specific deployment \DIFdelbegin \DIFdel{only }\DIFdelend when its transmitter
distribution matches the source model. The propagation \DIFdelbegin \DIFdel{model
}\DIFdelend \DIFaddbegin \DIFadd{calculation }\DIFaddend includes
direct paths and one specular or diffuse reflection\DIFdelbegin \DIFdel{only}\DIFdelend . The controlled comparison
\DIFdelbegin \DIFdel{validates }\DIFdelend \DIFaddbegin \DIFadd{covers }\DIFaddend the diffuse component in a one-reflection scene. \DIFdelbegin \DIFdel{The complete city calculation remains unvalidated. }\DIFdelend Variation across 64
runs \DIFdelbegin \DIFdel{measures
}\DIFdelend \DIFaddbegin \DIFadd{quantifies }\DIFaddend random ray-sampling error\DIFdelbegin \DIFdel{but not uncertainty in geometry}\DIFdelend \DIFaddbegin \DIFadd{. Geometry}\DIFaddend , image alignment, surface
labels, route choice, transmitter placement, body shape, \DIFdelbegin \DIFdel{or body orientation
}\DIFdelend \DIFaddbegin \DIFadd{and body orientation
form separate sources of uncertainty}\DIFaddend . The recorded GPU times cover \DIFdelbegin \DIFdel{only }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend ray and
body calculations.
...
the total at any point by at most 0.32\%. \DIFdelbegin \DIFdel{We do not rank cities or predict
exposure from a deployed network}\DIFdelend \DIFaddbegin \DIFadd{These values describe the selected
routes under the stated roofline source model}\DIFaddend .
