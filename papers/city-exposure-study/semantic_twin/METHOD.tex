...
wavelength. At $28$~GHz the free-space wavelength is $10.71$~mm, so a brick wall
with $1$~mm of RMS height variation gives, from this repository's implementation:
...
\bottomrule
\end{tabular}
\caption{A millimetre of roughness on brick at $28$~GHz. At normal incidence only
a quarter of the reflected power stays specular.}
\end{table}

Read the first row again. At normal incidence, three quarters of the power
leaving that wall is diffuse. The specular skeleton that the image-method search
finds so elegantly accounts for about a quarter of the physics there, rising to
most of it only at grazing incidence.

This is not a defect in the idea, but it does settle the architecture, and it
settles it against a pure search:
...
Is the method clever? Yes, and more so than the usual pitch for it. The pitch
``the first bounce is free so it is fast'' is weak, worth maybe a factor of
$1.5$. The real content is twofold. First, the panorama supplies material and
clutter at centimetre scale on the bounce that carries most of the power, where a
mesh-only tracer has a $2$~m-error triangle and no material at all. Second, the
closed specular paths form a measure-zero set that random ray shooting provably
cannot find, which makes the pair-search framing correct rather than merely
convenient, and which makes aggressive geometric simplification a hard
requirement rather than a nicety.

Where I would temper the enthusiasm: at $28$~GHz with realistic surface roughness
most of the reflected power is diffuse, so the elegant specular search covers a
minority of the physics and must be paired with ordinary gather-based Monte Carlo.
The result is a hybrid, and the paper should say so up front.
