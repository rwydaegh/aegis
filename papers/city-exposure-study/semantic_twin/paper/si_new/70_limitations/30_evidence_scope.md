% PREV: The five routes are selected case studies. Their differences do not rank the
% PREV: five cities and do not estimate population exposure. Each result is normalized
% PREV: per unit $\rho_A P_{\mathrm{EIRP}}$, so it is not an absolute prediction for an
% PREV: operator deployment. The study uses one 15 GHz frequency, one body phantom,
% PREV: one body orientation that faces along the walk, one roofline transmitter model, and five
% PREV: plaza routes. Additional frequencies, body models, headings, deployment laws,
% PREV: street canyons, parks, and repeated route selections are outside this study.
