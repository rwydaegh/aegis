...
population sit at $1.21\,\%$ for projected ZF and $2.39\,\%$
for the dual ascent at $K = 25$, both concentrated entirely on
undetected tier-C bystanders that absent themselves from the
QCQP; the dual ascent's $\sim 2 \times$ wider tail comes from
its per-stream shaping leaving more residual on undetected bodies
than the uniform projection does. Projected ZF is the preferred
...
saturate the MCS sum-cap, and ZF + proj (deployable) edges out
the dual ascent on aggregate compliance with margins of
$1.05\times$ to $19\times$ on violation rate, traceable to the
dual ascent's rank-$r$ truncation of $\Qmat^{(u)}$ and
FD-Jacobian noise that the closed-form scaling does not incur.
