"""What the image evidence is allowed to say.

Two vocabularies live here and they answer different questions.

:data:`OBJECT_WORDS` and :func:`is_object` are the coarse surface-or-object
test the quadtree projection path uses. A label naming a discrete object must
never paint the background triangle its camera ray happened to land on.

:class:`ConceptCatalog` is the open-vocabulary prompt set, and it is the larger
half. It carries three things that have to stay consistent with each other: the
prompts and their entity, material and attribute distributions; the binding from
every material label to the RF library row that supplies its dielectric model,
so an unbound label is visible rather than silent; and the bridge to the dense
Mapillary Vistas backend, which says which dense classes can support each
concept and what material spread a bare dense class carries.
...
:func:`gate_prompts` is the cheap half of the two-backend cascade: it asks a
concept only where the dense pass found a class that could carry it, which is
what keeps the prompt catalogue's cost linear in what a view actually contains.
"""
...
#: The coarse surface-or-object test, covering movable objects and street
#: furniture in one set.
#:
#: ``build_fishnet_surface.py`` deliberately keeps a finer split, separating
#: ``TRANSIENT_WORDS`` from ``NON_SURFACE_WORDS`` so its rejection table can say
#: whether a face was dropped because something moved through the shot or
#: because the class is reconstructed as its own proxy geometry. The two are not
#: merged here because the coarse set is what the quadtree path has always used
#: and its outputs are still being compared against the fishnet ones.
