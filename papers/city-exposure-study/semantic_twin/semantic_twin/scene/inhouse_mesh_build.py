...
from semantic_twin.gltf import mesh_node_matrices
...
def exact_world_matrices(
    imported: list[Any], payloads: list[tuple[dict[str, Any], pathlib.Path]]
) -> dict[str, np.ndarray]:
...
def load_manifest(tiles_dir: pathlib.Path) -> tuple[dict[str, Any], list[tuple[dict[str, Any], pathlib.Path]]]:
    ...
    for index, record in enumerate(records):
        if not isinstance(record, dict) or not isinstance(record.get("file"), str):
            raise ValueError(f"Tile record {index} must contain a string file field")  # noqa: TRY004
...
def import_tiles(payloads: list[tuple[dict[str, Any], pathlib.Path]], collection: Any) -> list[Any]:
    import bpy
...
def move_scene_to_local_enu(imported: list[Any], transform: np.ndarray) -> None:
...
def write_provenance(
    ...
    *,
    ...
    provenance = {
        ...
        "source_manifest": str((config.tiles / "manifest.json").resolve()),
        "source_acquisition": {
        ...
        "output_ply": str(config.out.resolve()),
        "output_bytes": config.out.stat().st_size,
        "blend": str(config.blend.resolve()) if config.blend is not None else None,
...
def build_inhouse_mesh(config: InhouseMeshBuildConfig) -> None:
    import bpy

    config = InhouseMeshBuildConfig(
        tiles=config.tiles.resolve(),
        out=config.out.resolve(),
        crop_radius_m=config.crop_radius_m,
        blend=config.blend.resolve() if config.blend is not None else None,
    )
    manifest, payloads = load_manifest(config.tiles)
    ...
    faces = crop_faces(vertices, faces, config.crop_radius_m)
    if not len(faces):
        raise RuntimeError("The horizontal crop removed every triangle")
    vertices, faces = compact(vertices, faces)

    write_ply(config.out, vertices, faces)
    provenance_path = config.out.with_suffix(".json")
    if config.blend is not None:
        move_scene_to_local_enu(imported, transform)
        bpy.ops.wm.save_as_mainfile(filepath=str(config.blend))
        print(f"[blend] {config.blend} (single precision, inspection only)", flush=True)
    write_provenance(
        provenance_path,
        config=config,
        manifest=manifest,
    ...
    print(f"[ply] {config.out}", flush=True)
