...
a tens-of-element panel saturates at $30$-$50\,$dB SINR margin), the
...
saturates this cap at $30$-$50\,$dB SINR for the served-user count
...
P_{\mathrm{tx}}$, and let $\Pabs^{(u)}_{\mathrm{ZF}} = \sum_k
...
\begin{equation}\label{eq:primal-projection}
  ...
  \alpha = \min\!\left(1,\;
    \sqrt{\eta_S\,\min_{u \in \AllBodies}
      \frac{L^{(u)}}{\Pabs^{(u)}_{\mathrm{ZF}}}}
  \right),\;}
\end{equation}
with $\Pabs^{(u)}_{\mathrm{ZF}}$ evaluated against the
...
\item \emph{Always feasible.} For any input $\WW_{\mathrm{ZF}}$,
  ...
  $\alpha^2\,\Pabs^{(u)}_{\mathrm{ZF}} \leq \eta_S\,L^{(u)} \leq
  L^{(u)}$ for every $u$.
\item \emph{Identity on iterates with margin.} If
  $\max_u \Pabs^{(u)}_{\mathrm{ZF}}/L^{(u)} \leq \eta_S$, the
...
\item \emph{Bounded SINR cost.} Per-stream SINR scales as
  ...
  \max_u \Pabs^{(u)}_{\mathrm{ZF}}/L^{(u)} > \eta_S^{-1}$,
...
so $\alpha^2\,\Pabs^{(u)}_{\mathrm{ZF}} \leq \eta_S\,L^{(u)} \leq
...
= 3\,$V/m and $\mathrm{SINR}^{\mathrm{ZF}}_{\min} \approx 50\,$dB
on the $99$th percentile served user, so the hypothesis holds with
margin of $\sim 8$-$13\,$dB and ZF + projection saturates the MCS
cap on every slot of the trace. \Cref{prop:projection-optimality}
...
makes the $\Pabs^{(u)}_{\mathrm{ZF}}$ in
\cref{eq:primal-projection} computable per slot.

\subsection{When dual ascent on $\bm\lambda$ earns its keep}\label{sec:dualascent-ref}
...
\cref{prop:projection-optimality}. Regularised ZF on the $M = 64$
panel achieves served-user SINR of $40$-$50\,$dB, which is
$10$-$20\,$dB above the MCS28 clip threshold of $\sim 30\,$dB. The
worst-body absorbed-power overshoot factor is bounded at
$\rho \leq 17$ in the plaza traces, which corresponds to
$\alpha^2 \geq \eta_S/17 \approx 0.057$ and post-projection SINR of
$\geq 28\,$dB above MCS threshold for the worst served user; the
clip engages and rate-cost is zero. The dual-ascent solver, by
...
constraints to non-served bodies, but ZF retains its $\sim 50\,$dB
SINR margin on the served users; projection scales the precoder
down by the bystander-overshoot factor and rate-cost is again zero
because the post-projection SINR is still above MCS clip.
...
\subsection*{Dual ascent on $\bm\lambda$}
