...
\begin{enumerate}
  ...
  about how a network is operated rather than about how waves travel, so the
  variant that weights each site by its own free space spreading is carried
  through and reported alongside. The choice is not cosmetic: the street level
...
and under the isotropic model it equals the sky fraction, which is the identity
checked in Section~\ref{sec:valid}. The second term is everything the
...
interaction reference over 40 standpoints at fixed seeds, so that only the budget
moves, $L = 3$ costs a median of 0.002\,dB and never moves a standpoint by more
...
estimator at the same cost, so it is switched off. The only approximation left
in the trace is a single one sided truncation whose size is quoted above.
...
the split in \eqref{eq:split}. The sky fraction is the share of rays that escape
without touching anything, which is the open solid angle above the standpoint
and depends on the geometry alone rather than on any illumination model. The
mean excess path length, taken over escaping rays weighted by their power, gives
