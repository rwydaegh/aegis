...
directions instead, its median disagreement with the tracer's direct term is
0.19\% isotropic, 1.14\% rooftop, and 3.70\% street. The corresponding
worst disagreements are 1.29\%, 6.44\%, and 23.05\%. The check uses an
...
median of 0.06\,dB isotropic, 0.15\,dB rooftop and 0.42\,dB street. Every choice
...
the same calculation at 2\,GHz raises those medians to 0.18, 0.53 and 1.78\,dB, a
...
Traced at 42 standpoints across four photogrammetric meshes on three continents,
...
in a thousand. The eighth returns 0.362, and since it stands eight meters from
one scoring 0.999 that is a registration failure rather than a coverage one. That
station had passed the skyline residual gate, so first interaction coverage is
the sharper test of a pose, and it is excluded. Away from a camera the two
...
93 to 96\% of the time and the third about 89\%, whereas beyond 40\,m every
...
above was measured on four meshes on three continents and varied by a few
percent, and that power-weighted photographic coverage at a station position
runs 0.941 to 0.999 at the first interaction across 35 stations in those four
cities. The reported budget is therefore set by evidence at one square and
carried to the others by power, with the geometric part checked in four.

What makes three sufficient rather than merely defensible is where the power is.
Of all launched power, 21.0\% never touches a surface and escapes straight
to the sky, which is the direct term of \eqref{eq:split}. The shares reaching
the first, second, and third surfaces are 79\%, 8.8\%, and 1.2\%.
Interactions past the third carry 0.23\% of launched power. Those depths can be justified
neither by evidence nor by the power they carry. $L = 3$ costs a median of
0.002\,dB and never moves a standpoint by more than 0.063\,dB when it is traced
against an eight interaction reference over 40 of these standpoints at fixed
seeds, so that only the budget moves. Not one standpoint of the 40 moves half a decibel under any of
...
runs out is 0.0037 at the median standpoint and 0.0201 at the worst, and every
...
few survivors twentyfold. A repeated-seed test found the same relative spread to
three significant figures with roulette on and off, and wall times of 101 and
100\,s. It is switched off because it changes neither variance nor work at this
budget. The only approximation left in the trace itself is a single one sided
...
the square would impose. That one is recorded per standpoint but is not analysed
...
baselines, 2.15 people per square meter cost a median 1.4\,dB under the street
...
street columns move only from 1.07 to 1.33 and from 1.40 to 1.61\,dB. A crowd
...
interaction. These are property tests over randomized inputs rather than single
cases.
...
Two parameters are measured here, and a third, the bounce budget, is corroborated
...
estimator. Integrated over the visible directions, its median disagreement with
the estimator's direct term is 0.19\% isotropic, 1.14\% rooftop, and
3.70\% street. Only the direct term is covered by this check. The reflected
term is checked against an independent ray tracer, run on the same meshes with
the same materials. Taking the median over the three illumination models, the
two agree to within 0.25\,dB at the reference square and 0.16\,dB at a second
square, against a Monte Carlo floor on the comparison itself of 0.24 to
0.46\,dB. The comparison therefore resolves no disagreement.
...
and 0.022\,dB under street-cell illumination. The corresponding within-Korenmarkt
spreads were 3.9, 8.4, and 16.7\,dB. The 0.024\,dB isotropic shift was 5.7 times
...
rooftop ratio by $-0.164$ and $+0.414$\,dB. The dielectric span was therefore
0.34\,dB isotropic and 0.58\,dB rooftop. Metal changed the same medians by
...
50 to 500\,m changed the median absolute ratio by 7.40\,dB rooftop and 9.24\,dB
...
rooftop, and street illumination. The largest changes were 0.242, 0.116, and
0.326\,dB. These values are 7 to 47 times their Monte Carlo floors. The
...
whereas 79\%, 8.8\%, and 1.2\% reached the first three surfaces. Interactions
past the third carried 0.23\% of launched power. Against an eight-interaction
reference, the three-interaction budget changed the median by 0.002\,dB and no
standpoint by more than 0.063\,dB. The truncated share was 0.0037 of escaping
power at the median standpoint and 0.0201 at the worst. Roulette changed neither
...
\begin{figure*}[!t]
  ...
  the roulette control. The bottom row gives every paired standpoint change for
...
An independent ray tracer checked the reflected term on the same meshes and
materials. The median difference over the three illumination models was
0.25\,dB at the reference square and 0.16\,dB at a second square, against a
0.24 to 0.46\,dB Monte Carlo floor on the comparison. No disagreement was
resolved. The check tests transport but not the illumination law because both
...
\begin{table}[!t]
  ...
  \begin{tabular}{@{}p{0.24\columnwidth}p{0.51\columnwidth}p{0.17\columnwidth}@{}}
    ...
    Three-interaction truncation & 0.0037 of escaping power at the median &
    Biases low \\
...
Monte Carlo error is smaller than standpoint sampling. The standard errors of a
...
three-interaction budget on the same 12 standpoints, where every headline number
moved by less than the estimator's own noise floor for its column. Its mechanism
...
ratio by only hundredths of a decibel. The measured dielectric bracket stayed
below 0.58\,dB, whereas metal changed the rooftop median by 4.374\,dB. Geometry
