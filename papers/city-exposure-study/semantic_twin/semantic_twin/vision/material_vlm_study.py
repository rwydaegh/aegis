...
def _decode(line: str) -> tuple[dict[str, Any], bool]:
    ...
    if deficit <= 0:
        # Preserve the historical bare-raise result while making the intended
        # failure explicit outside an exception handler.
...
def analyse_material_vlm(out: pathlib.Path, frequency_ghz: float = 15.0) -> None:
    """Analyse blinded material responses and write the study report."""
    responses, crops, notes = load_all(out)
    ...
    (out / "analysis.json").write_text(json.dumps(report, indent=2, default=float))
