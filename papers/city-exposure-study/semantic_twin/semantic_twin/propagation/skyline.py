...
distance rather than a distribution over one. The old law had to cap the range,
...
The answer is to connect to the source rather than wait to hit it. At each
vertex of a path, sample a point on the roofline, cast one ray to it, and take
the contribution if that ray is clear. The visibility question is then answered
from the vertex where it is asked. This is next event estimation, and the same
call at the zeroth vertex is the direct term, so both halves are one estimator.
...
def connect(
    ...
    *,
    ...
    per site flux and is unbiased for the line integral.
    """
