...
def propagating_reach(wall: MasonryWall, frequency_hz: float, theta_deg: float) -> tuple[int, int]:
    ...
    return math.ceil(span * wall.cell_x_m / lam), math.ceil(span * wall.cell_y_m / lam)
...
HERO_CASES = (
    {"label": "fr2_28ghz_30deg", "frequency_hz": 28e9, "theta_deg": 30.0, "recess_m": 0.005},
    {"label": "fr2_28ghz_60deg", "frequency_hz": 28e9, "theta_deg": 60.0, "recess_m": 0.005},
    {"label": "fr3_10ghz_30deg", "frequency_hz": 10e9, "theta_deg": 30.0, "recess_m": 0.005},
    {"label": "fr3_10ghz_30deg_deep", "frequency_hz": 10e9, "theta_deg": 30.0, "recess_m": 0.010},
)
...
def stage_converge(memory_budget_gib: float, time_budget_s: float) -> None:
    ...
    for hero in HERO_CASES:
        ...
        for multiplier in CONVERGENCE_MULTIPLIERS:
            harmonics = (max(2, round(multiplier * base_m)), max(2, round(multiplier * base_n)))
...
def run(
    stage: str = "all",
    *,
    memory_budget_gib: float = 18.0,
    time_budget_s: float = 2400.0,
    multiplier: float = 1.5,
) -> None:
    if stage in ("converge", "all"):
        stage_converge(memory_budget_gib, time_budget_s)
    if stage in ("spectrum", "all"):
        stage_spectrum(multiplier)
