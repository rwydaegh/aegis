...
from semantic_twin.illumination.sensitivity import (
    NORMALISATION_QUADRATURE,
    Reweighter,
    density_on,
)
...
VARIANT = "llvm_ad_rgb"
ROW_DOT = "ij,ij->i"
RANGE_CAP_LABEL = "range cap $d_{\\max}$, m"
...
class HarvestTracer(SbrTracer):
    ...
    def _built_range(self, last_vertex: np.ndarray, exit_direction: np.ndarray) -> np.ndarray:
        ...
        p = last_vertex[:, :2]
        u = exit_direction[:, :2]
        a = np.einsum(ROW_DOT, u, u)
        b = np.einsum(ROW_DOT, p, u)
        c = np.einsum(ROW_DOT, p, p) - self.crop_radius_m**2
...
def harvest_site(site: str, *, locations: int = LOCATIONS, rays: int = RAYS, stride: int = 1) -> pathlib.Path:
    ...
    mesh = paths.site_mesh(site, CROP_M)
    ...
    datum = ground_datum(geometry, radius_m=WALK_RADIUS_M)
    face_class = classify_faces(geometry.vertices, geometry.faces, datum)
    binding = load_table(paths.config_dir(), FREQUENCY_HZ)
...
def invariants(reweighters: dict[str, Reweighter], one_d: dict[str, Any]) -> dict[str, Any]:
...
def analyse(sites: tuple[str, ...]) -> pathlib.Path:
    ...
    for model in DEFAULTS:
        ...
        report[model] = {
            ...
            "invariants": invariants(reweighters, one_d),
...
def spans(reweighters: dict[str, Reweighter], model: str) -> dict[str, Any]:
    ...
    for site, values in out.items():
        values["dominant"] = max((k for k in DEFENSIBLE[model]), key=lambda k, values=values: values[k])
    return out
