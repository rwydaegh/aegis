...
Minimal voxel JSON (flat 2×2 slab) for manually checking **Environment view** (cubes vs hull vs tiles) in `python -m aegis.viewer --voxel-json tests/fixtures/viewer_env_test/voxels.json`.

Use with a small body, for example:

```bash
python -m aegis.viewer --no-open --port 5091 ^
