...
def test_compute_sites_binary(tmp_path):
    ...
    assert meta["count"] > 0
    assert len(raw) == meta["count"] * 12  # 12 bytes per record: lat(f4)+lon(f4)+op(u1)+tech(u1)+region(u1)+count(u1)
    assert len(meta["operators"]) > 0
    assert len(meta["technologies"]) > 0
    assert len(meta["region_names"]) > 0

    # Parse first record: lat(f32) + lon(f32) + op(u8) + tech(u8) + region(u8) + count(u8)
    lat, lon = struct.unpack_from("<ff", raw, 0)
    op_idx = raw[8]
    tech_idx = raw[9]
    region_idx = raw[10]
    antenna_count = raw[11]
