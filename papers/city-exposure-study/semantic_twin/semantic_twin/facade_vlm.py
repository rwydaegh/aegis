...
:func:`layered_power_reflectance` is a transfer matrix over such a stack, so a
model that names a stack can be answered analytically. Nothing else in this
repository can consume a stack.
...
happen in power, and :func:`posterior_power_reflectance` and
:func:`sample_face_materials` do it in the two defensible ways: the ensemble
mean, and a draw per face that keeps every patch a single physical material and
turns material ignorance into a spread on the answer rather than a bias in it.
...
#: The RF material vocabulary this study can actually ground. Every name here has
#: a row in ``config/itu_p2040_4.json`` reachable through ``MATERIAL_BINDING`` or
#: ``MATERIAL_SUBSTITUTION`` in ``propagation/semantic_binding.py``. A model is
#: not allowed to invent outside it, because a name with no row cannot be traced.
...
# --------------------------------------------------------------------------
# Analytical response of a layered stack
# --------------------------------------------------------------------------


def _layer_permittivity(material: str, frequency_hz: float, library: Any) -> complex:
    from .propagation.semantic_binding import MATERIAL_BINDING, MATERIAL_SUBSTITUTION

    row = MATERIAL_BINDING.get(material, (MATERIAL_SUBSTITUTION.get(material, material),))[0]
...
def stack_power_reflectance(
    ...
    permittivity = np.array([_layer_permittivity(layer.material, frequency_hz, library) for layer in stack])
...
# --------------------------------------------------------------------------
# Posteriors, and what to do with them instead of an argmax
# --------------------------------------------------------------------------
...
def material_power_reflectance(frequency_hz: float, cos_incidence: float, library: Any) -> dict[str, float]:
    ...
    for material in MATERIAL_VOCABULARY:
        if material == "unknown":
            continue
        permittivity = _layer_permittivity(material, frequency_hz, library)
