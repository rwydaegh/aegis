...
OUTPUT = paths.output("station_calibration")
...
class StationCalibrationConfig:
    ...
    seeds: tuple[int, ...] = (7, 8, 9, 10)
    conventions: tuple[str, ...] = ("pose", "pedestrian")
    rungs: tuple[str, ...] = ("walk", "semantic")
    baseline: str = "geometric"
...
def scene(site: str, crop_m: int, config: StationCalibrationConfig) -> dict[str, Any]:
    ...
    measured = measure_ground_datum(geometry, radius_m=config.walk_radius_m)
...
def stage_evidence(site: str, crop_m: int, config: StationCalibrationConfig) -> dict[str, Any]:
    ...
    throughput weight.
    """
    common = scene(site, crop_m, config)
    ...
    for convention in config.conventions:
        ...
        pooled = BounceEvidenceTally(plain, config.max_bounces)
        started = time.perf_counter()
        for index, (origin, ground) in enumerate(zip(origins, grounds, strict=True)):
            local = BounceEvidenceTally(plain, config.max_bounces)
            result = tracer.trace(
                origin,
                MODELS,
                ground_z_m=float(ground),
                seed=config.seeds[0] + 1000 * index,
                tally=local,
            )
            pooled.add(local)
...
def _eligible_rungs(site: str, config: StationCalibrationConfig) -> list[str]:
    rungs = [config.baseline]
    for materials in config.rungs:
        if materials == config.baseline:
            continue
...
def _trace_rung(
    ...
    origins: np.ndarray,
    grounds: np.ndarray,
    config: StationCalibrationConfig,
) -> dict[str, Any]:
    geometry, areas, face_class = common["geometry"], common["areas"], common["face_class"]
    bound_class, binding, provenance = bind_materials(
        materials, site, crop_m, geometry, areas, face_class, config.frequency_hz, config.variant
    )
    ...
    for seed in config.seeds:
        ...
        tracer = SbrTracer(geometry, bound_class, binding.permittivity, binding.rms_height_m, trace_config)
        standpoints = [(origins[i], float(grounds[i]), seed + 1000 * i) for i in range(origins.shape[0])]
        collected: dict[int, dict[str, float]] = {}
        started = time.perf_counter()
        for row, result in trace_standpoints(tracer, standpoints, MODELS, workers=config.workers):
        ...
        for key, values in extras.items():
            values.append([float(entry[key]) for entry in order])
        print(
...
def stage_exposure(site: str, crop_m: int, config: StationCalibrationConfig) -> dict[str, Any]:
    ...
    rungs = _eligible_rungs(site, config)
    for convention in config.conventions:
        origins, grounds, records = standpoint_set(geometry, common["entries"], common["ground_datum_m"], convention)
        report["conventions"][convention] = records
        per_rung = {
            materials: _trace_rung(site, crop_m, common, materials, convention, origins, grounds, config)
...
def _model_shift(base: list[list[float]], values: list[list[float]]) -> dict[str, Any]:
    detail = [reductions(np.asarray(low), np.asarray(values[index])) for index, low in enumerate(base)]
    paired = [entry["paired_median_shift_db"] for entry in detail]
    distribution = [entry["distribution_median_shift_db"] for entry in detail]
    return {
...
def _against_reference(per_rung: dict[str, Any], reference: str) -> dict[str, Any]:
    base = per_rung[reference]["per_seed"]
    return {
        materials: {
            "covered_fraction_by_area": record["covered_fraction_by_area"],
            "by_model": {key: _model_shift(base[key], record["per_seed"][key]) for key in MODEL_KEYS},
        }
    ...
    for convention, per_rung in exposure["rungs"].items():
        ...
        for reference in dict.fromkeys([baseline, "walk"]):
            if reference not in per_rung:
                continue
            against = _against_reference(per_rung, reference)
...
def _write_stage(site: str, kind: str, config: StationCalibrationConfig) -> None:
    path = config.out / f"{site}_{config.crop_m}m{config.tag}_station_{kind}.json"
    stage = stage_evidence if kind == "evidence" else stage_exposure
    try:
        report = stage(site, config.crop_m, config)
    except (FileNotFoundError, RuntimeError, ValueError) as error:
        print(f"[skip] {kind} {site}: {error}", flush=True)
        return
    if kind == "exposure":
...
def _refresh_report(site: str, config: StationCalibrationConfig) -> None:
    path = config.out / f"{site}_{config.crop_m}m{config.tag}_station_exposure.json"
    if not path.exists():
...
def execute(config: StationCalibrationConfig) -> int:
    config.out.mkdir(parents=True, exist_ok=True)
    for site in config.sites:
        if config.stage in ("evidence", "both"):
            _write_stage(site, "evidence", config)
        if config.stage in ("exposure", "both"):
            _write_stage(site, "exposure", config)
        if config.stage == "report":
            _refresh_report(site, config)
    return 0
