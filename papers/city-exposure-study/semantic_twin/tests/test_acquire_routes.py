...
from semantic_twin.scene.enu import EnuFrame, ecef_to_llh, llh_to_ecef
from semantic_twin.acquire.cache import route_key
from semantic_twin.acquire.routes import (
...
def test_the_same_waypoints_are_never_bought_twice(tmp_path: pathlib.Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("semantic_twin.acquire.routes.fetch_walking_route", stub)
...
def test_the_cache_name_ignores_a_rounding_difference_and_not_a_real_one():
    ...
    assert route_key(base, True) == route_key(same, True)
    assert route_key(base, True) != route_key(moved, True)
    assert route_key(base, True) != route_key(base, False)


# --- the anchor ---------------------------------------------------------------


def build_mesh(directory: pathlib.Path, name: str, *, version: int, lat: float, lon: float) -> None:
    """A mesh and the sidecar the study admits it on. The mesh is never opened."""
    directory.mkdir(parents=True, exist_ok=True)
    (directory / f"{name}.ply").write_bytes(b"ply\n")
    (directory / f"{name}.json").write_text(
        json.dumps({"format_version": version, "anchor": {"lat_deg": lat, "lon_deg": lon, "height_m": 0.0}})
    )


def test_the_anchor_comes_off_the_mesh_manifest(tmp_path: pathlib.Path):
    build_mesh(tmp_path / "data" / "geometry" / "somewhere", "inhouse_leaf_250m_f64", version=3, lat=12.5, lon=-7.25)
    paths.forget_disk_reads()
    assert site_anchor("somewhere", tmp_path) == (12.5, -7.25)


def test_the_anchor_comes_off_the_mesh_the_study_would_trace(tmp_path: pathlib.Path):
    """One resolver, so the anchor cannot come off a build the tracer refuses.

    This used to rebuild the file name here and take whichever manifest it found
    first, with no version gate. Korenmarkt has exactly this pair on disk: a
    version 2 build carrying up to a metre of tile seaming, and the ``_f64``
    rebuild beside it. A walk anchored on the first one is laid in metres the run
    never traces in.
    """
    directory = tmp_path / "data" / "geometry" / "somewhere"
    build_mesh(directory, "inhouse_leaf_250m", version=2, lat=1.0, lon=1.0)
    paths.forget_disk_reads()
    with pytest.raises(FileNotFoundError, match="format_version 2"):
        site_anchor("somewhere", tmp_path)

    build_mesh(directory, "inhouse_leaf_250m_f64", version=3, lat=12.5, lon=-7.25)
    paths.forget_disk_reads()
    assert site_anchor("somewhere", tmp_path) == (12.5, -7.25)
...
def test_a_and_b_are_the_two_cameras_furthest_apart():
    from semantic_twin.walk.site import span_endpoints
...
def test_distance_to_the_path_is_measured_to_the_line_and_not_to_its_corners():
    ...
    from semantic_twin.walk.site import gap_to_path
...
def test_the_walk_keeps_whichever_path_stands_nearer_a_camera(monkeypatch: pytest.MonkeyPatch):
    ...
    from semantic_twin.walk import site as route_module
