...
def test_compute_sites_count_matches(tmp_path):
    """Site count in binary response matches total antenna count."""
    from aegis.viewer.routes.coverage import _compute_coverage
...
def test_compute_sites_binary(tmp_path):
    ...
    assert meta["count"] > 0
    assert len(raw) == meta["count"] * 12  # 12 bytes per record: f4+f4+u1+u1+u1+u1
    assert len(meta["operators"]) > 0
    assert len(meta["technologies"]) > 0
