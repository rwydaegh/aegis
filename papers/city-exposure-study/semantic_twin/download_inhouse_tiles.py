...
The traversal, the region of interest and the cache live in
:mod:`semantic_twin.acquire.tiles`. This file is the argument parser and the
exit code.

The file keeps its old name on purpose. ``inhouse`` was the working name for the
Google tile feed, 79 mesh filenames on disk still carry it, and the Milan
acquisition record reproduces this command verbatim. Everything inside says
Google.
...
Set ``GOOGLE_MAPS_API_KEY`` (preferred) or ``GOOGLE_API_KEY`` first.
"""
...
from semantic_twin.acquire.tiles import (  # noqa: E402
    DEFAULT_MAX_BYTES,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        downloader = GoogleTilesDownloader(
            ...
            site_ellipsoid_height_m=args.site_height_m,
