...
def main() -> None:
    ...
    manifest = json.loads((args.out / "crops.json").read_text())
    blind_dir = args.out / "blind"
    ...
    for record in manifest["crops"]:
        for source in ("panorama", "texture"):
            ...
            shutil.copyfile(args.out / "crops" / f"{record['crop_id']}_{source}.png", blind_dir / f"{name}.png")
            key[name] = {
    ...
    (args.out / "blind_key.json").write_text(json.dumps({"key_string": KEY, "map": key}, indent=2, sort_keys=True))
    order = sorted(key)
    (args.out / "blind_order.json").write_text(json.dumps(order, indent=2))
