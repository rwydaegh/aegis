...
This supersedes `twin.mapillary` where Inhouse has coverage, for one reason that
...
Coverage still is not guaranteed. Where Inhouse has no pano within reach the building
...
CAM_HEIGHT = 2.5           # m, Inhouse's mast, close enough for the pitch solve
FILL = 0.80                # fraction of the frame the facade should occupy
FOV_MIN, FOV_MAX = 25.0, 100.0
PROBE_RADIUS = 26.0        # m, how far Inhouse may wander from the asked-for spot
...
def metadata(k: str, lat: float, lon: float, radius: float = PROBE_RADIUS,
    ...
    Inhouse's API-enable propagates unevenly across edges: immediately after
...
def is_official(meta: dict) -> bool:
    """Inhouse's own car or trekker, rather than a contributed photosphere.
    ...
    """
    return (meta.get("copyright") or "").strip() in {"© Inhouse", "©  Inhouse"}
...
def shoot_building(k: str, b: Building, anchor: Anchor, out_dir: pathlib.Path,
    ...
    """Up to `n_views` aimed shots, from standoffs Inhouse actually has a pano near.
    ...
    for off in offers:
        ...
        if any(float(np.linalg.norm(cam_xy - s)) < 7.0 for s in seen):
            continue                       # Inhouse returned the same pano again
        seen.append(cam_xy)
    ...
    for rank, meta, loc, _cam_xy, shot, box in found[:n_views]:
        ...
        recs.append({"pano_id": meta.get("pano_id"), "date": meta.get("date"),
                     ...
                     "rank": round(rank, 3), "source": "inhouse_streetview"})
    return recs
...
def main() -> None:
    ...
    ap.add_argument("--merge-mapillary", action="store_true", default=True,
                    help="keep Mapillary photos for buildings Inhouse cannot see")
    args = ap.parse_args()
    ...
    n_inhouse = sum(1 for v in photos.values() if v)
    ...
    print(f"\n[gsv] Inhouse covers {n_inhouse}/{len(targets)}, "
