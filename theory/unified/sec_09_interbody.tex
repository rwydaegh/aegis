...
\begin{equation}\label{eq:r-coeffs}
  r_s(\mu)=\frac{\mu-\xi}{\mu+\xi},\qquad
  r_p(\mu)=\frac{\ntilde^2\mu-\xi}{\ntilde^2\mu+\xi},\qquad
  \xi=\sqrt{\ntilde^2-1+\mu^2},\ \ \RE\xi>0,
\end{equation}
...
\begin{equation}\label{eq:rho-refl}
  \rho_m(\rr)=\abs{r_s(\mu)}^2 w_s+\abs{r_p(\mu)}^2 w_p
            =1-T_0(\rr),
\end{equation}
...
reflected power density $\rho_m\,\Sinc$ is deposited at the hit point,
...
\begin{equation}\label{eq:f-bound}
  f(\rr)\le 1-\eta(\rr),
\end{equation}
where $\eta$ is exactly the ambient-occlusion exposure fraction of
...
The enhancement $C$ is largest precisely where $\eta$ is smallest. A
deep concavity sees little direct power but recaptures a large share of
the reflected power, so the product $\eta\,C$ stays close to $\eta$.
By \cref{eq:f-bound} a point with $\eta=0.3$ admits $f\le 0.7$, hence
$C\le 1/(1-0.46\times 0.7)\approx 1.47$ and $\eta\,C\le 0.44$,
