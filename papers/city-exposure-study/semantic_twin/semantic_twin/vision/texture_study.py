...
class TextureStudyConfig:
    """Inputs and fixed protocol choices for the texture evidence study."""

    tiles: pathlib.Path
    mesh: pathlib.Path
    mesh_manifest: pathlib.Path
    panorama_semantics: pathlib.Path
    semantics_json: pathlib.Path
    panorama: pathlib.Path
    pose: pathlib.Path
    output: pathlib.Path
    concepts: pathlib.Path = pathlib.Path("config/semantic_concepts.json")
    crop_radius_m: float = 140.0
...
def input_provenance(args: TextureStudyConfig) -> dict[str, dict[str, Any]]:
...
def collapse_until_separable(
    ...
    *,
    ...
    while len(groups) > 2:
        ...
        if not pairs:
            break
        worst = min(pairs, key=lambda pair, matrix=separability: matrix[pair[0], pair[1]])
        first, second = worst
...
def run_texture_study(args: TextureStudyConfig) -> None:
    """Build texture evidence from validated command arguments."""
    import trimesh
