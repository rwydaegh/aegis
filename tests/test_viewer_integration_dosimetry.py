...
class TestComputeDosimetryHappyPath:
    ...
    def test_spatial_inter_body_specular1_succeeds(self, viewer_app):
        """inter_body="specular1" adds the single specular recapture bounce and
        the route returns a normal 200 spatial map."""
        with viewer_app.test_client() as c:
            resp = c.post(
                "/api/compute",
                json={
