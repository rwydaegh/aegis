...
from semantic_twin.blender_mesh import mesh_arrays
from semantic_twin.export import compact, scene_xml, write_ply
from semantic_twin.pano_geometry import panorama_to_world_matrix
from semantic_twin.vision.vocabulary import is_object
...
def project_semantics(config: SemanticProjectionConfig) -> None:
    import bpy

    config.out.mkdir(parents=True, exist_ok=True)
    (config.out / "meshes").mkdir(parents=True, exist_ok=True)

    bpy.ops.wm.read_factory_settings(use_empty=True)
    bpy.ops.wm.ply_import(filepath=str(config.mesh.resolve()))
    obj = bpy.context.object
    vertices, faces = mesh_arrays(obj)
    centroids = vertices[faces].mean(axis=1)

    with np.load(config.semantics) as semantic:
        entity = np.asarray(semantic["entity"])
        material = np.asarray(semantic["material_hint"])
        confidence = np.asarray(semantic["confidence"])
    document = json.loads(config.semantics_json.read_text())
    ...
    candidates = (~object_mask) & (~sky_mask) & (face_confidence >= config.min_confidence)
    ...
    for name, selection in groups.items():
        ...
        write_ply(config.out / relative, compact_vertices, compact_faces)
    ...
    (config.out / "scene.xml").write_text(scene_xml(shapes, comments=comments))
    np.savez_compressed(
        config.out / "face_semantics.npz",
        entity=face_entity,
    ...
    (config.out / "projection_manifest.json").write_text(json.dumps(manifest, indent=2))
    print(f"[project] {visible.sum()}/{len(faces)} first-visible faces labelled, {(~visible).sum()} left unobserved")
    print(f"[project] -> {config.out / 'scene.xml'}")
