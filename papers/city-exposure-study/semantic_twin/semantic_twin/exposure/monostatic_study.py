...
from semantic_twin.exposure.study import CONFIG, MODELS, REFERENCE_S0_W_M2, site_mesh, site_walk_semantics
...
OUTPUT = paths.output("monostatic")
UTC_FORMAT = "%Y-%m-%dT%H:%M:%SZ"


@dataclass(frozen=True)
class MonostaticStudyConfig:
...
def run_site(
    ...
    *,
    ...
    manifest = {
        ...
        "created_utc": time.strftime(UTC_FORMAT, time.gmtime()),
        "site": site,
        "mesh": str(mesh),
...
class _Plate:
    ...
    def intersect(self, origins: np.ndarray, directions: np.ndarray):
        ...
        hit, distance, normal, _face = PlaneGeometry(0.0).intersect(origins, directions)
...
def execute(config: MonostaticStudyConfig) -> None:
    if config.validate_only:
        validate()
        return
    if config.analyse_only:
        analyse(config.tag)
        figure(config.tag)
        return
    ...
    if config.visibility:
        for site in config.sites:
            run_visibility(
                ...
                walk_spacing_m=config.walk_spacing_m,
            )
        return
    if config.stations:
        for site in config.sites:
