...
@dataclass(frozen=True)
class FacadeCropConfig:
    """Selection and rendering controls for the matched facade crop study."""

    out: pathlib.Path = ROOT / "outputs" / "material_vlm"
    size: int = 384
    stride: int = 128
    min_building_fraction: float = 0.9
    per_view: int = 4
    per_wall: int = 5
    per_patch: int = 2
    patch_separation_m: float = 6.0
...
def barycentric(triangles: np.ndarray, points: np.ndarray) -> np.ndarray:
    ...
    d00 = np.einsum(ROW_DOT, v0, v0)
    d01 = np.einsum(ROW_DOT, v0, v1)
    d11 = np.einsum(ROW_DOT, v1, v1)
    d20 = np.einsum(ROW_DOT, v2, v0)
    d21 = np.einsum(ROW_DOT, v2, v1)
...
def _candidate_for_window(
    ...
    row, column, fraction = window
    local = sub_view_directions(view, row, column, context.args.size, full_size)
    directions = local @ context.rotation.T
    flat = directions.reshape(-1, 3)
    origins = np.broadcast_to(context.camera, flat.shape)
    hit, distance, normal, face = context.geometry.intersect(origins, flat)
    good = (
        hit
        & (distance < context.args.max_range_m)
        & (context.face_class[np.clip(face, 0, context.face_class.size - 1)] == 1)
    )
    if good.mean() < 0.6:
        return None
    points = origins[good] + distance[good, None] * flat[good]
    mean_normal = normal[good].mean(axis=0)
    mean_normal /= max(np.linalg.norm(mean_normal), 1e-12)
    incidence = np.arccos(np.clip(np.abs(np.einsum(ROW_DOT, flat[good], normal[good])), 0.0, 1.0))
    return {
        ...
        "incidence_deg": float(np.median(np.degrees(incidence))),
        "image_path": image_path,
    }


def _collect_candidates(context: _CandidateContext, building_id: int) -> list[dict[str, Any]]:
    views = {view.name: view for view in inference_views()}
    candidates: list[dict[str, Any]] = []
    for name in sorted(views):
        if not name.startswith(("h+00", "h+45")):
            continue
        ...
        windows = spread_pick(
            building_windows(
                ...
                stride=context.args.stride,
                minimum_fraction=context.args.min_building_fraction,
            ),
            limit=context.args.per_view,
            separation=context.args.size,
        )
        for window in windows:
            candidate = _candidate_for_window(context, views[name], name, window, image_path, labels.shape[0])
            if candidate is not None:
                candidates.append(candidate)
    return candidates


def _patch_for(item: dict[str, Any], patches: list[np.ndarray], separation_m: float) -> int:
    for index, centre in enumerate(patches):
        if float(np.linalg.norm(centre - item["centroid"])) < separation_m:
            return index
...
def _write_crop(
    ...
    return Crop(
        ...
        face_area_m2=float(render.areas[item["faces"]].sum()),
...
def _select_crops(
    ...
    candidates: list[dict[str, Any]],
    render: _RenderContext,
) -> tuple[list[Crop], list[tuple[np.ndarray, float]]]:
    ...
    for item in candidates:
        patch = _patch_for(item, patches, args.patch_separation_m)
        wall_name = _wall_for(item, patch, patch_wall, walls)
        if wall_count.get(wall_name, 0) >= args.per_wall or patch_count.get(patch, 0) >= args.per_patch:
            continue
        patch_count[patch] = patch_count.get(patch, 0) + 1
        wall_count[wall_name] = wall_count.get(wall_name, 0) + 1
        records.append(_write_crop(args, out, item, patch, wall_name, images, render))
        print(
            f"{item['crop_id']}  {wall_name}  patch_{patch:02d}  {item['faces'].size} faces  "
            f"{item['range_m']:.1f} m  {item['incidence_deg']:.0f} deg  "
            f"texture {records[-1].texture_pixel_fraction:.2f}"
        )
    return records, walls


def build_facade_crops(args: FacadeCropConfig) -> None:
    """Build matched panorama and tile crops from validated arguments."""
    out = args.out
    ...
    candidates = _collect_candidates(_CandidateContext(args, rotation, camera, geometry, face_class), building_id)
    records, walls = _select_crops(args, out, candidates, _RenderContext(camera, geometry, surface, match, areas))
