"""Choosing a site's capture set, and the weight that stops a union double counting.

The subject is which panoramas a square is photographed with, not where a
pedestrian stands. That is :mod:`semantic_twin.walk` and it is a different
...
from semantic_twin.vision import captures as capture
...
def stations(points, sequence="seq"):
    ...
    return capture.stations_from_images(images, FRAME, target_lat=51.055, target_lon=3.722)
...
def test_radius_and_panorama_filters_are_applied():
    ...
    result = capture.stations_from_images(images, FRAME, target_lat=51.055, target_lon=3.722, radius_m=60.0)
...
def test_measured_gravity_is_mandatory_by_default():
    ...
    strict = capture.stations_from_images(images, FRAME, target_lat=51.055, target_lon=3.722)
    lenient = capture.stations_from_images(images, FRAME, target_lat=51.055, target_lon=3.722, require_rotation=False)
...
def test_select_walk_thins_by_separation_and_prefers_the_near_field():
    result = capture.select_walk(stations([(2.0, 0.0), (3.0, 0.0), (30.0, 0.0)]), count=5, separation_m=8.0)
...
def test_select_walk_caps_a_single_dense_drive():
    ...
    result = capture.select_walk(dense + other, count=4, separation_m=8.0, per_sequence_cap=2)
...
def test_spread_reports_extent_and_baselines():
    report = capture.spread(stations([(0.0, 0.0), (30.0, 0.0), (0.0, 40.0)]))
...
def test_spread_of_an_empty_walk_is_not_an_error():
    assert capture.spread([]) == {"n_stations": 0}
...
def test_parallax_is_the_angle_at_the_surface_not_the_baseline():
    ...
    near = capture.parallax_angles_deg(pair, np.array([[0.0, -5.0]]))
    far = capture.parallax_angles_deg(pair, np.array([[0.0, -100.0]]))
...
def test_two_co_located_cameras_count_as_one_observation():
    ...
    weight = capture.parallax_independence(parallax, visible)
    assert weight[0].tolist() == pytest.approx([0.5, 0.5])
    assert capture.effective_looks(weight)[0] == pytest.approx(1.0)


def test_widely_separated_cameras_keep_their_full_weight():
    parallax = np.array([[[0.0, 90.0], [90.0, 0.0]]], dtype=float)
    weight = capture.parallax_independence(parallax, np.ones((1, 2), dtype=bool))
    assert weight[0].tolist() == pytest.approx([1.0, 1.0], abs=1e-6)
    assert capture.effective_looks(weight)[0] == pytest.approx(2.0, abs=1e-6)
...
def test_effective_looks_never_exceed_the_raw_count():
    ...
    chosen = stations([tuple(point) for point in points])
    parallax = capture.parallax_angles_deg(chosen, surfaces)
    visible = rng.random((50, len(chosen))) > 0.4
    weight = capture.parallax_independence(parallax, visible)
    looks = capture.effective_looks(weight)
...
def test_unseen_surfaces_carry_no_weight_and_no_looks():
    ...
    assert capture.effective_looks(weight)[0] == 0.0
...
def test_independence_weights_are_a_valid_evidence_quality_factor():
    ...
    from semantic_twin.vision.evidence import ObservationQuality

    pair = stations([(-2.0, 0.0), (2.0, 0.0), (40.0, 40.0)])
    parallax = capture.parallax_angles_deg(pair, np.array([[0.0, -30.0]]))
    weight = capture.parallax_independence(parallax, np.ones((1, 3), dtype=bool))[0]
    ones = np.ones(3)
...
def test_invalid_shapes_are_rejected():
    with pytest.raises(ValueError):
        capture.parallax_angles_deg(stations([(0.0, 0.0)]), np.zeros((2, 3)))
    with pytest.raises(ValueError):
        capture.parallax_independence(np.zeros((1, 2, 3)), np.ones((1, 2), dtype=bool))
    with pytest.raises(ValueError):
        capture.parallax_independence(np.zeros((1, 2, 2)), np.ones((1, 3), dtype=bool))
    with pytest.raises(ValueError):
        capture.parallax_independence(np.zeros((1, 2, 2)), np.ones((1, 2), dtype=bool), decorrelation_deg=0.0)
    with pytest.raises(ValueError):
        capture.select_walk([], count=0)
