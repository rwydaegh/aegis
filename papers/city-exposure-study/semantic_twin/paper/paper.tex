...
the square. This paper measures an exposure ratio: power density at a
...
Median within-square spreads were 3.92, 5.46, and 8.43\,dB. They exceeded the
between-square spread for the first two models but fell 1.15\,dB short for street
cells. The paired illumination-law correction had residual root mean square
...
0.128, 0.061, and 0.188\,dB root mean square, roughly 25 times the Monte Carlo
error in the isotropic case. Image-based facade materials shifted distribution
...
deployment-averaged ratio, and the image-coverage budget that combination makes
possible.
...
discover. It rests on title and abstract indexing across four databases plus
direct reading of the sources cited above, and its coverage of 2023 to 2026
...
than 0.063\,dB. Not one standpoint of the 40 moves half a decibel under any of
...
budget. What the trace itself then approximates reduces to a single one sided
...
that square under most of the illumination models considered, rather than under
all of them, and where a pedestrian stands matters more than which of these
eleven squares they stand in.
...
The eleven-city shifts were not common level changes. Rooftop shifts ran from
...
of $+3.43$\,dB, with a residual root mean square of 0.98\,dB. The former and
...
The old law concentrated power immediately above its lower elevation limit.
Across the eleven squares, the share of the old rooftop answer arriving below
...
towers, which the old law rewarded. The corrected law is therefore close to a
...
the 0.0042\,dB standard error of the walk median, so the design resolved a shift
that remained a small part of one decibel.
...
\begin{table}[!t]
  ...
  \begin{tabular}{lcc}
    ...
    $16\times16$ & $-0.61$ & $-2.32$ \\
    $32\times32$ & $-0.93$ & $-2.85$ \\
    Direct-path floor & $-1.57$ & $-2.97$ \\
    \bottomrule
  \end{tabular}
\end{table}

The direct part fixed the lower bound. It was 0.696 of the rooftop ratio and
0.505 of the street ratio, so no beam aimed at the pedestrian could reduce the
two ratios by more than 1.57 and 2.97\,dB. The $32\times32$ street result reached
96\% of that floor. A nearest-grid codebook sweep has no reported values, and
best-beam selection cannot be recovered from the stored estimator because it
does not retain the paths belonging to one site. Its reduction is bounded above
by the geometric-steering reduction.
...
Diffraction is small for two models and fails that description for the third.
...
An older street-cell evidence-ladder value of 0.079\,dB was the lowest of eight
seed draws. The seed-averaged shift is $0.167 \pm 0.022$\,dB, or 7.6 standard
...
Future work must trace finite sites to measure best-beam codebook selection and
fit diffuse scattering. The bystander study also needs repetition beyond one
square on the current three-interaction configuration.
