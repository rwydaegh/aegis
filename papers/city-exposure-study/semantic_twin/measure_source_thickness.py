...
#: A silhouette point counts as covered if a band point sits within this far of
#: it. Two metres is the same scale as the edge search radius, so the diagnostic
#: asks whether the extractor found the edge rather than whether it found the
#: exact triangle.
...
def prepare(
    ...
    *,
    ...
    """Everything the band filter needs that does not depend on the thickness.

    Returns the surviving sample points and, for each, the lowest and highest
    neighbouring height. Doing this once is what makes a sweep affordable: the
    surface draw, the sky test and the neighbourhood reduction are the expensive
    parts and the thickness enters only in the final comparison.
    """
...
def coverage(band: np.ndarray, silhouette_points: np.ndarray) -> float:
    """Fraction of the visible skyline with a band point within the radius."""
    if band.shape[0] == 0 or silhouette_points.shape[0] == 0:
        return float("nan")
    tree = cKDTree(band)
    near, _ = tree.query(silhouette_points, distance_upper_bound=COVERAGE_RADIUS_M)
