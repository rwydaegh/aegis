...
Fig.~\ref{fig:configuration} shows the configuration. A 360-degree street image
is aligned with a photogrammetric city mesh cropped to a 250~m radius around the
route. Image labels are assigned to the visible mesh triangles without changing
their geometry. Possible transmitters lie along visible rooflines. Fixed
points set the observation positions, and the anatomical body model faces the
...
Table~\ref{tab:routes} lists the study route in each of the ten cities. Each
...
observations. Population and place sampling lie outside the study. A different route
would change both position and body orientation.
...
Two image models are applied in sequence. Mask2Former assigns a Vistas object
class such as building, road, or vegetation to every pixel~\cite{mask2former,vistas}.
SAM~3 Agent performs the agentic AI step by assigning material and vegetation
labels inside compatible object regions~\cite{sam3}. The camera position and
...
Fig.~\ref{fig:flowchart} shows the flowchart of the method.
...
Every route point uses 15~GHz and a circular scene crop with radius
...
We consider a future distributed massive MIMO (DMaMIMO) architecture only to
define possible access-point locations~\cite{wydaeghe2026,ngo2017}. Therefore,
the model uses visible rooflines as possible transmitter locations. The calculation does not model coordinated
transmission or beamforming. It distributes expected transmitters uniformly per
...
uses the angular cells. \emph{Adjoint ray tracing} launches rays from the
observation point and tests visibility from each first surface hit to every
roofline source~\cite{behlouli2014,cocheril2007}. Fig.~\ref{fig:adjoint} shows
...
omitted. Woody vegetation identified by the images does not block rays because
the city geometry has no canopy volume. The supplementary material gives the
...
The calculation keeps each arrival direction until body coupling. For outward
...
Only the diffuse estimate varies between runs. Scalar values and body fields
...
Tokyo Hachiko & $3.66\times10^{-5}$ & 0.009674 & 0.02520 & 0.84 & 3 \\
