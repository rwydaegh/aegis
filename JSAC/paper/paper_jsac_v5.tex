...
The T-pose source is qualitatively worse: at $17.6\,\%$ violation
rate it is the only pose source where the dual-ascent precoder
violates the cap on the order of unconstrained ZF ($10.2\,\%$
...
T-pose fallback & $17.20\,$Gbps & $17.58\,\%$ & $0.0\,\%$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[!t]
  ...
  $\leq 1\,\%$ violation; the T-pose source is qualitatively
  worse at $17.6\,\%$ because the canonical T-pose's $\Qmat^{(u)}$
  does not envelope the actual moving-population operator, so the
  solver under-estimates absorption and the precoder pushes more
  power than the posed body can absorb in some directions.
