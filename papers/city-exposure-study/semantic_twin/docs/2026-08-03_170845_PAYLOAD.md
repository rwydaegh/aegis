...
The verdict follows the versioned registration gate. A camera is inside the
geometry only when more than 50 percent of sky directions return a mesh hit AND
their median range is below 2 m. A fraction above 50 percent at a larger range is
labelled `large sky-mesh mismatch`. The payload also shows residual and
altitude-bound refusals. Korenmarkt's sealed payload predates this v2 display
rule and remains tied to its v1 manifest.
