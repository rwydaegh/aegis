...
STATION = paths.output("station_calibration")
EXPOSURE = paths.legacy_exposure_dir()
BOUNCE = paths.output("bounce_budget")
...
class StationCalibrationSummaryConfig:
...
def _rung_shifts(per_rung: dict[str, Any], open_air: np.ndarray, baseline: str) -> dict[str, Any]:
    shifts: dict[str, Any] = {}
    for reference in dict.fromkeys([baseline, "walk"]):
        if reference not in per_rung:
            continue
        base = per_rung[reference]["per_seed"]
        shifts[f"against_{reference}"] = {
            materials: {
...
def _convention_block(entry: dict[str, Any], per_rung: dict[str, Any], baseline: str) -> dict[str, Any]:
    ...
    block: dict[str, Any] = {
        ...
        "shifts": _rung_shifts(per_rung, open_air, baseline),
    }
    if baseline in per_rung:
        rooftop = per_rung[baseline]["per_seed"]["chi_rooftop"]
        block["absolute_chi_rooftop"] = {
            "station_median_per_seed": [float(np.median(values)) for values in rooftop],
            "open_air_station_median_per_seed": [
                float(np.median(np.asarray(values)[open_air])) if np.any(open_air) else None for values in rooftop
            ],
            "rung": baseline,
        }
    return block
...
def site_row(site: str, crop_m: int, tag: str, seeds: tuple[int, ...], baseline: str) -> dict[str, Any] | None:
    ...
    for convention, entry in evidence["conventions"].items():
        ...
        row["conventions"][convention] = _convention_block(entry, per_rung, baseline)
...
def _print_tables(rows: list[dict[str, Any]], baseline: str) -> None:
    for convention in ("pose", "pedestrian"):
        for subset in ("all_stations", "open_air_stations"):
            for rung in ("walk", "semantic"):
                for key in MODEL_KEYS:
                    built = table(rows, convention, baseline, rung, key, subset)
                    if built:
                        print(f"\n### {convention}, {subset}, {rung} rung against {baseline}, {key}\n")
                        print(built)


def execute(config: StationCalibrationSummaryConfig) -> int:
    rows = [
        row
        for row in (site_row(site, config.crop_m, config.tag, config.seeds, config.baseline) for site in config.sites)
        if row
    ]
    if not rows:
    ...
    path = STATION / f"station_calibration_cross_site_{config.crop_m}m{config.tag}.json"
    path.write_text(json.dumps(summary, indent=2))

    _print_tables(rows, config.baseline)
