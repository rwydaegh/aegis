...
cap-violation rate trends upward with $\sigma_{\mathrm{joint}}$
across the range, with sub-percent slot-to-slot fluctuation
between adjacent grid points (single-seed). Sum-rate sits at the
MCS28 sum-cap of $74.0\,$Gbps across the full sweep, since the
SINR-margin condition holds throughout. The operationally-honest
target sits at $\sigma_{\mathrm{joint}} \in [1.5^\circ, 8^\circ]$,
the consumer-IMU band: in this band the violation rate stays
below the T-pose-fallback asymptote without requiring
...
\begin{figure}[!t]
  ...
  cap-violation rate against the ground-truth $\Qmat^{(u)}$, trending
  upward with $\sigma_{\mathrm{joint}}$ at single-seed slot-to-slot
  fluctuations of a few tenths of a percent. Green dashed: mean
  sum-rate, flat at the MCS28 sum-cap of $74.0\,$Gbps across the
  sweep. The T-pose-fallback asymptote (dotted) sits above the
  measured violation rate across the deployment-realistic
