...
\begin{equation}\label{eq:master-law}
  ...
  O(\rr,\khat)\;}
\end{equation}
where $\ReLU(\mu)=\max(0,\mu)$ and $O(\rr,\khat)\in[0,1]$ is the visibility
factor. Each factor is independent and is refined separately in later sections.
\end{proposition}
...
\begin{itemize}[leftmargin=1.4em,itemsep=1pt]
  ...
  \item $O(\rr,\khat)\in[0,1]$: the visibility or occlusion factor, unity for a
