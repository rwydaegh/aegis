...
class SeedReplicaConfig:
    reference: str = "clean_geometric"
    seeds: tuple[int, ...] = (7, 8, 9, 10, 11, 12, 13, 14)
    ...
    baseline: str = "clean_geometric"
    rung: str = "clean_walk9"
...
def execute(config: SeedReplicaConfig) -> None:
    if config.analyse_only:
        analyse(config.baseline, config.rung, config.frequency_hz, config.replicas)
        return
    replicate(
        config.reference,
        config.seeds,
        frequency_hz=config.frequency_hz,
        walk_npz=config.walk_npz,
