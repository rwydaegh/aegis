"""Write the authenticated ten-site geometric fixed-grid screening report."""
...
from semantic_twin.report.geometric_grid_screening import CampaignInput, write_geometric_grid_screening
...
def main(argv: list[str] | None = None) -> int:
    """Validate inputs and write fixed-grid screening artifacts."""
    parser = _parser()
    parsed = parser.parse_args(argv)
    try:
        inputs = [CampaignInput(site, Path(directory)) for site, directory in parsed.campaign]
        artifacts = write_geometric_grid_screening(inputs, parsed.output)
