...
def test_a_throttle_is_retried_where_other_client_errors_are_not(monkeypatch) -> None:
    ...
    """
    from semantic_twin.acquire import streetview
    ...
    monkeypatch.setattr(streetview.urllib.request, "urlopen", throttled_then_ok)
    monkeypatch.setattr(streetview.time, "sleep", lambda _seconds: None)
    client = streetview.StreetViewTiles("key")
...
def test_a_forbidden_request_is_not_retried(monkeypatch) -> None:
    ...
    monkeypatch.setattr(streetview.urllib.request, "urlopen", forbidden)
    monkeypatch.setattr(streetview.time, "sleep", lambda _seconds: None)
    client = streetview.StreetViewTiles("key")
