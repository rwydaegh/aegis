...
from semantic_twin.acquire.streetview import StreetViewTiles
from semantic_twin.scene.site_config import google_api_key
...
def screen_candidates(
    ...
    *,
    ...
    """Screen the requested candidates and write the ranked JSON and table."""
    candidates = [c for c in CANDIDATES if only is None or c.key in only]
    if not candidates:
        return None

    source = screening_source(api_key if api_key is not None else google_api_key())
    ...
    (out / "screening.json").write_text(json.dumps(document, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    (out / "screening_table.md").write_text(markdown_table(ranked), encoding="utf-8")
    print(f"[done] {len(rows)} candidates, {source.requests} metadata requests -> {out / 'screening.json'}")
    return document
