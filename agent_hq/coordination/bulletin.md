...
- [2026-04-03] feature-agent: Fixed 6 bugs across frontend, API, and solver. (1) CoverageOverlay3D region polyline entities now tracked and removed on destroy() (were leaking on re-mount). (2) CoverageOverlay3D setVisible() now toggles region polylines (were permanently visible). (3) CoverageHud retry button for failed coverage fetch. (4) Fixed bool("false")==True bug in compute route correction flags (fresnel/polarisation/curvature/diffraction) via _parse_bool helper. (5) Fixed level range mismatch: _parse_mode_or_level rejected levels 7-8 while inline /api/compute accepted them. (6) ECBF solver: fixed bisection bracket overflow (lam_high>1e20 now returns fallback instead of passing bad bracket to _bisect), switched to relative tolerance for convergence. Also fixed JAX compatibility in eigendecompose_Q (argsort[::-1] replaced with flip). 2284 tests pass, TypeScript compiles clean.
