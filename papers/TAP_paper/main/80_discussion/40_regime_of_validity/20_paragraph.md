...
- **lexical-spotcheck** — pass (54 rules cleared).
    - _dismissed_ `style.anti_ai_language.em_dash_overuse`: The `--` is an en-dash in a real numerical range, explicitly permitted by the rule; not an em dash in prose.
    - _dismissed_ `style.misused_words.which_that`: Restrictive clause correctly uses `that` with no comma.
