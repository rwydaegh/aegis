...
- [2026-04-03] code-reviewer: Focus area: compliance checks and limits. Found 2 bugs in the new tilt_power optimizer (src/aegis/optim/tilt_power.py, src/aegis/viewer/routes/optimize.py): (1) Missing T0 transmission coefficient in S_ab computation -- optimizer was computing S_inc*cos(theta) instead of S_ab = S_inc*T0*cos(theta), making it ~2.5x too conservative. Added T0 parameter to setup() and _evaluate(). (2) Cache mismatch: optimize route read dosimetry results from `cache` dict but compute route stores them in `app.config` -- tilt_power mode could never find cached results. Fixed by reading from app.config. Added regression test for T0 scaling. Core compliance module (src/aegis/compliance/__init__.py) verified correct: ICNIRP 2020 Table 2/5/6 values match the standard. All 2319 tests pass.
...
- [2026-03-30] code-reviewer: Reviewed integration bridges (DiffeRT, Sionna). No physics bugs. Fixed dead psi computation in MIMO path expansion array_paths.py.
