...
def plot_crop_convergence(site: str, out: pathlib.Path) -> None:
    payload = json.loads((out / f"{site}_crop_convergence.json").read_text())
    rows = payload["rows"]
    ...
    upper.set_title(f"{site}, {rows[0]['observers']} fixed observers, only the surroundings change")
    ...
    for suffix in ("png", "pdf"):
        figure.savefig(out / f"{site}_crop_convergence.{suffix}", dpi=170)
    print(f"wrote {out / f'{site}_crop_convergence.png'}")
