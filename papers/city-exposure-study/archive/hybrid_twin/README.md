...
"intelligent semi-manual hybrid" of Inhouse Earth photogrammetry and OSM vectors,
instead of choosing one mesh source for ray tracing?

## Why

Photogrammetry (Inhouse 3D Tiles): real heights, real terrain, real landmark
...
1. `download_inhouse_tiles.py` - traverses the Inhouse Photorealistic 3D Tiles tree
...
python download_inhouse_tiles.py --lat 51.0550 --lon 3.7220 --radius 200
...
- `route_foot.py` - pedestrian routing on the OSM street graph (Dijkstra over
  foot-tagged ways). Needed because the key's Inhouse project has neither the
...
- Legacy `outdoor_environment.py` (hybrid-QuaDRiGa-FDTD): resurrected headless
  ...
  map pre-fetch). Remaining blocker: this blosm version lands Inhouse tiles
...
- `route_inhouse.py` - the walk now comes from the Routes API (computeRoutes,
...
- `render_facades.py` - street-level Cycles shots of 32 walk-corridor
  ...
  is the deepest LOD Inhouse serves here (~25 cm/texel), so facades resolve at
...
- Tile GLBs and renders stay untracked (Inhouse ToS on caching tile content).
