...
- The facade occupies ``z <= 0`` with outward normal ``+z``, matching
  :mod:`semantic_twin.floquet`. Illumination arrives from ``z > 0``.
...
Validity. The method is exact for the permittivity profile it is given. Its
errors are the Fourier truncation, which is reported by
:func:`convergence_sweep`, and the assumption of a strictly periodic, infinite,
plane-wave-illuminated structure. Disorder and finite illumination are handled
in :mod:`semantic_twin.kirchhoff`, not here.
"""
