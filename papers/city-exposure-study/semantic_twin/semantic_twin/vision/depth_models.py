...
def run_unidepth(
    ...
    *,
    ...
    image_paths = sorted(views.glob("*.jpg"))
    if limit is not None:
        image_paths = image_paths[:limit]
    if not image_paths:
        raise FileNotFoundError(f"no JPEG views in {views}")

    model = UniDepthV2.from_pretrained(f"lpiccinelli/{model_name}").cuda().eval()
    ...
    for image_path in image_paths:
        ...
        log_sigma = relative_error_to_log_sigma(relative_error, reference_log_sigma=reference_log_sigma)
        output = out / f"{image_path.stem}.npz"
        np.savez_compressed(
            output,
        ...
        uncertainty_visual(relative_error).save(out / f"{image_path.stem}_uncertainty.png")
        manifest.append(
            {
    ...
    (out / "manifest.json").write_text(
        json.dumps(
            {
                "model": f"lpiccinelli/{model_name}",
                "source": str(views),
                ...
                "reference_log_sigma": reference_log_sigma,
                "views": manifest,
            },
            indent=2,
        )
