...
This split describes the cohort. It is not the admission rule by itself. The
inside-geometry state requires a conflict above 0.5 AND a median conflict range
below 2 m. A larger distant mismatch has its own state. Version-2 production
also requires complete diagnostics and an interior altitude optimum.
