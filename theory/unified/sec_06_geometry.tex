...
\begin{definition}[Exposure fraction]\label{def:exposure-frac}
The exposure fraction at $\rr$ is the cosine-weighted fraction of the hemisphere
from which $\rr$ is visible,
\begin{equation}\label{eq:exposure-frac-def}
  \Omega(\rr) = \frac{1}{\pi}\int_{S^2}
  \ReLU\!\bigl[\nhat(\rr)\cdot(-\khat)\bigr]\,O(\rr,\khat)\,\diff\Omega(\khat).
\end{equation}
\end{definition}

For convex bodies $\Omega\equiv 1$. For non-convex bodies $\Omega<1$ in concavities
and $\Omega=1$ on exposed regions. The exposure fraction is mathematically
...
For any body with surface $\Sigma$ and exposure fraction $\Omega$, the
...
\begin{equation}\label{eq:gen-cauchy}
  ...
  = \Sinc\,T_0\,\frac{\Aab}{4},
  \qquad
  \Aab \equiv \int_\Sigma \Omega(\rr)\,\diff A .
\end{equation}
\end{theorem}
...
for convex bodies $\Omega\equiv 1$ and $\Aab=A$, recovering $\langle\Aperp\rangle =
