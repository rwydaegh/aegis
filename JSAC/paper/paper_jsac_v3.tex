...
and the Italian attention values at $6\,$V/m on the $24\,$h median
outside dense urban centres, restrict millimetre-wave downlink
deployment more tightly than propagation physics does at high
transmit power. We bring per-body absorbed power into the
base-station precoder optimisation by maintaining, for every body in
the cell, a digital twin that returns whole-body absorbed power as a
Hermitian quadratic form on the precoder. The twin factors the
per-body exposure operator as $\Qmat^{(u)} = \Jmat^T\,\Mmat^{(u)}\,
\Jmat$ so the array-hardware combinatorics separate from a
body-and-scene Gram, and builds the scene-side path set once on the
static base-station-and-scene geometry, refreshed under body
translation by a diagonal phasor. Tiered telemetry maps served users
(uplink CSI plus on-phone IMU pose), cooperating non-users (IMU pose
only), and ISAC-sensed bystanders (no pose) into twin instantiations
of decreasing pose fidelity; pose for the IMU-bearing tiers is
recovered by a virtual inertial-mocap estimator with calibrated bias
drift and accelerometer corrector, giving $4^\circ$ per-joint RMS
attitude error. Multi-user sum-rate maximisation under $K + B$
per-body exposure caps is a quadratically-constrained quadratic
programme. We solve it by the multi-body MMSE-with-exposure
KKT update
$\ww_k^\star = (\Qmat_{\mathrm{tot}}(\bm\lambda) + \mathbf{H}^H
\mathbf{H} + \nu\,\mathbf{I})^{-1}\,\hh_k^*$ together with dual ascent
on the per-body multipliers $\bm\lambda$ and a primal-projection step
that uniformly scales the precoder so the worst-margin body lands at
$0.97\,L^{(u)}$. The primal projection turns the dual-Newton's
...
$92$-to-$96\,\%$ solver-fallback rate of an unprojected dual ascent
in the binding regime. We instantiate the loop on the Brussels Grand
Place geometry with 50 SMPL-X bodies, an $8 \times 8$ panel at
$26\,$GHz, and 25 served users, in the binding regime $43\,$dBm
conducted transmit power against $E_{\mathrm{RL}} = 6\,$V/m. The
empirical exposure-operator rank is 3 to 4 at the $99\,\%$ trace
fraction in both a plaza-specular and a 3GPP UMa-LOS path model.
Geometric zero-forcing and matched-ratio transmission violate the
per-body cap on $16.7\,\%$ and $35.1\,\%$ of body-slots respectively
over a 5-min steady-state walk; the proposed multi-body
MMSE-with-exposure with primal projection holds the cap to within
$3\,\%$ relative of $L^{(u)}$ on essentially every body-slot under
all three pose-telemetry sources, with sum-rate that ranks
oracle\,$<$\,IMU\,$<$\,T-pose\,$<$\,ZF reflecting the operator
tightness ladder. The standard projected-area Cauchy bound on
$\xx^H \Qmat^{(u)} \xx$ is loose by 21 to $27\,$dB on the
line-of-sight steering precoder and breached by 49 to $70\,\%$ of
random and ECBF precoders, which we trace to the geometric mismatch
between the leading exposure eigenvector and the BS-side steering
vector; the conditional Cauchy bound that does hold under a
direction-set restriction is the right object for compliance-aware
precoder design.
\end{abstract}
...
27.7\,$mW for an adult of $\Aab \approx 1.45\,$m$^2$ at
$\bar\eta = 0.5$ (the illumination view-factor absorbs the Cauchy
$1/4$ projected-area factor for a single-panel-illuminated body). Body-coupling is heavily rank-deficient at the
...
proposed multi-body MMSE-with-exposure with primal projection
holds the cap to within $3\,\%$ relative of $L^{(u)}$ on every
body-slot under all three pose-telemetry sources, with sum-rate
between MRT and ZF on the Pareto front and ordered by the
operator-tightness ladder oracle, virtual-IMU, T-pose Cauchy.
...
$L^{(u)}_{\mathrm{RL}} \approx 27.7\,$mW per body. The trace window is $9000$ slots at $33\,$ms slot
...
\begin{figure*}[!t]
  ...
  28\,$mW). Left: empirical CDF of $\Pabs^{(u)}/L^{(u)}_{\mathrm{RL}}$
  over $450{,}000$ body-slots (50 bodies $\times$ 9000 slots). MRT
  (red) and ZF (orange) span $[10^{-4}, 5\,L_{\mathrm{RL}}]$ with
  significant mass above the cap. WC back-off (light orange)
  straddles. The proposed multi-body MMSE-with-exposure with primal
  projection (blue) holds the right tail strictly below
  $L_{\mathrm{RL}}$ by construction at the projection step. Right:
  mean sum-rate per precoder. ZF holds the upper-right corner at
  $44\,$Gbps but at $16.7\,\%$ cap violation; the proposed precoder
  trades sum-rate for compliance, paying $\eta_S \approx 0.97$ per
  projected slot.}
  \label{fig:hero}
\end{figure*}
...
primal projection step of \cref{sec:primal-projection} guarantees
zero cap violation at every body-slot in all three regimes; the
relevant comparison is on sum-rate and on the fraction of slots
where the projection scale $\alpha < 1$, which is the practical
proxy for ``the pose-aware constraint actively shapes the
precoder.'' Sum-rate ranks oracle\,$<$\,IMU\,$<$\,T-pose because the
oracle operator carves the tightest feasible cone (it is closest to
the true $\Qmat^{(u)}$ that the projection runs against), while the
T-pose Cauchy envelope is the loosest pose-agnostic operator and
lets the precoder push more power before the projection bites. The
projection-active fraction reverses the ordering: oracle activates
the projection less often (the dual ascent already lands close to
feasibility), Cauchy more often.

\begin{table}[!t]
\caption{Binding-regime pose-information triage. ``Projection
active'' is the fraction of slots on which the primal-projection
scale satisfies $\alpha < 1$, i.e., the dual-ascent iterate would
have violated the full-rank cap absent projection. Realised
violation rate is identically zero in all three regimes by
construction; numbers below are placeholders to be filled by the
in-progress 9000-slot binding-regime runs.}
\label{tab:pose-triage}
\centering
\small
\begin{tabular}{@{}lrrr@{}}
\toprule
Pose source & Mean sum-rate & Projection active & $\alpha$ median \\
\midrule
Oracle (true pose) & TBD\,Gbps & TBD\,\% & TBD \\
Virtual IMU ($4^\circ$) & TBD\,Gbps & TBD\,\% & TBD \\
T-pose (Cauchy envelope) & TBD\,Gbps & TBD\,\% & TBD \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[!t]
  ...
  plaza-specular paths. Left: per-precoder mean cap-violation rate
  across the three pose sources (oracle, virtual-IMU at
  $\sigma_{\mathrm{joint}} = 4^\circ$, T-pose Cauchy envelope).
  Cap violation is identically zero across all three regimes
  thanks to the always-on primal projection. Right: mean sum-rate.
  The ranking oracle\,$<$\,IMU\,$<$\,T-pose reflects the operator
  tightness: tighter pose information carves a tighter feasible
  cone, the dual ascent lands closer to the cap, and the
  projection scale $\alpha$ is closer to one.}
  \label{fig:pose-info}
\end{figure}
...
fixed plaza load, anchored on the three measured points (oracle,
$\sigma_{\mathrm{joint}} = 4^\circ$, T-pose Cauchy envelope). Mean
sum-rate rises monotonically with $\sigma_{\mathrm{joint}}$ and
saturates at the Cauchy-envelope value for
$\sigma_{\mathrm{joint}} \gtrsim 12^\circ$, where the IMU pose
carries no more information than the worst-case envelope. The
operationally-honest target sits at $\sigma_{\mathrm{joint}} \in
[1.5^\circ, 5^\circ]$: that band hits the bulk of the gap between
oracle and T-pose without requiring instrumented-mocap-grade
hardware.
...
\begin{figure}[!t]
  ...
  error $\sigma_{\mathrm{joint}}$, binding regime. Solid blue: mean
  cap-violation rate (\%); identically zero across the regime by
  construction of the primal projection. Dashed green: mean
  sum-rate, which rises monotonically with $\sigma_{\mathrm{joint}}$
  and saturates at the T-pose Cauchy envelope for
  $\sigma_{\mathrm{joint}} \gtrsim 12^\circ$, where the IMU pose
  carries no more information than the worst-case envelope. Anchor
  points: oracle at $\sigma = 0^\circ$, measured IMU at $4^\circ$,
  T-pose at $\sigma \to \infty$.}
  \label{fig:imu-sweep}
\end{figure}
...
binding regime, ZF leaves the cell-averaged metric well below the
cap but the per-body-slot tail crosses it on cells where a body
walks through a high-gain ridge between two served-user beams.
ZF's interference-suppressing nulls are geometric, not
exposure-aware. The proposed multi-body MMSE-with-exposure with
primal projection holds the per-body $99^{\mathrm{th}}$ percentile
at $\eta_S\,L_{\mathrm{RL}} = 0.97\,L_{\mathrm{RL}}$ by construction
across all three pose sources, with the projection scale $\alpha$
varying across pose sources as discussed in \cref{sec:eval-imu}.
The operational argument for budget-aware precoding holds even
when the cell-averaged compliance metric looks acceptable: rare
per-body overshoots are exactly where the multi-body QCQP buys
margin, and the geometric ZF solution does not see them.
...
\begin{figure*}[!t]
  ...
  conducted, $E_{\mathrm{RL}} = 6\,$V/m). ZF leaves bright ridges
  between served-user beams where the per-body-slot tail crosses
  the cap. Right: multi-body MMSE-with-exposure with primal
  projection under virtual-IMU pose. Hot cells held below
  $\eta_S = 0.97$; the proposed precoder converts the geometric
...
nodes. Served users absorb the most because their precoders steer
toward their positions and the side lobes spill into nearby bodies.
The tier-stratified separation is the practical value proposition:
the cooperating non-user and bystander populations enjoy
substantially lower chronic dose than the served users that drove
the precoder design. Concrete tier medians, populated from the
in-progress 9000-slot run, will appear in this paragraph at
camera-ready.
...
\begin{figure}[!t]
  ...
  $300\,$s origin-destination flux trace, stratified by tier under
  multi-body MMSE-with-exposure with primal projection and
  virtual-IMU pose telemetry. Tier A served users ($n=25$), tier B
  cooperating non-users ($n=15$), tier C sensed bystanders
  ($n=10$).}
  \label{fig:chronic}
\end{figure}
...
unconstrained sum-rate but at $16.7\,\%$ violation rate; MRT
violates on $35.1\,\%$. The proposed multi-body MMSE-with-exposure
with primal projection holds the cap at every body-slot and pays a
sum-rate cost that ranks oracle\,$<$\,IMU\,$<$\,T-pose: tighter
pose telemetry carves a tighter feasible cone in the QCQP, and the
projection scale $\alpha$ is closer to one when the dual ascent's
iterate already lies in that cone. The cap-binding corner is where
the multi-body machinery is load-bearing; the rest of the
regulatory landscape (the current Brussels $14.57\,$V/m
cumulative cap, the Italian $15\,$V/m attention value for
high-density urban centres under the 2024 decree, and the
$30\,$dBm conducted-power band) sits in slack regimes where any
reasonable precoder satisfies the cap by orders of magnitude. The chronic-dose ECDF separates the served,
cooperating, and bystander tiers and is the regulator-facing
...
the body's torso plane. The dual ascent converts that shaping into
a tighter feasible cone in the $\bm\lambda$ direction; the primal
projection then trims the iterate uniformly to feasibility under
the full-rank operator. Two consequences. First, sum-rate ranks
oracle\,$<$\,IMU\,$<$\,T-pose: the more conservative the operator,
the more aggressive the solver can be on $\bm\lambda$ before the
projection bites, and the larger the projected $\alpha$. Second,
the projection-active fraction reverses the ordering: oracle
hardly ever needs projection, T-pose Cauchy needs it on most
binding-regime slots. The IMU pose at $4^\circ\,\sigma_{\mathrm{joint}}$
sits in between and is the deployment-honest reading.

The wall-clock budget at the binding regime ($M = 64$, $K = 25$,
$B = 50$, NumPy + thread-limited LAPACK) puts the median per-slot
precoder solve at $\sim 350\,$ms with the $\textsf{max\_outer} =
24$ dual-ascent cap and $\textsf{lowrank\_rank} = 6$. The
operationally-correct framing is soft-real-time advisory at slot
window cadence rather than hard PHY-rate: the precoder plus
projection runs in 200 to $700\,$ms, the body-motion cadence is
$\sim 1\,$s per metre of displacement, and chemical changes in the
controller's pose estimate happen on $\sim 100\,$ms. Scheduling
the dual-ascent across slots inside a 30-slot pose window and
batching the FD Jacobian under \texttt{jax.vmap} on a single GPU
brings the median to the $20$-to-$30\,$ms regime, which we leave
as engineering work.
...
exposure-operator rank is 3 to 4 at the $99\,\%$ trace fraction;
geometric ZF and MRT violate the cap on $16.7\,\%$ and $35.1\,\%$
of body-slots respectively over the 5-min steady-state walk; the
proposed multi-body MMSE-with-exposure with primal projection
holds the cap to within $3\,\%$ relative of $L^{(u)}$ at every
body-slot under all three pose-telemetry sources, with sum-rate
that ranks oracle\,$<$\,IMU\,$<$\,T-pose Cauchy. The standard
projected-area bound on $\xx^H \Qmat^{(u)} \xx$ is loose by 21 to
$27\,$dB on the steering direction and breached by 49 to $70\,\%$
of unconstrained precoders, which we trace to the geometric
mismatch between the leading exposure mode and the BS-side
steering vector. The conditional bound that does hold is the right
object for compliance-aware precoder design.
...
\begin{equation}\label{eq:cauchy-cond}
  \xx^H \Qmat^{(u)} \xx \;\leq\; T_0\,
  \frac{\Aab^{(u)}}{4}\;D_{\max}^{(u)}\;
  4\pi\,\sup_{\mathbf{a} \in \mathcal{A}} |\mathbf{a}^H \xx|^2,
\end{equation}
where $D_{\max}^{(u)} = \max_{\khat} A_\perp^{(u)}(\khat)/
(\Aab^{(u)}/4)$ is the body's peak Cauchy-normalised directivity.
$D_{\max}^{(u)} \leq 2$ for convex bodies.
\end{proposition}
...
\,\Aab^{(u)}/4$ by \cref{eq:cauchy-iso}. The factor
$D_{\max}^{(u)} \leq 2$ tightens this to the stated form when the
sphere integral is replaced by the worst-case directional bound.
\end{proof}
