...
under at $0.31$; projected ZF lands at median $0.19$ (lower than
unconstrained ZF because the projection cuts power on binding
slots and the binding mass is non-negligible) and is the right
operating point for the regulatory window because per-slot
violations are zero on its input set. The dual-ascent multi-body
MMSE-with-exposure sits at median $0.12$ across the same trace,
the smallest of the five precoders, since the dual-variable
shaping concentrates power on directions that miss bodies more
heavily than the uniform scaling does
(\cref{tab:baselines}). The ECDF scales linearly with window
