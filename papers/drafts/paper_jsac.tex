...
($43\,$dBm transmit, $L_{\mathrm{RL}} \times 0.1$): the same $5$-min
trace is re-executed with the tier-A and tier-B exposure operators
...
and therefore exhibit identically zero $\Delta$\textsf{SR}. The
constrained QCQP (multi-body ECBF and the per-body oracle variant)
exhibits a bimodal slot-level distribution: $\sim 10\%$ of slots sit
at $\Delta\textsf{SR} \approx -10\,$Gbps where the pose-aware solver
fell into min-absorption fallback while the Cauchy variant remained
feasible, $\sim 5\%$ sit at $\Delta\textsf{SR} \approx +20\,$Gbps in
the opposite configuration, and the remaining $\sim 85\%$ collapse to
zero where the two methods land at the same operating point. Median
$\Delta\textsf{SR}$ is $-185\,$Mbps for the multi-body ECBF and
$-76\,$Mbps for the oracle. The mechanism is solver-side: the Cauchy
envelope $\Mmat^{\mathrm{Cauchy}}_{(u)} \succeq \Mmat^{(u)}$ defines a
strictly tighter constraint set, so the dual-Newton sweep converges
in fewer iterations and triggers fallback less often
($88.0\%$ versus $96.6\%$ for the proposed solver under the
$\textsf{max\_outer} = 8$ cap of \cref{sec:multibody-ecbf}). The
exposure side tells the opposite story.
...
The pose-aware curve dominates: at every quantile the realised
$P_{\mathrm{abs}}$ is lower than the Cauchy variant. The Cauchy
solver's looser feasible set lets the maximizer push the actual
absorbed power right up to $L_{\mathrm{RL}} = 16.3\,$mW; the pose-aware
solver, working against the tighter posed operator, holds margin in
hand. Pose information shapes the QCQP feasible set tighter and
lowers realised exposure, at the cost of more frequent solver
fallback under the present iteration cap. Warm-starting
$\bm\lambda^{(t)}$ from $\bm\lambda^{(t-1)}$ and uncapping the outer
Newton sweep would close this gap, and is the most direct follow-up
the present data points to.

\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{pose_info_gain.pdf}
  \caption{Pose-information ablation, plaza-specular path model,
  binding regime ($43\,$dBm, $L_{\mathrm{RL}} \times 0.1$). Left:
...
the binding regime the operator binds and pose information becomes
a real lever, with two distinct effects. The pose-aware operator
defines a strictly tighter feasible set than its Cauchy envelope,
which lowers realised $P_{\mathrm{abs}}$ at every quantile under
the proposed solver, but the tighter set also makes the
dual-Newton sweep harder to converge inside the
$\textsf{max\_outer} = 8$ cap, raising the min-absorption fallback
rate from $88\%$ to $97\%$. The slot-level sum-rate gap is bimodal
in consequence rather than monotone in either direction. The
honest reading is that pose telemetry buys a tighter exposure
profile in exchange for a more delicate solver, and that warm-
starting $\bm\lambda$ across slots and uncapping the Newton sweep
are the next move. The same logic applies to the multi-body solver
structure: it is a generalisation that contains MRT, ZF, and
worst-case back-off as limits and degrades to one of them in the
slack regime without a code path change.
...
walks, and a binding-regime pose-information ablation in which
pose-aware operators yield strictly lower realised exposure at
every quantile while exchanging some sum-rate against
unconverged constrained-QCQP slots. The standard
