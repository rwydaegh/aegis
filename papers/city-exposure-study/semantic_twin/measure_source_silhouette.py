...
over weighted. Thinning to one site per occupied cell of a horizontal grid undoes
that without a correction factor: a cell holds one site whether one ray found it
or a thousand did. That is also why no obliquity term appears anywhere here.
"""
...
def thin(points: np.ndarray, cell_m: float) -> np.ndarray:
    """One point per occupied cell of a horizontal grid.

    Cells rather than a fixed count, so the surviving density is uniform per unit
    of ground the roofline covers however unevenly the fan sampled it.
    """
    if points.shape[0] == 0:
        return points
...
def main() -> None:
    ...
    for row in rows:
        ...
        for name, key in (("cell", "cell_m"), ("builders", "builders")):
            ...
            case = [
                c for c in row["per_case"] if c["azimuths"] == finest and c[fixed] == hold
            ]
            if len(case) < 2:
                continue
