...
MESH_RAY = "mesh, by ray"
MESH_VERTEX = "mesh, by vertex"
MESH_VERTEX_FILTERED = "mesh, by vertex, filtered"
...
def statistics(curves: dict[str, np.ndarray]) -> dict:
    ...
    reference = curves[MESH_RAY]
    for name in ("panorama", MESH_VERTEX, MESH_VERTEX_FILTERED):
...
def draw(
    ...
    style = {
        "panorama": ("#d62728", 1.1, "-"),
        MESH_RAY: ("#1f77b4", 1.1, "-"),
        MESH_VERTEX: ("#7f7f7f", 0.8, "-"),
        MESH_VERTEX_FILTERED: ("#2ca02c", 0.9, "--"),
    }
    for name, curve in curves.items():
...
def plot_skyline_functions(args: Any) -> None:
    ...
    for site in args.sites:
        ...
        curves = {
            ...
            MESH_RAY: ray_curve(geometry, camera, grid),
        }
        ...
        curves[MESH_VERTEX] = resample(centres[order], raw[order], grid)
        curves[MESH_VERTEX_FILTERED] = resample(centres[order], filtered[order], grid)

        stats = statistics(curves)
        ...
        delta = np.degrees(curves["panorama"] - curves[MESH_RAY])
    ...
    if panels:
        ...
        for ax, (site, curves) in zip(axes, panels):
            for name, curve in curves.items():
                if name == MESH_VERTEX:
                    continue
                colour = {"panorama": "#d62728", MESH_RAY: "#1f77b4"}.get(name, "#2ca02c")
