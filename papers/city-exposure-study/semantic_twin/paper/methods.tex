...
spreading instead of counting sites. Over a plausible range, 25 to 100 sites per
...
eight registered stations at the reference site. That confirms the geometric
...
Of all launched power, 79\,\% interacts once, a further 8.9\,\% twice, 1.2\,\%
three times, and 0.22\,\% ever again. Depths past the third can be justified
...
value reported here is a lower bound in that sense. Russian roulette is the
...
absorption rate. Two things are left out of this step, and the first matters more than its size
...
it. The second omission is that the body is treated as convex, so one limb never
...
interactions $L$ was raised to eight with the budget the only thing varying, and
the threshold was that no standpoint move by 0.5\,dB. None did, at any budget
from three upward, which is the measurement quoted in
Section~\ref{sec:budget}. That comparison needs one remark, because the obvious way to read it is
