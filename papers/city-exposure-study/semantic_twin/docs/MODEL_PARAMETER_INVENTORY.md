...
| Comparable-city v2 transport | 250 m, 15 GHz, 200000 IID rays, 4096 passive output cells, exact direct, stochastic diffuse/mixed next-event, adaptive order-1 all-specular plus sampled mixed suffix, 120000000 logical candidates by default, a recorded 320000000 Tokyo cap for exact zero-path enumeration, 262144 chunk, 0.02 tolerance, one suffix sample with offset 2000, exact mirrored-receiver triangle-cone broad phase at computational threshold 20000 and source chunk 16, finish-only roughness, physical 3D edge-length baseline, level-2 body with explicit CUDA Float64 reduction or NumPy reference | `parameters.study_transport.comparable_city_v2_production` | comparable production transport and coupling | canonical | yes | horizontal projected edge length is a sensitivity. 4096 cells are not launch bins. Candidate caps and broad-phase controls are computational, not scientific. Body backend is recorded in provenance. Certified CUDA broad phase leaves the host exact final kernel unchanged. | Current contract aggregate. Tokyo fully enumerates 319.046 million order-1 candidates rather than defining relative error around zero. |
...
| Comparable runtime anchors | The integrated optimized Korenmarkt provider-corridor campaign completed 16 replicas over 10 standpoints in 83.37 s wall, with 61.584 s accumulated estimator, 42.119 s stochastic, 13.317 s specular, and 2.041 s body work. Landed seed-independent device kernels separately measured 3.14-3.24 s for the 14-point stochastic stage versus roughly 62 s before the fix. CUDA level-2 body coupling is exact to 6.64e-16 relative error, 46x faster on Duke and 81x on the real-point benchmark. Persistent Korenmarkt cache is 74.09 to 38.90 s wall and 39.22 to 4.35 s estimator with byte-identical arrays. Older route and Prague anchors remain historical | `parameters.runtime_and_visualisation.comparable_runtime_anchors` | measured implementation timing | diagnostic | no | measured stage and integrated anchors, not free runtime parameters | Values are hardware-specific observations. The 83.37 s run is the current integrated anchor. The roughly 62 s value is a historical kernel-compilation symptom. Route contracts remain different estimands. |
| Current provider-corridor campaigns | Korenmarkt 10 points and 16 replicas in 83.37 s, Prague 22 and 16 in 185.68 s, Madrid 14 and 16 in 115.75 s, Mexico City 11 and 64 in 219.52 s, Tokyo Hachiko 16 and 64 in 325.76 s. Mexico and Tokyo use cached deterministic transport. Tokyo also has a separate exact 16-replica proof of approximately 753 s. | current provider-corridor campaign manifests | multicity result snapshot | diagnostic | no | not a parameter change or a ten-city completion claim | Korenmarkt, Prague, and Madrid 12-to-16 total-transfer changes are 0.00663, 0.00640, and 0.00628 dB. Mexico and Tokyo 48-to-64 changes are 0.20077 and 0.29756 dB, with mean `Sab` changes of 0.17070 and 0.28785. Medians and p90s are stable, but rare no-direct mixed suffix tails remain. |
| Source-conditioned mixed-suffix screen | proposed oracle design, no production value | planned transport estimator redesign | proposed science | diagnostic | yes | not landed | Promotion gates are at least 4x variance-time improvement, at least 10x lower zero-score frequency, no more than 2x point cost, and no detectable bias. |
