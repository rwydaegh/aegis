...
`--walk-path closest` builds both and keeps whichever stands nearer a camera. It
is the default on both runners, and the losing candidate's number is written into
`walk_provenance` as `path_candidates_m`, so the choice is visible without
rerunning it. `--walk-path street` and `--walk-path links` force one.
