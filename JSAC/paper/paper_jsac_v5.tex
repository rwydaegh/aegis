...
$\Qmat^{(u)}$, which rises monotonically with pose-source
mismatch: $0.29\,\%$ under oracle pose, $0.53\,\%$ under
virtual-IMU pose at $\sigma_{\mathrm{joint}} = 4^\circ$, and
$0.75\,\%$ under T-pose fallback. Even the T-pose source remains
well below the $10.2\,\%$ violation rate of unconstrained ZF and
within an order of magnitude of the oracle.
...
\begin{figure}[!t]
  ...
  plaza-specular paths, dual-ascent solver. (a) Cap-violation rate
  against the ground-truth $\Qmat^{(u)}$ trends upward with
  pose-source mismatch: oracle $0.29\,\%$, virtual-IMU $4^\circ$
  $0.53\,\%$, T-pose fallback $0.75\,\%$, all well below the
  $10.2\,\%$ unconstrained-ZF baseline. (b) Mean sum-rate sits at
  the MCS sum-cap of $74.0\,$Gbps in all three pose sources because
