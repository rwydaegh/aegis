...
Sign convention for permittivity follows :mod:`~.rcwa`, that is
``exp(-i omega t)`` and therefore ``eps = eps' + i eps''`` with a positive
imaginary part for a lossy material. :mod:`semantic_twin.materials.itu` uses the
...
class MasonryWall:
    ...
    def rcwa_layers(self, samples_x: int, samples_y: int) -> list:
        """Layer stack for :func:`~.rcwa.solve`.
