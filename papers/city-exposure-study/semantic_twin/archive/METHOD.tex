...
repair costs nothing because it reuses the identical computation. Two arguments
that earlier drafts leaned on are stated here with their range of validity
attached. The closed monostatic paths do form a measure-zero set invisible to
Monte Carlo ray shooting, but that holds only because transmitter and receiver
coincide at a point, and the repair of Section~\ref{sec:repair} removes exactly
that condition, so the pair search ends up as one branch of the estimator rather
than its foundation. And the claim that most reflected power is diffuse rested on
an unsourced millimetre of surface roughness, where the metrology says every
common facade material is nearly specular at $28$~GHz. What survives both
corrections, and does not depend on which observable is computed, is that the
panorama supplies material, roughness and clutter at centimetre scale on the
bounce that carries most of the power. All numbers below come from this
repository.
\end{abstract}
...
\section{Why the co-located problem is a search and not a ray trace}

This is the part that is genuinely clever, and it deserves a careful statement
because it is easy to get backwards. It is also conditional on the co-located
geometry, and Section~\ref{subsec:stops} says where that condition fails.
...
\section{What the search misses, and why we cannot yet say what produces it}

Everything above assumes mirror-like reflection. Real surfaces at millimetre
wavelengths are not perfect mirrors, and the size of the departure was the single
most-revised number in this document.
...
described below says it is between ten and forty times too large.}
\end{table}

That millimetre does not survive contact with the metrology. Every direct
measurement of a brick face returns $0.024$ to $0.095$~mm, a Rayleigh $g$ of
$0.03$ to $0.11$ at normal incidence, which leaves the face $98.8$ to $99.9\%$
specular. The same holds across the
...
That negative is worth more than it looks, because it was reached twice over,
from sources that do not overlap. One route is the metrology: five samples
measured both ways in one study, plus a campaign that states outright that the
diffuse power on its own flat slabs cannot be surface roughness because all three
materials are smooth, plus a 1996 characterisation of real exterior walls at
fifty to two hundred fifty times the coupon values. The other route is the
scattering-model literature, where reproducing a brick wall's measured lobe width
from a Gaussian surface needs a directivity exponent of sixty-five while the wall
itself fits four, and the authors attribute the excess to indentations, brick and
mortar alternation and sub-surface inhomogeneity. Two independent readings, one
conclusion.

The candidate that fits the magnitude is therefore structure rather than
roughness: mortar joints, unit-to-unit offsets and block relief, at a pitch of
seven to fifteen wavelengths. That is a grating with on the order of fifteen
propagating orders, not a smooth lobe, and the two readings imply genuinely
...
\end{equation}
% LAW CHANGE NOTE, 2026-08-03. This file moved to archive/ on the same day.
% See ../docs/LAW_CHANGE.md, and ./README.md for why it moved. The rooftop weighting named
% just below is being replaced by one that reads sites off facade tips rather
% than off a height band and a range band, so chi_roof is a stale label for a
% model that is going. The argument here does not depend on it: the whole point
% of the passage is that the worst-direction bound dominates any Q a reviewer
% proposes, and that holds for the new Q as well. chi_iso is untouched.
