...
def run_blender_local(args: Any, out: pathlib.Path, payload: pathlib.Path) -> None:
    command = [
        str(args.blender),
        "--background",
        "--python",
...
def run_blender_remote(args: Any, out: pathlib.Path, payload: pathlib.Path, tiles: pathlib.Path | None) -> None:
    ...
    subprocess.run(
        ["rsync", "-a", str(payload), str(payload.with_suffix(JSON_SUFFIX)), f"{args.render_host}:{root}/payload/"],
        check=True,
    )
    ...
    if tiles is not None:
        ...
        sidecar = json.loads(payload.with_suffix(JSON_SUFFIX).read_text())
...
def render_showcase(args: Any) -> None:
