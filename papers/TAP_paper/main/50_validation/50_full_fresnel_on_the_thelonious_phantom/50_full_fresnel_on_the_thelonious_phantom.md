% PREV: \Cref{tab:phantom} reports the pointwise comparison. For the
% PREV: $4\,907$ illuminated triangles with
% PREV: $\theta < 75^\circ$, the local statistics are mean error $-2.6\%$,
% PREV: root-mean-square $3.2\%$, and range $[-5.3\%, 0.0\%]$. The peak
% PREV: $\APD$ is recovered exactly because the maximum is at normal incidence,
% PREV: where $\Teff(0) = T_0$ regardless of polarization. The local error is
% PREV: below $5.5\%$ everywhere with $\theta < 75^\circ$.
% PREV: Section~\ref{si:apd-direction} of the SI extends the analysis to
% PREV: $128$ illumination directions and three polarization states. The
% PREV: per-direction distribution of total absorbed power clusters around
% PREV: $T_0\,\Aperp$ within the directional spread set by self-shadowing.
