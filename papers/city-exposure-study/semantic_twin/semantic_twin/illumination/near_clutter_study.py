...
from semantic_twin.illumination.source_studies import skyline_term as term
...
def run(args: Any) -> None:
    ...
    for site in args.sites:
        ...
        for point in points:
            ...
            for floor in args.floors_m:
                per_floor[floor].append(term(alpha, horizontal, found, floor_m=floor))
    ...
    if len(rows) > 1:
        ...
        for floor in args.floors_m:
            order = sorted(rows, key=lambda r, floor=floor: -r["direct_term_by_floor_m"][str(floor)])
