...
distance $s$ from the edge and wavenumber $k$, which at 15\,GHz and 10\,m is
about $1/56$ in
...
required to see open sky, by the filter of Section~\ref{sec:standpoints}. For
the street cell band that closes the question, because
...
the answer at all. \Cref{fig:box} shows the region in the height and
range plane.
...
weights all directions equally, so under it $\chi$ depends on the geometry and
on nothing else, which is what makes it a check on the estimator as well as a
reference. The rooftop model covers macro sites on buildings and the street
...
\subsection{Stratified estimate}
\label{sec:estimator}
...
evaluated, and the reason matters because it decides how many rays a
...
many standpoints does help, by roughly the square root of their number.
...
had passed the skyline residual gate. Re-registering the same camera for the
wider binding takes it to 1.000, which settles the diagnosis: the surfaces were
always photographed and the pose was wrong. First interaction coverage is
therefore a sharper test of a pose than the residual the registration itself
minimises, it costs one trace, and it is available at any square with a
registered panorama. The station is excluded from the coverage figures above under its
original pose. Away from a camera the two
...
The two legs also apply in different places, which decides what carries the
budget where. The cross city comparison is run with geometric classes and no
image evidence at all, so the evidence leg says nothing about it. $L$ is one
operating point for every run in the study, and at the squares with no
photographs behind their materials it rests on the measurement below and on
nothing else.

What makes three sufficient rather than merely defensible is where the power is.
Of all launched power, 21.0\,\% never touches a surface and escapes straight to
the sky, which is what the first term of \eqref{eq:split} collects. The shares
that reach a first, a second and a third surface are 79\,\%, 8.8\,\% and
1.2\,\%, and 0.23\,\% is still finding surfaces past the third. Those are not
parts of one whole and are not to be added, because power that reaches the third
surface was already counted at the first and the second. Each is what is left of
the same launched power at a greater depth. Depths past the third can be
justified neither by evidence nor by the power they carry. Traced against an
eight interaction reference over the 40 standpoints of the published walk at
fixed seeds, so that only the budget moves, $L = 3$ costs a median of 0.002\,dB
and never moves a standpoint by more than 0.063\,dB. Not one standpoint of the
40 moves half a decibel under any of the three illumination models.
...
few survivors twentyfold. A repeated seed test over the same standpoints returns
the same relative spread to three significant figures with it on and off, and
wall times of 101 and 100\,s, so it is switched off for changing neither
variance nor work at this budget. What the trace itself then
...
The reflected term is checked against an independent ray tracer, run on the same
meshes with the same materials. Taking the median over the three illumination
models, the two agree to within 0.25\,dB at the reference square and 0.16\,dB at
a second square, against a Monte Carlo floor on the comparison itself of 0.24 to
0.46\,dB. The right way to report that is that no disagreement is resolvable,
rather than that the two agree to a sixth of a decibel. Two limits travel with
it. The comparison cannot test the illumination law, because $Q$ is common to
both legs and an error in it cancels exactly, so what is tested is the transport
and not the weighting. And the two are matched on roughness rather than on
model: this estimator forms a product of per bounce polarisation averages where
the other forms an average of products, and the residual quoted above contains
that difference rather than excluding it.
