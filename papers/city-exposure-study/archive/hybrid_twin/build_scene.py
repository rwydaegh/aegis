"""Build an aligned Blender scene: Inhouse 3D tiles + OSM extrusions in a local ENU frame.
...
def import_inhouse_tiles(tiles_dir, p0, R, coll):
...
def main():
    ...
    g_coll = get_collection("inhouse")
    o_coll = get_collection("osm")
    import_inhouse_tiles(tiles_dir, p0, R, g_coll)
    ...
    render(rd / "inhouse_top.png", cams["top"], {"inhouse"})
    render(rd / "osm_top.png", cams["top"], {"osm"})
    render(rd / "inhouse_persp.png", cams["persp"], {"inhouse"})
    render(rd / "both_persp.png", cams["persp"], {"inhouse", "osm"})
