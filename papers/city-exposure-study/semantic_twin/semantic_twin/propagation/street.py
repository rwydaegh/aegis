"""The walking path between the cameras, from the routing service.

`route.py` builds a path out of the provider's own panorama links. That path is
dense, about ten metres between points, and it is free. It is also the wrong
path: those links are where the survey car drove, and the squares in this study
are pedestrianised, so the car went around what a person walks across.

This asks Google Routes for one walking path instead, from A to B, where A and B
are the two cameras furthest apart. The answer comes off the pedestrian network
and it crosses the open square rather than skirting it.
...
**It costs money and it is cached.** One request per site per waypoint set. The
response lands in ``data/street_routes/<site>.json`` and is reused, so a rerun
is free. Delete the file to buy it again.
...
def request_key(waypoints: Sequence[tuple[float, float]], optimise: bool) -> str:
    """A name for one request, so the same one is never paid for twice."""
    digest = hashlib.sha256()
    digest.update(json.dumps([[round(v, 7) for v in p] for p in waypoints]).encode())
...
def cached_walking_route(
    ...
    *,
    ...
    The cache is keyed by the waypoints, so adding a camera buys a new route and
    rerunning the same set does not. The old answer is left in place beside the
    new one, because a route is evidence for the run that used it.
    """
    directory = root / "data" / "street_routes"
    path = directory / f"{site}_{request_key(waypoints, optimise)}.json"
...
def site_anchor(site: str, root: pathlib.Path, crop_m: int = 250) -> tuple[float, float]:
    """Where the site's local metres are measured from, read off the mesh manifest."""
    for name in (f"inhouse_leaf_{crop_m}m_f64.json", f"inhouse_leaf_{crop_m}m.json"):
        path = root / "data" / "geometry" / site / name
        if path.exists():
            anchor = json.loads(path.read_text())["anchor"]
            return float(anchor["lat_deg"]), float(anchor["lon_deg"])
    raise FileNotFoundError(f"{site} has no {crop_m} m mesh manifest to take an anchor from")
