...
def test_ws_clarification_reply_enqueues_followup(app, paper_dir):
    """A clarification reply creates a follow-up annotation and enqueues it (clarifications-first)."""
    ...
    class CountingSession:
        ...
        async def send_batch(self, pass_, annotations, page_pngs, build_status_text):
            ...
            if self.calls == 1:
                # First batch: clarify.
                aid = annotations[0].id
                return ({aid: {"status": "needs_clarification", "clarification": "which §?"}}, "ok")
            # Second batch: done.
            return ({a.id: {"status": "done", "one_liner": "ok"} for a in annotations}, "ok")
    ...
    app.state.claude_session = CountingSession()
    app.state.snapshot_mgr = NopMgr()
    app.state.hunk_mgr = NopMgr()
    with TestClient(app) as c:
        with c.websocket_connect("/stream") as ws:
            ...
            # Drain until needs_clarification arrives.
            for _ in range(10):
                m = ws.receive_json()
                if m.get("type") == "annotation_status" and m.get("status") == "needs_clarification":
                    break
            ws.send_text(
                json.dumps(
                    {
            ...
            for _ in range(10):
                ...
                if m.get("type") == "annotation_status" and m.get("status") == "done":
                    return
            assert False, "never got done after clarification reply"
